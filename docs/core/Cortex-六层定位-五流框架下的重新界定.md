...
**定位**：将 v2.7.1 的"约束层/内化层/Engine 容器"三层架构，重新映射到五流框架下的六层模型。每层不再按"暴露不可靠/吸收可靠"分类——而是按**数据流向、责任边界、被哪条原则约束**来界定。
...
治理层不得执行业务操作。它不能写业务文件、不能调用业务工具、不能规划任务。它唯一的"行动"是触发修宪和上报告警——且修宪本身受原则七约束。
...
**认知基底——不是层，是基底。** 记忆层不参与任何单一业务流——它被规划-执行流读写、被治理层校验、被技能-工具层检索。它是所有流的共享基础设施。
...
| ConsistencyLayer | `packages/consistency/src/consistency-layer.ts` | 六层一致性防御 |
...
记忆是索引，不是证据。所有可验证结论必须锚定到文件系统/API 响应/用户确认。cancel() 贯通 Pending→rollback/Active→archive 三层，不留下半成品。
