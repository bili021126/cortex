...
**定位**：五流（交互流/治理流/规划-执行流/技能-工具流/记忆流）在代码库中的精确路径。每个节点标注文件、函数和行号。

**生成日期**：2026-06-19
**生成人**：昔涟（Cyrene），与开拓者共同完成
...
|---|--------|--------|------------|------------|--------|
| **交互流** | — | 用户裁决→修宪 | 用户意图→规划 | 用户确认→放行 | — |
| **治理流** | — | — | 审查执行结果 | 审查工具调用 | 审查记忆一致性 |
| **规划-执行流** | 输出给用户 | emit 事件 | — | 引用技能 | 读/写记忆 |
| **技能-工具流** | 注册技能 | — | 注入 planning | — | 持久化技能 |
| **记忆流** | — | — | 供给上下文 | — | — |
...
*五流是 v2.7.1 宪法 1607 行的蒸馏产物。每个节点对应精确的代码位置。*
