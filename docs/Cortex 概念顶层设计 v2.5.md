...
│   ├── CLIAdapter (CLI 平台桥接，实现 PlatformBridge)
...
> **物理包结构（v2.5.6）**：4 个包——`@cortex/shared`（全部类型定义 + SafeErrorReporter 协议 + Toolkit / FileLockManager / CLIAdapter 基础设施）、`@cortex/llm`（LLM 适配层：LlmAdapter——API 适配、缓存、重试、流式、指纹匹配）、`@cortex/engine`（Engine 执行引擎：Scheduler / MemoryStore / AgentPool / PipelineObserver / ConfirmGate / MetaAgent / 全部 Agent）、`@cortex/testing`（Mock 基础设施）。依赖方向 shared ← llm ← engine ← testing，严格依赖倒置单向无循环。`@cortex/infra` 包在当前代码中实际不存在——Toolkit/FileLockManager/CLIAdapter 归于 shared 层，infra 独立拆分留待 Core-2。Meso-Lite 中曾独立存在的 `@cortex/memory`、`@cortex/meta-agent`、`@cortex/scheduler`、`@cortex/doc-govern` 四个包已删除，功能并入 engine。
...
**文档状态**：v2.5.8。替代 v2.5.7 作为 Core 阶段准入依据。v2.5.7 已归档保留。
