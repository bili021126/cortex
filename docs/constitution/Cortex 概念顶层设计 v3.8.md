...
**版本档案存续**（2026-08-11 实测）：仅 v3.0 与 v2.7.1（经 v2.5.35 文件）有档案，v1.1 / v2.0 / v3.1 / v3.2 / v3.3 / v3.4 / v3.5 / v3.6 共 8 个版本无独立档案——版本治理以 docs/amendments/AM-*.json 与 git 历史为权威源，本行不承诺未归档版本存在。

**生成日期**：2026-06-19（v3.0）→ 2026-06-22（v3.1）→ 2026-06-28（v3.2）→ 2026-07-06（v3.3）→ 2026-07-16（v3.4）→ 2026-07-20（v3.5）→ 2026-07-20（v3.6）→ 2026-08-01（v3.7）→ 2026-08-24（v3.8）
**修正案**：AM-2026-0801-001（v3.6→v3.7，图景盘点全面修订）→ AM-2026-0811-001（v3.7→v3.8，事实勘误型修订）
...
- 测试规模：全仓 vitest 3982 passed / 13 skipped（2026-08-01 实测）；engine 1036 passed / 91 文件（2026-08-11 实跑）
- 全量 tsc -b 零错误（29 包含 src 编译，含 strict + noUncheckedIndexedAccess）
- CI 门禁：五段执行（tsc -b → eslint packages --ext .ts,.tsx --max-warnings 0 → critical-fixes 混沌校验 → vitest unit+verify+contract 按包串行 → coverage 阈值 [--coverage 显式启用]）+ eval 活性层（门禁 4.5/5：pnpm exec tsx packages/engine/tests/eval/eval-gate.ts，report 模式 exit 恒 0），scripts/ci-gate.ts 驱动
...
├── scheduler/        # 调度引擎——TaskBoard/AgentPool/PipelineObserver/ConfirmGate（scheduling 实现已拆分：strategies / drivers / execution-models / model-routers 四文件，行数以 tsc -b 与测试基线为准）
├── shared/           # 共享类型中枢——AgentType 单源枚举 + EventPayloadMap + agent-registry
├── platform/         # 平台抽象——Toolkit + MCP 鉴权
├── config/           # 配置——EngineConfig/常量集中 + 18 配置域（2026-08-11 实测）+ agents.json/manifests
...
内化层（吸收可靠）
├── SkillRegistry     — 27 技能模板 JSON（2026-08-11 实测：skills/ 目录，编号 p10–p41 跳跃）
...
**代码**：`packages/engine/src/core/meta-agent.ts`（行数锚点已去绝对值化——以 tsc -b 编译与测试基线为准，不锁死行数）
...
| 15 | 霜凝 | strategist | deepseek-v4-pro | 方向监理（status=awake） | Core-2 预留 |
| 16 | 烟绯 | confirm-gate | deepseek-v4-flash | 确认门决策——审计级确认与策略放行 | Core-2 预留 |

> 实例数核对：agents.json 16 个实例 / 15 种类型（strategist 双实例：钟离 + 霜凝）；agent-manifests.json 18 个条目（含 shuangning，L3 差异声明层——2026-08-11 实测）。烟绯类型名实测为 `confirm-gate`（非 confirmGate）。
...
**代码**：`packages/engine/src/agents/butler-agent.ts`（行数锚点已去绝对值化——以 tsc -b 编译与测试基线为准，不锁死行数）
...
**代码**：`packages/scheduler/src/core/confirm-gate.ts`（行数锚点已去绝对值化——以 tsc -b 编译与测试基线为准，不锁死行数）

基于可逆性等级（L0-L3）拦截工具调用。L2/L3 永远确认，L1 视信任放行。

### 7.2 工具权限

**代码**：`packages/platform/src/toolkit.ts`（行数锚点已去绝对值化——以 tsc -b 编译与测试基线为准，不锁死行数）
...
**代码**：`packages/engine/src/core/scheduler.ts`（行数锚点已去绝对值化——以 tsc -b 编译与测试基线为准）+ `packages/scheduler/src/core/scheduling-implementations.ts`（已拆分：原文件退化为 12 行转出层，实现迁至 strategies / drivers / execution-models / model-routers 四文件，行数以 tsc -b 与测试基线为准）
...
- **修宪记录** → `docs/amendments/AM-*.json`（修正案全记录，最新 AM-2026-0811-001——2026-08-24 订正：原『AM-0722-003』与版本头『AM-0801-001』自指矛盾，以版本头为准）
...
`@cortex/config` 通过 `CONFIG_DOMAINS` 声明 18 个配置域（2026-08-11 实测）：agents（@deprecated，保留向后兼容）/ engine / enginePlugins / tools / eventRouting / roundtable / searchProviders / mcpServers / selfExamination / crossVerification / seedMemories / governancePipeline / cognition / docs / models / keysContext / agentManifests / tuning。每个域独立声明文件名、是否必需、数据键名与 schema。配置数据文件位于 `packages/config/src/data/`。
...
**27 技能模板 JSON**（2026-08-11 实测）：`skills/*.json`——编号 p10–p41 跳跃（p18–p24 缺号），p17 双文件重复编号（skill-p17-memorystore-inspection / skill-p17-vitest-hierarchical-test-org），另有 data-2pc-rollback-verify.json 与莫娜归档 md 共存。技能总数以 skills/ 目录实测为准，不设承诺绝对值。由 Mona（LoopAgent）从已完成工作中提取。
...
Core-2 过渡期催生了 Cortex 演进方法论——九阶段闭环：混沌 → 收敛 → 诊断 → 规划 → 执行 → 审查 → 结晶 → 升级 → 归纳。详见 `docs/archive/Cortex-演进方法论-九阶段闭环.md`（2026-08-11 实测：docs/core/ 下无此文件，实际存于 archive/）。
...
| 4.5 | eval 活性层 | `pnpm exec tsx packages/engine/tests/eval/eval-gate.ts`——report 模式 exit 恒 0（AM-2026-0811-001 补录：机制要有消费者）；--fail 模式断言失败即红（2026-08-24 增强） |
...
**代码事实**：`scripts/ci-gate.ts` 五主段顺序执行（tsc → eslint → critical-fixes → vitest → coverage[可选]）+ eval 活性层（门禁 4.5/5），vitest 按包分组串行调用，`process.exit(allOk ? 0 : 1)` 真实阻断。
...
| engine 测试 | 1036 passed / 91 文件（2026-08-11 实跑） |
...
- 前端：`packages/cli/src/tui/web/static/src/`（React 组件，8 个）——2026-08-11 实测：static/ 目录不存在，WebUI 前端组件尚未落地；在编后端 gateway.ts / state-aggregator.ts / api-router.ts / config-api-handler.ts 位于 packages/cli/src/tui/web/
...
- 自审视不替代 tsc -b / vitest / 五段 CI + eval 活性层（2026-08-24 订正——原『三段 CI』为 AM-2026-0801-001 修订错误口径）

#### 代码锚点
- 脚本：`scripts/self-exam-soft.ts`（7 阶段全流程，626 行）
- 输出：`test-output/self-examination-soft/`（45 文件）——2026-08-11 实测：输出目录当前不存在（test-output/ 下仅 governance-llm-verify），自审视产物尚未落盘，输出目录随 scripts/self-exam-soft.ts 运行生成，具体文件数以实际运行为准
