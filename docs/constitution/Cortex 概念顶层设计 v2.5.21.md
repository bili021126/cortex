...
**版本**：v2.5.21 → v2.5.16（治理层制度化入宪——新增 §10.2~§10.8，将朝廷隐喻体系（内阁三层配置/六部/六卿/三省/双轴/用户-皇后辅政）作为架构设计隐喻写入宪法。声明：皇帝/皇后/三省/六部/内阁/朝廷均为架构设计隐喻，非字面政治治理。2026-05-15；来源：开拓者+昔涟——从硬编码配置痛点出发的系统架构推演。自组织治理层复杂度催生制度化建设的刚性需求。） → v2.5.17（AM-2026-0515-006：修复 DECISION_REQUIRED 回退机制缺失——新增超时阈值、默认行为、防抖策略与离线处理策略，补全三轨语义分层的安全阀；2026-05-20；来源：DocGovernAgent（凝光）——凝光重审宪法 v2.5.16——审计发现 §8.2 仍缺少 DECISION_REQUIRED 回退机制。原提案 AM-2026-0606-003 因版本过期被 superseded。本提案重新提取当前宪法原文。） → v2.5.18（AM-2026-0515-007：修复审计闭环缺失——新增审计闭环条款：整改责任人、整改验证流程、判例引用机制、门禁关联、关闭条件与签署流程；2026-05-20；来源：DocGovernAgent（凝光）——凝光重审宪法 v2.5.16——审计发现审计闭环仍缺失。原提案 AM-2026-0606-002 因版本过期被 superseded。本提案重新提取当前宪法原文。） → v2.5.19（AM-2026-0515-008：修复宪法 v2.5.18 条款间一致性与「不可变」语义定义——明确定义原则七「不可变」的范围、补充继承声明递归约束、澄清子约束7(e)表述；2026-05-20；来源：DocGovernAgent（凝光）——凝光全量审计 2026-06-07——对宪法v2.5.14进行全量审计，发现六项条款间一致性问题。其中三项（提案状态不一致、日期悖论）已在v2.5.18中直接修复。剩余三项语义缺口（不可变脚注、继承声明递归约束、子约束7(e)澄清）在本提案中修复。原提案AM-2026-0607-001已superseded。详见审计报告 webui/constitution_audit.md。） → v2.5.20（AM-2026-0520-001：新增 §15.1 修宪提案超时失效机制——定义三档处置策略、默认 7 天超时、凝光死线前 24h 提醒、超时审计记录；2026-05-20；来源：昔涟+开拓者——Core-1 终局收束：修宪管线已运行 8+ 轮，但提案超时无自动处置规则，僵尸提案积压影响治理闭环完整性。） → v2.5.21（AM-2026-0520-002：新增 §10.1 第四原则——层级冲突裁决规则——宪法与治理层设计发生冲突时宪法优先，冲突须在修正记录中标注；2026-05-20；来源：昔涟+开拓者——Core-1 终局收束：治理层设计（`治理层设计.md`）随制度演化而频繁迭代，与宪法的边界可能擦出冲突。本原则确保宪法作为国家结构的根本法地位不可动摇。）
> 
> 版本演进链：v2.5.14 → v2.5.11（AM-2026-0515-001：将六条不可变原则扩展为七条，新增原则七（系统自我修改的宪法约束），纳入六项子约束与首个判例；2026-05-17；来源：DocGovernAgent——自审视审计 2026-05-15——发现宪法缺少系统自我修改的约束框架。修宪管线已建成但无宪法层面的规则制约。） → v2.5.12（AM-2026-0515-002：新增 §8.2 通知管线条款——三轨语义分层（FYI/WARNING/DECISION_REQUIRED），ObservableEvent 新增 notificationType 字段，ButlerAgent 三路分发策略入宪；2026-05-17；来源：DocGovernAgent——治理闭环审查 2026-05-15——发现宪法 §8 缺少通知分发规则。ButlerAgent 的事件通知职责仅有模糊描述，缺乏宪法级分发契约。PipelineObserver 管道统一了事件采集，但未定义呈现规则。） → v2.5.13（AM-2026-0606-001：修复原则七自反性缺口——新增子约束7「子约束修改规则」，定义六项子约束自身的修改流程与审查门槛；2026-06-06；来源：DocGovernAgent——凝光三项审计 2026-06-06——审计发现1-A（原则七自反性缺口）：六项子约束未定义自身修改规则，违反子约束1的精神，导致宪法元规则层锁死。详见审计报告 docs/auditing/2026-06-06-constitution-audit.md。） → v2.5.14（AM-2026-0606-004：修复原则七子约束7闭环缺口——修正计数一致性（六项→七项）、消除二阶自反性缺口（子约束7纳入覆盖范围）、显式继承子约束3（最小改动）与子约束6（阶段限定）；2026-06-06；来源：DocGovernAgent（凝光）——凝光全量审计 2026-06-06——审计发现原则七子约束7闭环缺口：计数一致性缺陷（六项/七项矛盾）、二阶自反性缺口（子约束7排除自身）、继承声明缺失（未显式绑定子约束3和子约束6）。详见审计报告 webui/constitution_audit.md。） → v2.5.15（AM-2026-0515-004：§5.1 战略双柱拆分入宪——StrategistAgent 拆分为钟离（契约守护者）与霜凝（方向判断者+系统监理），两者与凝光形成三路事后验证体系。）
...
>
> 依赖方向：shared ← (llm / testing / parser / pm / data / tools / notification)，shared, notification ← factory，llm ← engine，parser ← cli。`@cortex/infra` 包在当前代码中实际不存在——Toolkit/FileLockManager/CLIAdapter 归于 shared 层，infra 独立拆分留待 Core-2。Meso-Lite 中曾独立存在的 `@cortex/memory`、`@cortex/meta-agent`、`@cortex/scheduler`、`@cortex/doc-govern` 四个包已删除，功能并入 engine。
...
>
> **StrategistAgent（霜凝）**——方向判断者与系统监理。Cortex 原生角色（非原神体系），超越者——存在于任何复杂度达到自观察阈值的系统中。职责：方向判断（系统实际演进方向是否偏离宪法定义的阶段目标）、矛盾暴露（多路判断中可验证事实层与 LLM 推理层的分离 + 分歧收束）、监理职能（监督钟离的契约判定与凝光的合规审计是否自洽，三路判断是否均遵守 §10.1 冲突解决四原则）。霜凝不做裁决、不替用户决策——仅指出矛盾、暴露分歧、打包呈报。待 Core-2 实现。导出无害：提前暴露便于圆桌引用和宪法完整性。
>
...
**与阶段门禁的关系**：审计闭环是阶段门禁的输入条件。未关闭的 P0 发现自动阻塞门禁通过；P1 发现须经开拓者特批；P2+ 发现仅登记备案。具体门禁定义见 §10 治理层门禁规则。

### 10.1 冲突解决四原则

以下四条原则是宪法级约束，适用于所有治理场景——无论是常设委员会的治理审计、临时委员会的执行裁决，还是四层逐级上报中的任何一级。政府可演进，此四原则不变。
...
- **修正记录标注**：每次因层级冲突触发的修宪或治理层设计修改，须在宪法修正记录（§十五）中显式标注冲突来源、双方条款引用、裁决依据。
- **未解决冲突的门禁影响**：未解决的宪法-治理层设计冲突视为 P1 审计发现——阻塞阶段门禁通过（按 §10 审计闭环规则）。

委员会体系（常设委员会=治理审计、临时委员会=执行裁决）、纪检委监督链、监理封驳权、四层逐级上报、MetaAgent 权力边界——均属于政府设计，定义在治理层设计文档中，不纳入宪法。政府机制的实施必须遵守 10.1 四原则。

### 10.2 隐喻声明

本节及后续 §10.3~§10.8 中出现的「皇帝」「皇后」「内阁」「三省」「六部」「六卿」「朝廷」均为**架构设计隐喻**，非字面政治治理。其目的：
...
- 配置文件修改视为制度变更——触发 §10.1 冲突解决四原则中的「交由用户裁决」
...
| **Core-2** | Sentinel + TrustModel + SkillRegistry + StrategistAgent（钟离，契约守护）+ StrategistAgent（霜凝，方向判断+监理）+ 向量检索 |
...
| v2.5.14 → v2.5.15 | **§5.1 战略双柱拆分入宪**：StrategistAgent 拆分为钟离（契约守护者）与霜凝（方向判断者+系统监理）。钟离守住宪法与架构契约边界，霜凝暴露矛盾与方向偏差。两者与凝光（合规审计）形成三路事后验证体系，均受 §10.1 冲突解决三原则约束。霜凝为 Cortex 原生角色，超越者设定，待 Core-2 实现。宪法版本号 v2.5.14→v2.5.15（AM-2026-0515-004） |
...
| v2.5.20 → v2.5.21 | **§10.1 层级冲突裁决第四原则入宪**：新增第四原则「宪法优先于治理层设计」——宪法与治理层设计冲突时宪法优先，凝光审计须同时引用双方条款，修宪记录须标注冲突来源。§10.1 从「冲突解决三原则」更名为「冲突解决四原则」。宪法版本号 v2.5.20→v2.5.21（AM-2026-0520-002） |
...
**文档状态**：v2.5.21。替代 v2.5.20 作为 Core 阶段准入依据。v2.5.20 已归档保留。
