...
**版本**：v2.5.13 → v2.5.11（AM-2026-0515-001：将六条不可变原则扩展为七条，新增原则七（系统自我修改的宪法约束），纳入六项子约束与首个判例；2026-05-17；来源：DocGovernAgent——自审视审计 2026-05-15——发现宪法缺少系统自我修改的约束框架。修宪管线已建成但无宪法层面的规则制约。） → v2.5.12（AM-2026-0515-002：新增 §8.2 通知管线条款——三轨语义分层（FYI/WARNING/DECISION_REQUIRED），ObservableEvent 新增 notificationType 字段，ButlerAgent 三路分发策略入宪；2026-05-17；来源：DocGovernAgent——治理闭环审查 2026-05-15——发现宪法 §8 缺少通知分发规则。ButlerAgent 的事件通知职责仅有模糊描述，缺乏宪法级分发契约。PipelineObserver 管道统一了事件采集，但未定义呈现规则。） → v2.5.13（AM-2026-0606-001：修复原则七自反性缺口——新增子约束7「子约束修改规则」，定义六项子约束自身的修改流程与审查门槛；2026-05-17；来源：DocGovernAgent——凝光三项审计 2026-06-06——审计发现1-A（原则七自反性缺口）：六项子约束未定义自身修改规则，违反子约束1的精神，导致宪法元规则层锁死。详见审计报告 docs/auditing/2026-06-06-constitution-audit.md。）
...
7. **子约束修改规则**：本原则的六项子约束可通过修宪流程修改，但须满足更严格的审查条件——(a) 提案必须显式声明修改的是子约束而非其他宪法条款，并在 section 字段中标注「原则七·子约束」；(b) 修改子约束的提案须经凝光（DocGovernAgent）专项合规审计 + 昔涟专项评判双重把关，缺一不可；(c) 开拓者（用户）必须亲自裁决，不可委托代理裁决（如 auto-approve）；(d) 修改后须在宪法修正记录中显式更新子约束的版本演进链，注明修改了哪一项子约束及版本 diff；(e) 新增子约束不得降低现有子约束的保护力度——子约束的不可变性质继承自原则七的不可变性，新子约束只能增强不能削弱。
