...
## 8. 虚拟陪伴/桌宠 UX 广度调研（2026-08-06 补充）

### 8.1 桌宠形态对标（与 Cortex desktop 的桌宠定位直接相关）

| 产品/模式 | 核心特征 | 对 Cortex desktop 的启示 |
|---|---|---|
| **AI Desktop Pet**（透明 Live2D 浮窗） | 透明窗口 + Live2D 常驻桌面 | Cortex desktop 的最终形态参照（陪伴定位） |
| **DesktopFriends**（跨平台桌宠） | Live2D + LLM 对话 + agentic tool-calling | 对话与工具调用并存——与 Cortex 的聊天+任务能力对应 |
| **iF DESIGN AWARD 2026 Virtual Pets** | 情感连接/身份认同（设计概念） | 陪伴感的"身份锚定"——昔涟人格呈现的核心理念 |

### 8.2 桌宠交互的关键模式（抄这些）

1. **常驻透明浮窗**：不抢焦点、可拖动、点击唤起——桌面陪伴的核心形态（替代全屏应用）
2. **触摸式互动**（tap/pet/feed——真实情感反应）：轻交互（点击/抚摸）触发动画/语音——Cortex 的 Live2D focus.ts 已有基础（E1-E3 提案的落点）
3. **情感状态可视**（角色情绪 → 动画/配色变化）：昔涟的情绪呈现（视觉层）
4. **轻任务融合**：对话 + 工具调用在同一个角色身上（DesktopFriends 模式）——聊天流与任务视图层（§7）共存

### 8.3 对 Cortex desktop 的落地建议

- **形态**：先做"聊天窗 + 角色浮窗"双形态（浮窗常驻 + 点击展开聊天）——不是纯聊天应用
- **轻交互优先**：点击/抚摸/喂食类轻交互先做（低成本高陪伴感）——完整消息状态机（U1）是展开态的事
- **情感反馈**：昔涟的情绪状态映射到 Live2D 动画/配色（视觉层）——与 U1 的消息状态联动（sending 时角色等待态等）

### 8.4 不做（边界）

- 移动端桌宠（DesktopFriends 跨平台——但 Cortex 定位桌面）
- 社交功能（虚拟宠物社区——非陪伴定位）

---

*桌宠 UX 广度补充完成——常驻透明浮窗 + 轻交互 + 情感状态可视是桌宠三要素，与 U1 消息状态机联动。*
