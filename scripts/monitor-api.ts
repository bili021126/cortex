...
function loadEntries(hours: number): AuditEntry[] {
...
function analyze(entries: AuditEntry[]): {
  keyStats: KeyStats[];
  hourly: HourlyBucket[];
  ...
  // ── 按 Key 分组 ──
  const keyMap = new Map<string, AuditEntry[]>();
  ...
  // ── 按小时分布 ──
  const hourlyMap = new Map<string, { calls: number; errors: number }>();
  for (const e of entries) {
    const hour = e.ts.slice(0, 13) + ":00";
    const b = hourlyMap.get(hour) ?? { calls: 0, errors: 0 };
    b.calls++;
    if (e.status >= 400 || e.error) b.errors++;
...
function printReport(
  { keyStats, hourly, recentErrors, alerts }: ReturnType<typeof analyze>,
  hours: number,
): void {
  ...
  console.log(
    `${"Key".padEnd(20)} ${"调用".padStart(6)} ${"错误".padStart(5)} ${"错误率".padStart(7)} ${"平均耗时".padStart(9)} ${"模型"}`,
  );
  console.log("─".repeat(90));

  for (const ks of keyStats) {
    console.log(
      `${ks.label.padEnd(20)} ${String(ks.calls).padStart(6)} ${String(ks.errors).padStart(5)} ${ks.errorRate.padStart(7)} ${(ks.avgDurationMs + "ms").padStart(9)} ${ks.models.join(", ")}`,
    );
  }
  console.log("─".repeat(90));
  ...
  if (hourly.length > 0) {
    ...
    for (const h of hourly) {
      ...
      console.log(
        `   ${h.hour}  ${"█".repeat(bars)}${"░".repeat(barWidth - bars)} ${String(h.calls).padStart(4)}${errMark}`,
      );
    }
  }
  ...
  if (recentErrors.length > 0) {
    ...
    for (const e of recentErrors) {
      ...
      console.log(
        `   ${e.ts.slice(0, 19)}  ${label.padEnd(18)} ${String(e.status).padStart(4)}  ${(e.error ?? "无详情").slice(0, 60)}`,
      );
    }
    console.log("─".repeat(100));
  }
...
function inspectKeys(entries: AuditEntry[]): void {
  const keyMap = new Map<string, { count: number; models: string[]; lastSeen: string }>();
  for (const e of entries) {
    const info = keyMap.get(e.key) ?? { count: 0, models: [], lastSeen: "" };
    info.count++;
    if (!info.models.includes(e.model)) info.models.push(e.model);
    if (e.ts > info.lastSeen) info.lastSeen = e.ts;
    ...
    try {
      ...
      for (const line of newLines) {
        try {
          ...
          console.log(
            `${e.ts.slice(11, 19)} ${statusIcon} ${label.padEnd(18)} ${e.model.padEnd(22)} ${String(e.duration_ms).padStart(5)}ms  msgs:${e.messages_count}`,
          );
        } catch {
          // skip
        }
