...
import { evaluateAmendment, applyAmendment, } from "@cortex/governance";
// ════════════════════════════════════════════════════════
// §0 准备
// ════════════════════════════════════════════════════════
(function loadEnv() {
...
let p013Proposal = null;
let p010Proposal = null;
if (hasP013) {
    try {
...
if (p013Proposal) {
    try {
        ...
        $(`T3.2`, `Agent 提案 FIND-013: ${j013.verdict} (score=${j013.weightedScore.toFixed(2)})`, j013.verdict !== "BLOCKED");
        for (const c of j013.checks) {
            console.log(`     ${c.passed ? "✅" : "❌"} ${c.name}: ${c.detail.slice(0, 80)}`);
        }
        if (j013.blocking.length > 0) {
            for (const b of j013.blocking)
...
if (p010Proposal) {
    try {
        ...
        $(`T3.3`, `Agent 提案 FIND-010: ${j010.verdict} (score=${j010.weightedScore.toFixed(2)})`, j010.verdict !== "BLOCKED");
        for (const c of j010.checks) {
            console.log(`     ${c.passed ? "✅" : "❌"} ${c.name}: ${c.detail.slice(0, 80)}`);
        }
        if (j010.blocking.length > 0) {
            for (const b of j010.blocking)
                console.log(`     🚫 BLOCKED: ${b}`);
        }
    }
    catch (e) {
...
//# sourceMappingURL=amendment-stress.js.map