...
function makeNode(id, type, tags, payload, multi = false) {
    return {
        id, type, tags: tags, needsMultiPerspective: multi,
...
function printPhaseResult(phase, result, start) {
...
function isTaskLog(msg) {
    // Phase 结果行
    if (msg.includes("Phase") || msg.includes("✅") || msg.includes("❌"))
        return true;
    // 产出统计
    if (msg.includes("产出") || msg.includes("文件") || msg.includes("共识修复清单"))
        return true;
    // 特定 emoji
    for (const e of TASK_EMOJIS) {
        if (msg.includes(e))
            return true;
    }
    // 圆桌发言文件
    if (msg.includes("roundtable-") && msg.includes(".md"))
        return true;
    return false;
}
console.log = (...args) => {
...
// ════════════════════════════════════════════════════════
const p1Nodes = parsedTasks.map((t) => makeNode(`exam-${t.type}`, t.type, [t.type], [
...
roundtableNodes.push(makeNode("roundtable-strategist-zhongli", "strategist", ["strategist"], [
...
roundtableNodes.push(makeNode("roundtable-strategist-shuangning", "strategist", ["strategist"], [
...
//# sourceMappingURL=self-exam-soft.js.map