...
function cwidth(s: string): number {
  return [...s].reduce((w, c) => w + (c.charCodeAt(0) > 255 ? 2 : 1), 0);
}

function boxPrint(lines: string[]): void {
  console.log(boxTop());
...
function showMenu(title: string, items: { key: string; label: string; hint?: string }[]): void {
...
async function pressAnyKey(rl: readline.Interface): Promise<void> {
  await ask(rl, "\n  \u23ce 按回车继续...");
}

async function askWithDefault(rl: readline.Interface, prompt: string, def: string): Promise<string> {
  const a = await ask(rl, `${prompt} [${def}]: `);
  return a === "" ? def : a;
}

async function askNumber(rl: readline.Interface, prompt: string, def: number): Promise<number> {
  const a = await ask(rl, `${prompt} [${def}]: `);
  if (a === "") return def;
  const n = Number(a);
...
function saveJson(filePath: string, data: unknown): boolean {
  try {
...
async function delegateWithArgs(rl: readline.Interface, cmd: string, hint: string): Promise<void> {
  clear();
...
async function screenVersion(rl: readline.Interface): Promise<void> {
  clear();
  const pkg = loadJson<{ version: string }>(path.join(ROOT, "package.json"));
...
async function screenInspect(rl: readline.Interface): Promise<void> {
  clear();
...
async function screenDoc(rl: readline.Interface): Promise<void> {
  const docsCfg = loadJson<{ constitutionPath: string; docRegistry: { path: string; type: string; version: string; canonical: boolean }[] }>(
    path.join(ROOT, "cortex-docs.json"),
  );

  clear();
  const lines: string[] = ["", "  \u25b6 文档工具", ""];
...
// ═══════════════════════════════════════════════════════════
// 屏幕: 配置管理（复用 setup-config 逻辑）
// ═══════════════════════════════════════════════════════════

// 类型定义
interface AgentDef {
  id: string; type: string; role: string; systemPrompt: string;
  produces: string[]; model: string; key: string; maxInstances: number;
  tags: string[]; toolPermissions: string[]; memoryQueryStrategy?: string;
  planningPrompt?: string; replanPrompt?: string;
}

interface AgentsConfig {
  agents: Record<string, AgentDef>;
  eventRouting: {
    routeTable: Record<string, { channel: string; ackRequired: boolean }>;
    committeeRules: { id: string; triggerEvent: string; members: string[]; urgent: boolean }[];
  };
  roundtableTemplates: { name: string; description: string; personas: number; rounds: number; agents: string[] }[];
  searchProviders: {
    backends: { id: string; command: string; args: string[]; enabled: boolean }[];
    aggregation: { deduplicateBy: string; resultTimeout: number; minBackends: number };
  };
}

interface CognitionConfig {
  activationMatrix: { agentType: string; active: boolean; orientation: string }[];
  attention: { hcaWeight: number; csaWeight: number; maxMemoryItems: number };
}

interface DocsConfig {
  constitutionPath: string;
  docRegistry: { path: string; type: string; version: string; canonical: boolean }[];
}
...
let gAgents: AgentsConfig | null = null;
let gCognition: CognitionConfig | null = null;
let gDocs: DocsConfig | null = null;

async function askArray(rl: readline.Interface, prompt: string, current: string[]): Promise<string[]> {
...
async function editAgent(rl: readline.Interface, agent: AgentDef): Promise<AgentDef> {
  clear();
  const result = { ...agent };
  ...
  while (true) {
    ...
    switch (c) {
      case "1": result.model = await askWithDefault(rl, "  model", result.model); break;
      case "2": result.key = await askWithDefault(rl, "  key", result.key); break;
      case "3": result.maxInstances = await askNumber(rl, "  maxInstances", result.maxInstances); break;
      case "4": result.tags = await askArray(rl, "tags", result.tags); break;
      case "5": result.toolPermissions = await askArray(rl, "toolPermissions", result.toolPermissions); break;
      case "6":
        result.memoryQueryStrategy = await askWithDefault(rl, "  memoryQueryStrategy (\u7559\u7a7a\u6e05\u9664)", result.memoryQueryStrategy ?? "");
        if (result.memoryQueryStrategy === "") delete result.memoryQueryStrategy;
        break;
      case "7": {
        clear();
      ...
      case "8": {
        ...
        if (cf.toLowerCase() === "y" || cf.toLowerCase() === "yes") return result;
        break;
      }
      case "0": return agent;
  ...
  while (true) {
    ...
    switch (c) {
      ...
      case "2": {
        ...
        boxPrint(lines); await pressAnyKey(rl); break;
      }
      case "3": {
        clear();
        const lines: string[] = ["", "  \u25b6 \u59d4\u5458\u4f1a\u89c4\u5219", ""];
      ...
      case "S": case "s": {
        ...
        if (saveJson(AGENTS_PATH, gAgents)) console.log("\n  OK cortex-agents.json \u5df2\u4fdd\u5b58");
        else console.log("\n  ERR \u4fdd\u5b58\u5931\u8d25");
        await pressAnyKey(rl);
        return;
      }
...
async function screenCognitionConfig(rl: readline.Interface): Promise<void> {
  ...
  while (true) {
    ...
    lines.push("", "  -- \u6ce8\u610f\u529b\u914d\u7f6e --",
      `    hcaWeight:       ${attention.hcaWeight}`,
      `    csaWeight:       ${attention.csaWeight}`,
      `    maxMemoryItems:  ${attention.maxMemoryItems}`,
      "", "    [T]  \u5207\u6362\u6fc0\u6d3b\u72b6\u6001", "    [A]  \u7f16\u8f91\u6ce8\u610f\u529b\u53c2\u6570",
      "", "    [S]  \u4fdd\u5b58\u5e76\u8fd4\u56de", "");
    ...
    if (c === "T" || c === "t") {
      ...
      if (saveJson(COGNITION_PATH, gCognition)) console.log("\n  OK cortex-cognition.json \u5df2\u4fdd\u5b58");
      else console.log("\n  ERR \u4fdd\u5b58\u5931\u8d25");
      await pressAnyKey(rl);
      return;
    } else if (c === "0") {
...
async function screenDocsConfig(rl: readline.Interface): Promise<void> {
  ...
  while (true) {
    ...
    if (c === "E" || c === "e") {
      ...
      if (saveJson(DOCS_PATH, gDocs)) console.log("\n  OK cortex-docs.json \u5df2\u4fdd\u5b58");
      else console.log("\n  ERR \u4fdd\u5b58\u5931\u8d25");
      await pressAnyKey(rl);
      return;
    } else if (c === "0") {
...
async function screenSetup(rl: readline.Interface): Promise<void> {
  while (true) {
    clear();
    const items: { key: string; label: string; hint?: string }[] = [];
    if (gAgents) items.push({ key: "1", label: "cortex-agents.json", hint: `(${Object.keys(gAgents.agents).length} Agent)` });
    if (gCognition) {
    ...
    if (gDocs) items.push({ key: "3", label: "cortex-docs.json", hint: `(${gDocs.docRegistry.length} \u6761\u6ce8\u518c)` });

    showMenu("\u914d\u7f6e\u7ba1\u7406\u4e2d\u5fc3", items);

    const c = await ask(rl, "  > ");
    switch (c) {
      case "1": if (gAgents) await screenAgentsConfig(rl); break;
      case "2": if (gCognition) await screenCognitionConfig(rl); break;
      case "3": if (gDocs) await screenDocsConfig(rl); break;
      case "0": return;
      default: console.log("  \u65e0\u6548\u9009\u62e9");
    }
  }
...
async function mainMenu(rl: readline.Interface): Promise<void> {
  ...
  while (true) {
    ...
    if (cmd.builtin) {
      switch (cmd.key) {
        case "5": await screenSetup(rl); break;
        case "6": await screenDoc(rl); break;
        case "7": await screenInspect(rl); break;
        case "V": await screenVersion(rl); break;
      }
    } else {
      // 委托到 cortex CLI
...
async function main(): Promise<void> {
  try { execSync("chcp 65001", { stdio: "pipe" }); } catch { /* non-Windows */ }
