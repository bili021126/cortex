...
try {
    fs.unlinkSync(path.join(OUTPUT, "t2.txt"));
}
catch { }
// 重建非 bypass gate
...
try {
    rGateCheck = await Promise.race([
        ...
        new Promise((_, rej) => setTimeout(() => rej(new Error("T3_TIMEOUT")), 8000)),
    ]);
}
catch (e) {
...
if (gateHanging) {
    ...
    try {
        fs.unlinkSync(path.join(OUTPUT, "t3-gate-check.txt"));
    }
    catch { }
}
...
if (node?.results?.[0]?.error) {
    $("T4.3", `错误: ${node.results[0].error.slice(0, 120)}`, (node.results[0].error ?? "").includes("ConfirmGate"));
}
...
//# sourceMappingURL=confirmgate-stress.js.map