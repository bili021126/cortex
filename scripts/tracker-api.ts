...
interface DayAggregate {
  ...
  models: string[];
  hourlyDistribution: number[];  // index 0~23
  reqIds: string[];              // 最近 10 个 request-id
}
...
interface TrackerDB {
  version: 1;
  updatedAt: string;
  lastAuditCursor: number;  // 已处理到的审计日志行号
...
function loadDB(): TrackerDB {
  if (fs.existsSync(DB_PATH)) {
    try {
      return JSON.parse(fs.readFileSync(DB_PATH, "utf-8")) as TrackerDB;
    } catch {
...
function saveDB(db: TrackerDB): void {
...
function ensureKey(db: TrackerDB, fingerprint: string): KeyRecord {
...
function ensureDay(kr: KeyRecord, date: string): DayAggregate {
  if (!kr.days[date]) {
    kr.days[date] = {
      date,
      calls: 0,
...
function ingestAuditLog(db: TrackerDB): { processed: number; newEntries: number } {
  if (!fs.existsSync(AUDIT_LOG)) return { processed: 0, newEntries: 0 };

  const raw = fs.readFileSync(AUDIT_LOG, "utf-8").trim();
  if (!raw) return { processed: 0, newEntries: 0 };
  ...
  for (let i = db.lastAuditCursor; i < lines.length; i++) {
    try {
      ...
      if (e.model && !day.models.includes(e.model)) day.models.push(e.model);
      if (e.req_id && day.reqIds.length < 10) day.reqIds.push(e.req_id);
      if (e.status >= 400 || e.error) day.errors++;
      ...
      if (e.status >= 400 || e.error) kr.cumulative.errors++;
      if (!kr.cumulative.firstSeen || e.ts < kr.cumulative.firstSeen) kr.cumulative.firstSeen = e.ts;
      if (!kr.cumulative.lastSeen || e.ts > kr.cumulative.lastSeen) kr.cumulative.lastSeen = e.ts;
...
function computeBaseline(kr: KeyRecord): void {
...
function detectAnomalies(db: TrackerDB, today: string): Alert[] {
  const alerts: Alert[] = [];

  for (const kr of Object.values(db.keys)) {
    const label = kr.label;
    const todayDay = kr.days[today];
    if (!todayDay || !kr.baseline) continue;
...
function estimateCost(kr: KeyRecord): { totalCost: number; byModel: Record<string, { calls: number; cost: number }> } {
  let totalCost = 0;
...
function printTodayReport(db: TrackerDB, alerts: Alert[], today: string): void {
  console.log(`\n╔══════════════════════════════════════════════════════╗`);
  ...
  for (const kr of Object.values(db.keys)) {
    ...
    console.log(
      `${kr.label.padEnd(18)} ${String(day.calls).padStart(5)} ${String(day.errors).padStart(4)} ${tokenStr.padStart(12)} ${avgMs.padStart(8)} ${day.models.join(", ")}`,
    );
  }
  console.log("─".repeat(105));
  console.log(
    `${"合计".padEnd(18)} ${String(grandTotalCalls).padStart(5)} ${"-".padStart(4)} ${(grandTotalTokens > 1000 ? `${(grandTotalTokens / 1000).toFixed(1)}k` : String(grandTotalTokens)).padStart(12)}`,
  );
  console.log();
...
function printSummary(db: TrackerDB): void {
  console.log(`\n╔══════════════════════════════════════════════════════════════╗`);
  ...
  console.log(
    `${"Key".padEnd(18)} ${"总调用".padStart(7)} ${"总错误".padStart(6)} ${"总Token".padStart(12)} ${"活跃天数".padStart(6)} ${"估费(元)".padStart(10)} ${"首次→最后"}`,
  );
  console.log("─".repeat(115));
  ...
  for (const kr of Object.values(db.keys)) {
    ...
    console.log(
      `${kr.label.padEnd(18)} ${String(kr.cumulative.calls).padStart(7)} ${String(kr.cumulative.errors).padStart(6)} ${tokenStr.padStart(12)} ${String(activeDays).padStart(6)} ${cost.totalCost.toFixed(4).padStart(10)} ${first}→${last}`,
    );
  }
  console.log("─".repeat(115));
  console.log();
...
function printTrend(db: TrackerDB, days: number): void {
  const allDates = new Set<string>();
  for (const kr of Object.values(db.keys)) {
    for (const d of Object.keys(kr.days)) allDates.add(d);
  }
...
function exportCsv(db: TrackerDB): void {
  const csvPath = path.resolve(process.cwd(), ".cortex", "logs", "api-tracker.csv");
  const rows: string[] = ["date,key,calls,errors,prompt_tokens,completion_tokens,total_tokens,models"];
  ...
  if (args.includes("--daemon")) {
    ...
    const tick = () => {
      ...
      if (processed > 0) {
        ...
        for (const kr of Object.values(db.keys)) computeBaseline(kr);
        saveDB(db);
