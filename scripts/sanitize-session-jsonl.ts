...
const CONVERSATION_HISTORY_DIR = path.resolve(
  process.env.USERPROFILE ?? process.env.HOME ?? ".",
  ".qoder/cache/projects/cortex-d945f0ba/conversation-history"
);
...
function isImageUrlBlock(block: ContentBlock): boolean {
  return block.type === "image_url";
}

function sanitizeLine(line: string): { result: string | null; reason: "ok" | "empty" | "unparseable" | "no_content_array" | "image_removed" | "all_image_stripped" } {
  ...
  if (!Array.isArray(content)) return { result: trimmed, reason: "no_content_array" };

  const hasImage = content.some(isImageUrlBlock);
  if (!hasImage) return { result: trimmed, reason: "ok" };
...
function processFile(filePath: string, dryRun: boolean): { imageRemoved: number; emptyRemoved: number; lines: number } {
  ...
  for (const line of lines) {
    ...
    if (result === null) {
      if (reason === "empty") emptyRemoved++;
      else if (reason === "all_image_stripped") imageRemoved++;
      continue;
    }
    if (reason === "image_removed") imageRemoved++;
...
function main() {
  ...
  for (const entry of entries) {
    ...
    if (imageRemoved > 0) {
      ...
      console.log(
        `[${dryRun ? "DRY-RUN" : "FIXED"}] ${entry.name}.jsonl — 移除 ${imageRemoved} 行 image_url${emptyRemoved > 0 ? `，清理 ${emptyRemoved} 行空行` : ""}，剩余 ${lines} 行`
      );
    } else {
