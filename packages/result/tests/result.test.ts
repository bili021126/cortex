...
describe("isOk / isErr", () => {
  ...
  it("isErr 在类型守卫中正确窄化类型", () => {
    const r: Result<number, string> = err("error");
    if (isErr(r)) {
      // 此处 error 应为 string
...
describe("map", () => {
  ...
  it("map 在 Err 时保持不变", () => {
    const r = map(err("fail") as Result<number, string>, (n) => n.toString());
    vitestExpect(isErr(r)).toBe(true);
  });
});
...
describe("mapErr", () => {
  ...
  it("mapErr 在 Ok 时保持不变", () => {
    const r = mapErr(ok(42), (e: string) => `error: ${e}`);
    vitestExpect(unwrap(r)).toBe(42);
  });
});
...
describe("andThen", () => {
  ...
  it("andThen 在 Err 时跳过链式调用", () => {
    const r = andThen(err("fail") as Result<number, string>, (n) => ok(n * 2));
    vitestExpect(isErr(r)).toBe(true);
  });
});
...
describe("orElse", () => {
  ...
  it("orElse 在 Ok 时保持不变", () => {
    const r = orElse(ok(42), (_e: string) => ok(0));
    vitestExpect(unwrap(r)).toBe(42);
  });
});
...
describe("tryCatch", () => {
  ...
  it("tryCatch 在函数抛出异常时返回 Err", () => {
    const r = tryCatch(
      ...
      (e) => `caught: ${e instanceof Error ? e.message : String(e)}`,
    );
    if (isErr(r)) {
...
describe("fromNullable", () => {
  ...
  it("fromNullable 在值为 null 时返回 Err", () => {
    const r = fromNullable(null as string | null, () => "value is null");
    if (isErr(r)) {
  ...
  it("fromNullable 在值为 undefined 时返回 Err", () => {
    const r = fromNullable(undefined as string | undefined, () => "value is undefined");
    if (isErr(r)) {
