...
import { MemoryStore, CognitiveEngine, WeightAger, DedupService, BM25Index, HybridRetriever, DEFAULT_COGNITIVE_CONFIG, bayesianRelevanceScore, fourierTimeDecay, timeDecayScore, } from "@cortex/memory-store";
...
function makeInput(overrides) {
    return {
        source: { agentType: "Code", taskId: "task-1" },
        kind: "TaskLog",
...
function makeEntry(id, accessCount, lastAccessedAt) {
    return {
        id,
        source: { agentType: "Code", taskId: "test" },
        kind: "TaskLog",
        summary: `Entry ${id}`,
...
describe("WeightAger 权重衰减", () => {
    ...
    it("obliterateFrozen 识别湮灭条目", () => {
        ...
        const entries = [{ ...makeEntry("old", 5, oldTs), semantic_state: "Archived" }];
...
describe("DedupService 去重", () => {
    ...
    it("exactMatch 重复检测", () => {
        ...
        const entries = [makeEntry("e1", 0, 0)];
...
describe("CognitiveEngine 认知评分", () => {
    ...
    it("timeDecayScore 综合衰减", () => {
        const score = timeDecayScore(makeEntry("e1", 10, Date.now() - 1000), Date.now(), DEFAULT_COGNITIVE_CONFIG);
        expect(typeof score).toBe("number");
...
describe("HybridRetriever 混合检索", () => {
    ...
    it("空候选列表返回空", async () => {
        const retriever = new HybridRetriever();
        const mockEmbedder = { embedBatch: async (texts) => texts.map(() => new Array(128).fill(0)) };
        const result = await retriever.score([], new Map(), new Array(128).fill(0), mockEmbedder);
