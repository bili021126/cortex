// ============================================================
// engine-telemetry.ts —— 引擎遥测封装
//
// 对 @cortex/telemetry 的薄封装，提供：
//   1. 默认 ConsoleCollector（开发期直接可见）
//   2. record() 统一入口——所有引擎关键路径事件走此通道
//
// @since Cortex Core-2 — 上下文生命周期管理协议
// ============================================================
import { ConsoleCollector } from "@cortex/telemetry";
let _collector = null;
/** 获取或创建默认遥测采集器 */
export function getTelemetry() {
    if (!_collector) {
        _collector = new ConsoleCollector();
    }
    return _collector;
}
/** 替换遥测采集器（测试/生产环境注入 FileCollector 等） */
export function setTelemetry(collector) {
    void _collector?.shutdown();
    _collector = collector;
}
/** 记录引擎遥测事件 */
export async function recordTelemetry(name, value, tags, metadata) {
    const data = {
        id: `${name}_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        name,
        value,
        tags: tags ?? [],
        timestamp: Date.now(),
        metadata,
    };
    await getTelemetry().collect(data);
}
/** 关闭遥测（刷新缓冲区 + 释放资源） */
export function shutdownTelemetry() {
    void _collector?.shutdown();
    _collector = null;
}
//# sourceMappingURL=engine-telemetry.js.map