...
import { extractJsonBlock, PRESET_CONTEXT_POLICIES, PipelinePriority } from "@cortex/shared";
import { buildPlanningSystem, buildPlanningSystemBlank, REPLAN_SYSTEM, PIPELINE_CTX_MAX_OUTPUT_LEN, PIPELINE_CTX_MAX_ERROR_LEN, PIPELINE_CTX_RECENT_LIMIT, PIPELINE_CTX_HARD_CAP, } from "@cortex/config";
import { resolveByScope } from "./skill-scope.js";
/**
 * MetaAgent —— 战术引擎。
...
export class MetaAgent {
    llm;
    _nodeCounter = 0; // 防 Date.now() 高频碰撞
    _safeReporter;
    _skillRegistry;
    _observer;
    ...
    _workspaceRoot;
    _promptManager;
    _loopStrategyRegistry;
    constructor(llm, skillRegistry, planningSystemPrompt, replanSystemPrompt, observer, workspaceRoot) {
        this.llm = llm;
    ...
    setLoopStrategyRegistry(registry) {
    ...
    setObserver(observer) {
        ...
        this._onNodeComplete = (event) => {
            if (event.type === "node.complete") {
                const { nodeId, agentType, output } = event.payload;
                if (output) {
    ...
    _unsubscribe() {
        ...
        if (this._onNodeComplete)
            this._observer.off(PipelinePriority.HIGH, this._onNodeComplete);
        if (this._onNodeFailed)
            this._observer.off(PipelinePriority.CRITICAL, this._onNodeFailed);
        this._onNodeComplete = undefined;
        this._onNodeFailed = undefined;
    }
    /** 将事件入队到管线上下文（带硬上限防内存泄漏） */
    _enqueuePipelineCtx(entry) {
    ...
    _clearPipelineContext() {
        this._pipelineContext = [];
    }
    /** 获取当前积累的管线上下文 */
    _getPipelineContext() {
        if (this._pipelineContext.length === 0)
            return "";
     ...
     */
    async requestReplan(failedNode, reason, replanCount, originalIntent, maxReplan = 3) {
        const prompt = [
     ...
     */
    async requestBoundaryReplan(violatingNode, boundaryReason, replanCount, originalIntent, maxReplan = 3) {
        const prompt = [
    ...
    /** 解析意图确认结果 */
    _parseClarification(raw, originalIntent) {
        try {
            const cleaned = raw
                .replace(/```json\s*/gi, "")
                .replace(/```\s*/g, "")
    ...
    _validActionTypes = new Set(["analysis", "modification", "audit", "refactor", "generation", "inquiry"]);
    _normalizeActionType(raw) {
     ...
     * 否则回退到手拼 parts.join("\n") 以保持向后兼容。
     */
    async _planningPrompt(intent, context) {
        ...
        if (this._promptManager) {
            ...
            if (this._skillRegistry && context?.existingTags) {
                ...
                const matched = skills.filter((s) => context.existingTags.some((t) => s.triggerTags.includes(t)));
                if (matched.length > 0) {
                    const skillLines = matched.map((s) => `  · ${s.name} (id:${s.id}) [${s.kind}] tags:[${s.triggerTags.join(",")}] — ${s.trigger}`);
                    blocks.skillContext =
        ...
        if (this._skillRegistry && context?.existingTags) {
            ...
            if (matched.length > 0) {
                const skillLines = matched.map((s) => `  · ${s.name} (id:${s.id}) [${s.kind}] tags:[${s.triggerTags.join(",")}] — ${s.trigger}`);
                parts.push(`Available skill templates (pre-existing patterns):\n${skillLines.join("\n")}\n\n` +
                    "You MAY reference these skills in your plan by mentioning their id in the payload. " +
                    "These are vetted, repeatable workflows — prefer them over inventing new task sequences.");
            }
        }
        // ── 策略顾问上下文 ──
        if (this._loopStrategyRegistry) {
            parts.push("Available loop strategies (set preferredStrategy on task nodes):\n" +
                this._loopStrategyRegistry.getAdvisorContext() +
                "\n\nChoose the most appropriate strategy for each task based on its nature.");
        }
    ...
    _toTaskNode(item, parentId, index) {
        const now = Date.now();
        const nodeId = `task-${now}-${this._nodeCounter++}-${index}`;
        // 子任务递归——拿到的是扁平数组，每个子节点的 parentId 已指回当前节点
        const children = (item.children ?? []).flatMap((child, ci) => this._toTaskNode(child, nodeId, ci));
    ...
    _parseReplanResult(raw, parentId) {
        ...
        try {
            ...
            const impactScope = (!Array.isArray(parsed) && parsed.impactScope === "subtree") ? "subtree" : "local";
 ...
 *   3. 回退 → "single-step"
 */
function _resolveContextPolicy(type, tags) {
    // 规则 1: type 精确命中
    if (type && PRESET_CONTEXT_POLICIES[type])
        return type;
    // 规则 2: tags 匹配
    if (tags) {
...
function _validTier(t) {
    return t && _VALID_TIERS.has(t) ? t : undefined;
}
