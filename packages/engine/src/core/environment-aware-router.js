// ============================================================
// @cortex/engine/core/environment-aware-router —— 环境感知路由器
//
// @layer 规划-执行层
...
export class EnvironmentAwareRouter {
     ...
     * @returns 最终选中的模型
     */
    async resolve(preferredModel, node) {
    ...
    reportFailure(model) {
        ...
        if (health.consecutiveFailures >= this.config.failureThreshold) {
            ...
            console.warn(`[EnvironmentAwareRouter] 模型 ${model} 连续失败 ${health.consecutiveFailures} 次，标记为不可用`);
        }
    }
    /**
     * 获取所有模型健康状态——用于可观测性。
     ...
     */
    _selectFallback(preferredModel) {
        ...
        switch (this.config.fallbackStrategy) {
            case "next-in-priority": {
                ...
                for (let i = preferredIdx + 1; i < priority.length; i++) {
                    if (this.isAvailable(priority[i]))
                        return priority[i];
                }
            ...
            case "fastest": {
                ...
                for (const model of priority) {
                    if (model !== preferredModel && this.isAvailable(model))
                        return model;
                }
                return null;
            }
     ...
     * 发射遥测数据——记录路由决策。
     */
    _emitTelemetry(nodeId, preferred, selected, reason, durationMs) {
...
//# sourceMappingURL=environment-aware-router.js.map