 ...
 *
 * ## 设计原则
 * - 组件式：独立 IDispatchStep，可插拔到任意调度管线
 * - 可插拔：BOUNDARY_RULES 注册表支持任意扩展
 * - 可读可维护：规则即文档，违规即事件
 * - 复用四路事件管线：走 PipelineObserver，不耦合 NodeResult
 *
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        // 跳过 node_modules / .git / dist 以及所有点目录
        if (
 ...
 *
 * 检测策略：
 * 1. 以节点创建时间（node.createdAt）为分界线
 * 2. 扫描 workspace 中 mtimeMs > node.createdAt 的文件
 * 3. 对照 BOUNDARY_RULES 检查是否命中 forbidden 域
 * 4. 违规 → 发射 AgentBoundaryViolation 事件 + 标记 ctx.boundaryViolation
 */
export class BoundaryGuardStep implements IDispatchStep {
  ...
  async run(ctx: DispatchCtx): Promise<DispatchCtx> {
    ...
    // 标记到 ctx 供 Scheduler 消费（触发 replan）
  ...
  private async _scanViolations(
    ...
    try {
      const allFiles = await walkDir(this._workspaceRoot);

      for (const [absPath, mtime] of allFiles) {
        // 仅检查节点创建后修改的文件，容忍时钟偏差
