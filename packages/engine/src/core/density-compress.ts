 ...
 *
 * @contract 模块边界契约
 * - 调用方: RlmExecuteStep, annotateAndCompress()
 * - 保证: 纯函数，永远返回合法 DensityLevel
 */
 ...
 *
 * @contract 模块边界契约
 * - 调用方: RlmExecuteStep._executeOneSubTask()
 * - 保证: 纯函数，无副作用
 */
 ...
 * @contract 模块边界契约
 * - 调用方: RlmExecuteStep._executeOneSubTask()
 * - 保证: 纯函数，输入合法 DensityLevel 永远返回合法策略
 */
 ...
 *
 * @contract 模块边界契约
 * - 调用方: RlmExecuteStep._executeSubTasks()
 * - 保证: 纯函数，无副作用
 */
...
function compressMedium(text: string): string {
  ...
  for (const line of lines) {
    ...
    if (
      /^[-*#>]/.test(trimmed) ||          // 列表项、标题
      /^[A-Za-z]+:/.test(trimmed) ||      // 键值对
      /^(✅|❌|⚠️|🔴|🟢|🟡)/.test(trimmed) || // 状态标记
      /\b(error|warning|fail|pass|ok)\b/i.test(trimmed) // 关键信息
    ) {
      kept.push(trimmed);
    }
    // 否则只保留非空行中的"看起来像内容"的行
