 ...
 */
export function loadEngineDefaults(overrides) {
...
function _readEnvOverrides() {
    const overrides = {};
    for (const [envKey, configKey] of Object.entries(ENV_MAP)) {
        const val = process.env[envKey];
        if (val !== undefined) {
            const num = Number(val);
            overrides[configKey] = Number.isNaN(num) ? val : num;
        }
    }
    return overrides;
}
//# sourceMappingURL=engine-defaults.js.map