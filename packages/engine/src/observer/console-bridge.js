 ...
 */
const MESSAGE_PREFIX_WHITELIST = [
 ...
 */
function _isWhitelisted(args) {
    // 消息前缀检查（O(1) 首参字符串前缀匹配）
    if (args.length > 0 && typeof args[0] === "string") {
        const firstArg = args[0];
        if (MESSAGE_PREFIX_WHITELIST.some((p) => p.test(firstArg)))
            return true;
    }
    // 调用栈检查
...
/** 构造 ErrorReported 事件 */
function _buildErrorEvent(severity, message) {
    return {
 ...
 */
export function installConsoleBridge(observer) {
    if (_installed)
        return;
