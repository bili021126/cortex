...
export class DocRegistry {
    ...
    pathTemplates;
    _loaded = false;
    constructor(fs, workspaceRoot, pathTemplates) {
        this.fs = fs;
    ...
    async register(input) {
        ...
        const fm = [
            ...
            `status: draft`,
            `id: "${id}"`,
            `authors: [${input.authors.map((a) => `"${a}"`).join(", ")}]`,
            `registeredAt: "${new Date(now).toISOString()}"`,
        ];
        if (input.committeeType)
            fm.push(`committeeType: "${input.committeeType}"`);
        if (input.triggerSource)
            fm.push(`triggerSource: "${input.triggerSource}"`);
        if (input.constitutionVersion)
            fm.push(`constitutionVersion: "${input.constitutionVersion}"`);
     ...
     * 正史门槛：reviewers 必须非空。
     */
    async promote(id, reviewers) {
        this._ensureLoaded();
        const entry = this.index.entries[id];
        if (!entry)
            throw new Error(`DocEntry not found: ${id}`);
    ...
    list(filter) {
        ...
        if (filter?.committeeType)
            entries = entries.filter((e) => e.committeeType === filter.committeeType);
        return entries;
    }
    /** 按 ID 获取单条 */
    get(id) {
