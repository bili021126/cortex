 ...
 * @param model LLM 模型名
 */
export async function runReActLoop(ctx, node, model) {
    const { agentType, llm, toolkit, systemPrompt, maxLoops } = ctx;
    const toolDefs = toolkit.listDefinitions(agentType).map((d) => ({
        name: d.name,
    ...
    const diagnostic = (msg) => {
        if (REACT_DEBUG)
            process.stderr.write(`  🔁 [ReAct-${agentType}#${loops}] ${msg}\n`);
    };
    ...
    while (loops < maxLoops) {
        ...
        try {
            ...
            const forceWrite = (loops === 0 && [AgentType.Code, AgentType.Fix, AgentType.Ops].includes(agentType) && toolDefs.some(t => t.name === "write_file"))
            ...
            if (l0Calls.length > 0) {
                ...
                const l0Results = await Promise.allSettled(l0Calls.map(async (tc) => {
                    const toolStart = Date.now();
                    const result = await toolkit.execute({ toolName: tc.name, params: tc.arguments }, agentType);
                ...
                for (const settled of l0Results) {
                    if (settled.status === "fulfilled") {
                        ...
                        diagnostic(`⚠️ L0 工具被拒绝: ${settled.reason?.tc?.name ?? "unknown"}`);
                        const tc = settled.reason?.tc;
                        messages.push({
                            role: "tool",
