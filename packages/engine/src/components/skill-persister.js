 ...
 */
export async function crystallizeSkillToKnowledge(skill, memory, opts) {
    try {
        ...
        let version = 1;
        let existingEvidenceIds = [];
        if (existing.length > 0) {
            // 2. 已有记录 → 归档旧版本 → 继承版本号 + 证据链
            const oldEntry = existing[0];
            version = (oldEntry.content_blob?.version ?? 1) + 1;
            existingEvidenceIds = oldEntry.content_blob?.evidenceIds ?? [];
        ...
        if (opts?.evidenceIds) {
            for (const eid of opts.evidenceIds) {
                try {
                    memory.link(memId, eid, LinkType.DerivedFrom);
                }
 ...
 * @returns 搜索结果列表
 */
export async function searchExternalEvidence(skill, searcher) {
 ...
 * @returns 验证结果
 */
export async function verifySkillKnowledge(skill, memory, verifier, opts) {
    // 检索与该技能关联的情景记忆
 ...
 * @returns 成功持久化的技能数量。
 */
export function persistSkillsToMemory(skills, memory) {
    let count = 0;
 ...
 */
export async function loadSkillsFromMemory(memory) {
    ...
    try {
        ...
        for (const entry of entries) {
            if (entry.kind !== "Skill")
                continue;
            if (entry.semantic_state !== "Active")
                continue;
            const content = entry.content_blob;
            if (!content || typeof content !== "object")
                continue;
            const skill = content;
            if (!skill?.id || !skill?.name || !skill?.triggerTags || !Array.isArray(skill.steps))
                continue;
 ...
 */
export function scanOutputFilesForSkills(workspaceDir) {
    const allSkills = [];
 ...
 */
function patternDefinitionToSkillTemplate(p, kind) {
 ...
 *   3. 若无产出，回退到旧版全文兜底逻辑
 */
function extractSkillsFromMarkdown(content, kind, filePath) {
 ...
 */
function _extractPNSections(content, kind) {
    const patterns = [];
    const timestamp = Date.now();
    const sectionRegex = /(?:^|\n)#{2}\s*P(\d+)\s*[—\-–]\s*(.+?)(?:\n|$)/g;
    let match;
    while ((match = sectionRegex.exec(content)) !== null) {
        ...
        if (recipeMatch) {
            ...
            if (stepLines) {
                const stepParts = stepLines.map((s) => s.replace(/^[-*]\s*/, "").trim());
 ...
 */
function _extractPatternSections(content, kind) {
    const patterns = [];
    const timestamp = Date.now();
    const sectionRegex = /(?:^|\n)#{2}\s*模式\s*(\d+)\s*[：:]\s*(.+?)(?:\n|$)/g;
    let match;
    while ((match = sectionRegex.exec(content)) !== null) {
        ...
        // 提取 steps（多个可能的模式）
        const stepLines2 = sectionContent.match(/步骤[：:]s*([\s\S]*?)(?:\n(?:#{1,3}|\n)|$)/)?.[1]
            ...
            ??
                sectionContent.match(/[-*]\s*(.+)/g)
                    ?.map((s) => s.replace(/^[-*]\s*/, "").trim())
                    .filter(Boolean);
        const steps2 = stepLines2
            ? stepLines2.map((s) => s.replace(/^-\s*/, "").trim()).filter(Boolean)
            : ["参考文档中的模式描述"];
        // 提取 expected output
...
//# sourceMappingURL=skill-persister.js.map