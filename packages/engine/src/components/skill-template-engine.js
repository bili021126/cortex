...
export class SkillTemplateEngine {
     ...
     */
    renderEachLine(templates, context) {
    ...
    renderVariables(template, context) {
        const variableRegex = new RegExp(`${this.escapeRegex(this.openTag)}\\s*(?!#|/)([\\s\\S]+?)\\s*${this.escapeRegex(this.closeTag)}`, "g");
    ...
    renderConditionals(template, context) {
        const ifRegex = new RegExp(`${this.escapeRegex(this.openTag)}\\s*#if\\s+(.+?)\\s*${this.escapeRegex(this.closeTag)}([\\s\\S]*?)${this.escapeRegex(this.openTag)}\\s*/if\\s*${this.escapeRegex(this.closeTag)}`, "g");
        return template.replace(ifRegex, (_match, condition, body) => {
            const conditionValue = this.evaluateCondition(condition.trim(), context);
            // 处理 {{else}} 分支
            const elseMatch = body.match(new RegExp(`${this.escapeRegex(this.openTag)}\\s*else\\s*${this.escapeRegex(this.closeTag)}([\\s\\S]*)`));
            let trueBody;
            let falseBody;
            if (elseMatch) {
    ...
    // ── 循环渲染 ──────────────────────────────────────────────
    renderEach(template, context) {
        const eachRegex = new RegExp(`${this.escapeRegex(this.openTag)}\\s*#each\\s+(.+?)\\s*${this.escapeRegex(this.closeTag)}([\\s\\S]*?)${this.escapeRegex(this.openTag)}\\s*/each\\s*${this.escapeRegex(this.closeTag)}`, "g");
        return template.replace(eachRegex, (_match, listExpr, body) => {
...
//# sourceMappingURL=skill-template-engine.js.map