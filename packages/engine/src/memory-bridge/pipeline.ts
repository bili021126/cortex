// @cortex/engine/memory-bridge/pipeline —— 记忆增强执行管道
// @layer 记忆层
...
import { runReActLoop, type ReActContext } from "../execution/react-loop.js";
...
/** 根据 agentType 推断记忆 kind */
function inferKind(agentType: string): MemoryKind {
  if (agentType === 'doc-govern' || agentType === 'ningguang') return 'Governance';
  if (agentType === 'meta' || agentType === 'ganyu') return 'Intent';
  if (agentType === 'code' || agentType === 'fix') return 'TaskLog';
  if (agentType === 'review' || agentType === 'analysis') return 'Insight';
  return 'TaskLog';
}
 ...
 *
 * @todo Core-2 — FSM 已接入（commit/rollback 前 guard 校验）
 */
async function _rememberResult(
  ...
  const source = { agentType: node.claimedBy?.[0] ?? agentType, taskId: node.id };
  // G3-3：自动注入 source（调用方未提供时使用当前 Agent 类型和任务 ID）
  // 已通过 node.claimedBy/agentType + node.id 确保 source 始终非空
  const memoryKind = inferKind(agentType);
  ...
  try {
    ...
    {
      const sm = new MemoryEntryStateMachine("pending");
      const ok = sm.cas(memId, "pending", "commit", {
        weight: isSuccess ? 5 : 3,
        memoryId: memId,
      });
      if (!ok) {
    ...
    if (!ok1 || !ok2) {
      ...
      if (isFix && node.parentId) {
        ...
        if (parentMemories.length > 0) {
          const parentMem = parentMemories[0];
          if (parentMem) {
            memory.link(memId, parentMem.id, LinkType.ProducedBy);
          }
        }
      }
    }
    ...
    try {
      if (memId !== undefined) {
        const sm = new MemoryEntryStateMachine("pending");
        const ok = sm.cas(memId, "pending", "rollback", { memoryId: memId });
        if (!ok) {
           
          console.warn(`[memory] FSM guard 拒绝 rollback: memId=${memId} from=${sm.current}`);
        } else {
          memory.cancel(memId);
        }
      }
    } catch (err) { DegradationBoundary.handle(err, 'memory-pipeline-cleanup', 'trace'); }
    try {
      if (ctxMemId !== undefined) {
        const sm = new MemoryEntryStateMachine("pending");
        const ok = sm.cas(ctxMemId, "pending", "rollback", { memoryId: ctxMemId });
        if (!ok) {
           
          console.warn(`[memory] FSM guard 拒绝 rollback: ctxMemId=${ctxMemId} from=${sm.current}`);
        } else {
