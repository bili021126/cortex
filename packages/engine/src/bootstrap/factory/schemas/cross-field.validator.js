...
export function validateCrossField(config) {
    ...
    // 收集所有 produces 声明
    const allProduces = new Map(); // eventType → agentIds[]
    ...
    for (const [eventType, agentIds] of allProduces) {
        if (!routeTable[eventType]) {
            result.valid = false;
            result.errors.push(`跨字段校验失败: 事件 "${eventType}" 被 ${agentIds.join(", ")} 声明为 produces，但 routeTable 中无对应路由`);
        }
    }
    // ── 维度二：消费端校验 ──
    for (const [eventType, entry] of Object.entries(routeTable)) {
        ...
        if (!validChannels.includes(entry.channel)) {
            result.valid = false;
            result.errors.push(`跨字段校验失败: routeTable["${eventType}"].channel = "${entry.channel}" 不是合法的通道值。有效值: ${validChannels.join(", ")}`);
        }
        // 检查是否有 Agent 声明了 produces 但路由指向不存在的通道
        if (!allProduces.has(eventType) && eventType !== "MERGED:") {
            // 路由表中定义了但无 Agent 生产——可能是配置残留
            result.warnings.push(`跨字段校验警告: routeTable 中定义了 "${eventType}" 的路由，但没有 Agent 声明 produces 该事件`);
        }
    }
    // ── 维度三：冲突检测 ──
    for (const [eventType, agentIds] of allProduces) {
        if (agentIds.length > 1) {
            ...
            if (!hasMergeRule) {
                result.warnings.push(`跨字段校验警告: 事件 "${eventType}" 被 ${agentIds.length} 个 Agent (${agentIds.join(", ")}) 声明 produces，但未配置 mergeRule。建议添加归并规则`);
            }
        }
    }
        ...
        if (agent.tags) {
            for (const tag of agent.tags) {
                if (typeof tag !== "string" || !tag.trim()) {
                    result.valid = false;
                    result.errors.push(`跨字段校验失败: agents.${agentId}.tags 包含无效标签: "${tag}"`);
                }
            }
        }
        // toolPermissions 内的值必须有效——从 config.tools 读取真相源而非硬编码
        if (agent.toolPermissions) {
            const validTools = config.tools ? Object.keys(config.tools) : ["read_file", "write_file", "search_code", "web_search", "run_shell", "list_files", "delete_file", "parse_ast", "browser_do"];
            for (const tool of agent.toolPermissions) {
                if (!validTools.includes(tool)) {
                    result.warnings.push(`跨字段校验警告: agents.${agentId}.toolPermissions 包含未知工具: "${tool}"。有效工具: ${validTools.join(", ")}`);
                }
            }
        }
    }
...
//# sourceMappingURL=cross-field.validator.js.map