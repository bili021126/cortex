...
// ════════════════════════════════════════════════════════════
export function createEngineCore(toolkit) {
...
// ════════════════════════════════════════════════════════════
export async function createSpecialAgents(config, llms, codingStandards, observer, workspaceRoot) {
    // MetaAgent（甘雨）
    const metaDef = config.agentDefinitions.find((d) => d.type === "meta");
    const metaAgent = new MetaAgent(resolveLlm(llms, metaDef?.key), undefined, metaDef?.planningPrompt, metaDef?.replanPrompt, observer, // v2.5.41: 注入管线——MetaAgent 订阅事件以获取执行层信息
    workspaceRoot);
...
// ════════════════════════════════════════════════════════════
export function configAndInject(config, toolkit) {
    injectRegistryFromConfig(config.agentDefinitions);
    if (config.tools) {
        toolkit.setToolMeta(config.tools);
    }
}
// ════════════════════════════════════════════════════════════
// 工厂: createScheduler — 创建调度器
// ════════════════════════════════════════════════════════════
export function createScheduler(board, pool, observer, metaAgent, engineConfig) {
