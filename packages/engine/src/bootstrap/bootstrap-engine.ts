...
export async function bootstrapEngine(
  ...
  // 3b. StrategistAgent（钟离 + 霜凝）—— 从 JSON 配置注入 systemPrompt
  const strategistDefs = config.agentDefinitions.filter(
    (d: AgentDefinition) => d.type === "strategist",
  );
  const strategists = new Map<string, StrategistAgent>();
  for (const def of strategistDefs) {
    const agent = new StrategistAgent(options.llm, def.systemPrompt);
    await agent.wakeup();
