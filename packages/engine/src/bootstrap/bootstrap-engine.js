 ...
 */
export async function bootstrapEngine(projectRoot, options) {
    // §1 加载配置
    ...
    let pluginsJson;
    try {
        pluginsJson = JSON.parse(readFileSync(`${pluginsDataDir}/engine-plugins.json`, "utf-8"));
    }
    catch (e) {
        throw new Error(`[bootstrapEngine] 无法加载引擎插件清单 ${pluginsDataDir}/engine-plugins.json: ` +
    ...
    const pluginConfig = {
        ...
        onPostInit: async (ctx, plugins) => {
            const schedulerPlugin = plugins.get("scheduler");
            if (schedulerPlugin) {
    ...
    // §6 从插件容器中取出组件
    const observer = container.get("pipelineObserver").getInstance();
    const board = container.get("taskBoard").getInstance();
    const pool = container.get("agentPool").getInstance();
    const gate = container.get("confirmGate").getInstance();
    const cliAdapter = container.get("confirmGate").getCliAdapter();
    // 信任模型可选注入——不在插件清单中时静默跳过
    if (container.has("trustModel")) {
        const trustModel = container.get("trustModel").getInstance();
        gate.setTrustModel(trustModel);
    }
    const memory = container.get("memoryStore").getInstance();
    const consistencyLayer = container.get("consistencyLayer").getInstance();
    const metaAgent = container.get("metaAgent").getInstance();
    // §6.0a 注入 PromptManager —— MetaAgent 的 planning prompt 走声明式块组装
    metaAgent.setPromptManager(promptManager);
    const scheduler = container.get("scheduler").getInstance();
    const agents = container.get("scheduler").getAgents();
    const butler = container.get("scheduler").getButler();
    ...
    if (taskRouter) {
        const compositeRouter = {
            name: "core-2-composite",
            route: async (node, agentType, _defaultModel) => {
                const decision = await taskRouter.route(node, agentType);
                return await envRouter.resolve(decision.model, node);
            },
        };
    ...
    const sentinelHandler = (event) => {
        const signal = sentinelFilter.filter(event);
        if (signal?.suggestedAction === "alert") {
            void import("@cortex/telemetry").then(({ recordTelemetry }) => recordTelemetry("sentinel.alert", 1, [
    ...
    const skillRegistry = await initSkillSystem(observer, memory, metaAgent, projectRoot);
    // §7 创建 StrategistAgent（钟离 + 霜凝——Core-2 预留）
    const strategistDefs = config.agentDefinitions.filter((d) => d.type === "strategist");
    const strategists = new Map();
    for (const def of strategistDefs) {
        const agent = new StrategistAgent(resolveLlm(options.llms, def.key), codingStandards ? def.systemPrompt?.replace(/%%CODING_STANDARDS%%/g, codingStandards) ?? "" : def.systemPrompt ?? "");
        await agent.wakeup();
        strategists.set(def.id, agent);
    }
    // §7.1 StrategistAgent 订阅治理事件
    const strategistHandlers = [];
    for (const agent of strategists.values()) {
        const handler = (event) => agent.onGovernanceEvent(event);
...
//# sourceMappingURL=bootstrap-engine.js.map