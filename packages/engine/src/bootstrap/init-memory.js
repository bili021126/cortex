...
// ─── 初始化 MemoryStore ──────────────────────────────
export async function initMemoryStore(observer, memory, dbPath) {
    let store = memory;
    if (!store && dbPath) {
...
export async function initConsistencyLayer(memory, projectRoot, fs, existingFilterRead) {
    if (!memory)
        return undefined;
    // ConsistencyLayer 需要 MemoryStore 具体实例（内部依赖其私有API）
    // IMemoryStore 实现者即 MemoryStore，此转型安全
    const consistencyLayer = new ConsistencyLayer(memory, {
        projectRoot,
        ...
        try {
            ...
            if (report) {
                const missingRatio = report.checkedMemories > 0
                    ...
                    : 0;
                process.stderr.write(`[ConsistencyLayer] 启动校验完成——总数 ${report.totalMemories}，` +
...
//# sourceMappingURL=init-memory.js.map