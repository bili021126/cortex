 ...
 */
function collectFacts(workspaceRoot, safeReporter, timeouts) {
    const facts = [];
    ...
    try {
        try {
            ...
            if (tscOut.trim())
                facts.push(`[tsc 输出] ${tscOut.trim().slice(0, 500)}`);
        }
        catch (e) {
            const err = e;
 ...
 */
export function createInspectorAgent(llm, toolkit, memory, engineConfig, systemPrompt, filterRead) {
    ...
    wrapped.setWorkspaceRoot = function (root) {
        workspaceRoot = root;
    };
    wrapped.setSafeReporter = function (reporter) {
        safeReporterRef = reporter;
        agent.setSafeReporter(reporter);
    };
    return wrapped;
}
//# sourceMappingURL=inspector-agent.js.map