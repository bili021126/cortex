import { AgentType as AT, AgentStatus as AS, PipelineEventType } from "@cortex/shared";
...
export class StrategistAgent {
    ...
    onGovernanceEvent(event) {
        if (event.type !== PipelineEventType.ConstitutionViolation)
            return;
    ...
    async execute(node, model) {
        ...
        try {
            const messages = [
                ...
                { role: "user", content: node.payload },
            ];
