...
export function analysisMemoryQuery(node: TaskNode): MemoryQuery {
...
export function analysisAgentConfig(): AgentFactoryConfig {
  return {
    type: AT.Analysis,
    systemPrompt: SYSTEM_PROMPT,
    memoryEnabled: true,
    getMemoryQuery: analysisMemoryQuery,
  };
}
...
export class AnalysisAgent extends BaseAgent {
  ...
  protected getMemoryQuery(node: TaskNode): MemoryQuery {
    return analysisMemoryQuery(node);
  }
}
