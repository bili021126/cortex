...
export function createMemoryQuery(params) {
    return (node) => makeMemoryQuery(node, { ...params });
}
// ─── 声明式注册表（新增 Agent 只需在此追加一项） ──────────
export const AGENT_REGISTRY = [
    {
        type: AgentType.Code,
        memoryParams: { kind: "TaskLog", linkTypes: ["ProducedBy", "DerivedFrom"], bfsDepth: 2, limit: 3 },
        autoRegister: true,
    ...
    },
    {
        ...
        memoryParams: { kind: "TaskLog", linkTypes: ["DerivedFrom", "DerivedFrom"], bfsDepth: 2, limit: 5 },
        autoRegister: true,
        description: "代码审查——刻晴",
        capability: {
    ...
    },
    {
        ...
        memoryParams: { kind: "TaskLog", linkTypes: ["ProducedBy", "ConfirmedUseful"], bfsDepth: 2, limit: 3 },
        autoRegister: true,
        description: "深度分析——莫娜",
        capability: {
    ...
    {
        type: AgentType.Ops,
        memoryParams: { kind: "TaskLog", linkTypes: ["ProducedBy", "ConfirmedUseful"], bfsDepth: 3, limit: 10 },
        autoRegister: true,
        description: "运维操作——北斗",
        capability: {
    ...
    {
        type: AgentType.Loop,
        memoryParams: { kind: "TaskLog", linkTypes: ["ProducedBy", "DerivedFrom"], bfsDepth: 2, limit: 5 },
        autoRegister: true,
        description: "模式发现——莫娜",
        capability: {
    ...
    },
    {
        ...
        memoryParams: { kind: "TaskLog", linkTypes: ["DerivedFrom", "DerivedFrom"], bfsDepth: 2, limit: 3 },
        autoRegister: true,
        description: "治理审计——凝光",
        capability: {
    ...
    {
        type: AgentType.Api,
        memoryParams: { kind: "TaskLog", linkTypes: ["DerivedFrom", "ProducedBy"], bfsDepth: 2, limit: 5 },
        autoRegister: true,
    ...
    {
        type: AgentType.Fix,
        memoryParams: { kind: "TaskLog", linkTypes: ["ProducedBy", "DerivedFrom", "ConfirmedUseful"], readMode: "CSA", bfsDepth: 2, limit: 3 },
        autoRegister: true,
...
// ─── 按类型查找 ────────────────────────────────────
export function findRegistration(type) {
    return AGENT_REGISTRY.find((r) => r.type === type);
}
export function getAutoRegisterable() {
...
if (!opsReg)
    throw new Error("Ops agent not found in registry");
// ─── 向后兼容：具名导出 ──
/** 通用：从注册表按类型生成具名 config + memoryQuery */
function namedExports(type) {
...
export function opsMemoryQuery(node) {
    const reg = opsReg;
    if (!reg)
        return { kind: undefined, linkTypes: [], bfsDepth: 2, limit: 20 };
...
export function opsAgentConfig(systemPrompt) {
    return {
        type: AgentType.Ops,
        systemPrompt,
        memoryEnabled: true,
