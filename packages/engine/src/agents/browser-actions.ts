...
export const BUILTIN_BROWSER_ACTIONS: BrowserActionDef[] = [
  {
    name: "navigate",
    requiredParams: ["url"],
  ...
  // wait —— 等待条件满足（选择器出现 / 固定毫秒 / 网络空闲）
  {
    ...
    handler: async (page, params, timeout) => {
      const waitFor = params.waitFor as string;
      const selector = params.selector as string | undefined;
  ...
  // scroll —— 页面滚动（顶部/底部/指定元素）
  {
    ...
    handler: async (page, params, _timeout) => {
      ...
      const selector = params.scrollToSelector as string | undefined;
 ...
 */
export function buildBrowserDoHandler(
  actions: BrowserActionDef[],
  pageRef: { current: Page | null },
): (params: BrowserActionParams) => Promise<BrowserActionResult> {
  const actionMap = new Map<string, BrowserActionDef>();
  for (const a of actions) actionMap.set(a.name, a);

  return async (params: BrowserActionParams): Promise<BrowserActionResult> => {
