...
describe("SkillExtractor", () => {
    ...
    it("should skip entry without steps", () => {
        ...
        expect(result.skills.length).toBe(0);
        expect(result.diagnostics.some((d) => d.includes("缺少 steps"))).toBe(true);
    });
    ...
    it("should downgrade active status to trial", () => {
        ...
        expect(result.skills.length).toBe(1);
        expect(result.skills[0].status).toBe("trial");
        expect(result.diagnostics.some((d) => d.includes("降级"))).toBe(true);
    });
    it("should preserve tags not in vocabulary with diagnostic", () => {
        ...
        expect(result.diagnostics.some((d) => d.includes("not_a_tag"))).toBe(true);
    });
    ...
    it("should normalize unknown agentType to action", () => {
        ...
        expect(result.diagnostics.some((d) => d.includes("kind"))).toBe(true);
    });
...
//# sourceMappingURL=skill-extractor.test.js.map