...
describe("ResiliencePolicyFactory", () => {
    ...
    describe("execute() 韧性保护执行", () => {
        ...
        it("函数抛异常 → 重试后仍然抛出", async () => {
            ...
            let attempts = 0;
            await expect(factory.execute("fail-component", async () => {
                attempts++;
        ...
        it("未注册组件 → 抛出错误", async () => {
            await expect(factory.execute("unregistered-component", async () => "value")).rejects.toThrow();
        });
        it("超时保护 → 长时间运行被截断", async () => {
            ...
            await expect(factory.execute("slow-component", async () => {
                // 模拟长时间运行
