...
describe("Toolkit sandbox", () => {
    ...
    it("P0-5 regression: setWorkspaceRoot confines file reads within sandbox", async () => {
        ...
        const result = await tk.execute({ toolName: "read_file", params: { file_path: absPath } }, AgentType.Code);
        expect(result.success).toBe(true);
    });
    it("P0-5 regression: sandbox rejects path escape attempts", async () => {
        tk.setWorkspaceRoot(__dirname);
        // 工作区外的路径应被拒绝
        const result = await tk.execute({ toolName: "read_file", params: { file_path: "/etc/passwd" } }, AgentType.Code);
    ...
    it("P0-5 regression: agent without tool permission is denied", async () => {
        ...
        const result = await tk.execute({ toolName: "write_file", params: { file_path: "toolkit.test.ts", content: "x" } }, AgentType.Butler);
        expect(result.success).toBe(false);
        expect(result.error).toContain("not permitted");
    });
    it("P0-5 regression: LegacyAgent (via PoolAwareState) workspace root is inherited", async () => {
        // 未设沙箱时允许任意路径（向后兼容测试场景）
        const result = await tk.execute({ toolName: "list_files", params: { dir_path: __dirname } }, AgentType.Code);
        expect(result.success).toBe(true);
    });
});
