...
import { LifecyclePhase, type ILifecycle } from "@cortex/shared";

// ── 辅助：生成可追踪的 ILifecycle 实现 ───────

interface CallRecord {
  component: string;
  method: "init" | "start" | "stop" | "dispose";
}

function makeComponent(
  name: string,
  errorOn?: "init" | "start" | "stop",
): ILifecycle & { calls: CallRecord[]; _phase: LifecyclePhase } {
  ...
  return {
    ...
    async init() {
      if (errorOn === "init") throw new Error(`${name} init failed`);
      calls.push({ component: name, method: "init" });
      _phase = LifecyclePhase.Running;
    },
    async start() {
      if (errorOn === "start") throw new Error(`${name} start failed`);
      calls.push({ component: name, method: "start" });
    },
    async stop() {
      if (errorOn === "stop") throw new Error(`${name} stop failed`);
...
describe("LifecycleManager", () => {
  ...
  it("三层依赖链: a ← b ← c 拓扑序正确", async () => {
    ...
    const aInitEnd = Math.max(
      ...a.calls.filter((c) => c.method === "init").map((_, i) => i),
    );
