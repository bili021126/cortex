...
import { decompose, shouldDecompose, shouldExecuteDecomposition, parseDecomposeResponse, buildDecomposePrompt, MAX_RLM_DEPTH, } from "@cortex/scheduler";
// ── 辅助 ──────────────────────────────────────────────
/** 创建条件满足的 mock LLM */
...
describe("decompose — 主拆解流程", () => {
    it("超长 payload LLM 成功返回 → 解析子任务", async () => {
        ...
        // 验证传入的 messages 结构
        const callArgs = llm.mock.calls[0];
    ...
    it("LLM 返回 JSON 解析失败 → 回退空结果", async () => {
        const llm = vi.fn().mockResolvedValue("this is not valid JSON {broken");
        const result = await decompose(llm, "test-model", LONG_PAYLOAD);
        expect(result.subTasks).toHaveLength(0);
        expect(result.confidence).toBe(0);
        expect(result.rationale).toContain("JSON 解析失败");
...
//# sourceMappingURL=rlm-decompose.test.js.map