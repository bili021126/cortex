// @ci: unit
/**
 * MemoryStore 关闭保护测试 —— 修复 D3：read() 关闭保护
 *
 * 验证点：
 * 1. close() 后 read() 抛出 Error
 * 2. close() 后 write() 抛出 Error（已有保护，验证一致性）
 */
import { describe, it, expect, beforeEach } from "vitest";
import { MemoryStore } from "@cortex/memory-store";
import { InMemoryMemoryStore } from "@cortex/memory";
import { AgentType } from "@cortex/shared";
const mockEmbedder = {
    embedText: async () => { throw new Error("mock embedding unavailable"); },
    embedBatch: async () => { throw new Error("mock embedding unavailable"); },
};
describe("D3: MemoryStore read() 关闭保护", () => {
    let store;
    beforeEach(async () => {
        store = new MemoryStore(new InMemoryMemoryStore(), undefined, mockEmbedder);
        await store.init(":memory:");
    });
    it("正常状态下 read() 正常工作", async () => {
        const id = await store.write({
            kind: "TaskLog",
            content_blob: { test: true },
            summary: "test memory",
            semantic_gist: "test memory",
            source: { agentType: AgentType.Code, taskId: "" },
            content_hash: "",
        });
        const results = await store.read({ keywords: ["test"], kind: "TaskLog", limit: 10 });
        expect(results.length).toBe(1);
        expect(results[0].id).toBe(id);
    });
    it("close() 后 read() 抛出 Error", async () => {
        await store.write({
            kind: "TaskLog",
            content_blob: { test: true },
            summary: "test memory",
            semantic_gist: "test memory",
            source: { agentType: AgentType.Code, taskId: "" },
            content_hash: "",
        });
        await store.close();
        await expect(store.read({ keywords: ["test"], kind: "TaskLog", limit: 10 })).rejects.toThrow(/已关闭/);
    });
    it("close() 后 write() 抛出 Error（与现有行为一致）", async () => {
        await store.close();
        await expect(store.write({
            kind: "TaskLog",
            content_blob: { test: true },
            summary: "test",
            semantic_gist: "test",
            source: { agentType: AgentType.Code, taskId: "" },
            content_hash: "",
        })).rejects.toThrow(/已关闭/);
    });
});
//# sourceMappingURL=memory-store-close-read.test.js.map