...
import { createAgent } from "@cortex/engine";
function mockLlm() {
...
describe("createAgent (factory)", () => {
    ...
    it("should execute a task and return success", async () => {
        const agent = createAgent(config, mockLlm(), new Toolkit());
        await agent.wakeup();
        const result = await agent.execute({ id: "n1", type: "test", payload: "Do nothing", tags: [], needsMultiPerspective: false, status: "pending", claimedBy: [], results: [], createdAt: Date.now() }, "mock-model");
    ...
    it("should support maxLoops override", async () => {
        ...
        const agent = createAgent({ type: AgentType.Code, systemPrompt: "Test", maxLoops: 4 }, infiniteMocker, tk);
        await agent.wakeup();
        const result = await agent.execute({ id: "n2", type: "test", payload: "Loop forever", tags: [], needsMultiPerspective: false, status: "pending", claimedBy: [], results: [], createdAt: Date.now() }, "mock-model");
...
describe("AgentFactoryConfig validation", () => {
    it("should default maxLoops to 64", () => {
        const agent = createAgent({ type: AgentType.Review, systemPrompt: "Review" }, mockLlm(), new Toolkit());
        // No assertion needed — just verifies no throw
        expect(agent.type).toBe(AgentType.Review);
    });
    it("should accept maxLoops override", () => {
        const agent = createAgent({ type: AgentType.Inspector, systemPrompt: "Inspect", maxLoops: 24 }, mockLlm(), new Toolkit());
        expect(agent.type).toBe(AgentType.Inspector);
    });
});
describe("PoolAwareState integration", () => {
    it("should report status from PoolAwareState", () => {
        const agent = createAgent({ type: AgentType.Code, systemPrompt: "Test" }, mockLlm(), new Toolkit());
        expect(agent.status).toBe(AS.Created);
    });
