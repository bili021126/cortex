...
function makeMockObserver(): IPipelineObserver & { emitted: ObservableEvent[]; handlers: Map<PipelinePriority, Array<(e: ObservableEvent) => void>> } {
  const handlers = new Map<PipelinePriority, Array<(e: ObservableEvent) => void>>();
  ...
  return {
    ...
    on(priority: PipelinePriority, handler: (event: ObservableEvent) => void): void {
      const hs = handlers.get(priority) ?? [];
      hs.push(handler);
      handlers.set(priority, hs);
    },
    off(priority: PipelinePriority, handler?: (event: ObservableEvent) => void): void {
      if (!handler) {
...
describe("MemoryStoreMonitor", () => {
  ...
  it("persist_failed 事件落盘归档", () => {
    ...
    const archiveCalls = stderrSpy.mock.calls.filter(
      (c) => String(c[0]).includes("ARCHIVE")
    );
