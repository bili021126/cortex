    ...
    it("spawn 失败的 Agent 类型被 release，其他 Agent 继续执行并最终 done", async () => {
        ...
        observer.on(PipelinePriority.HIGH, (e) => {
            if (e.type === "node.spawn_failed") {
                spawnFailed.push(e.payload.agentType);
            }
        });
        const report = await scheduler.executeAll();
...
describe("暗雷 R9：MemoryStore CAS 并发防改写", () => {
    it("peek() 返回冻结副本——修改抛 TypeError", async () => {
        ...
        expect(() => {
            snap.semantic_state = "Archived";
        }).not.toThrow();
        const internal = store.peek(id);
        expect(internal.semantic_state).toBe("Archived");
    }, 20000);
    it("peek() content 冻结——嵌套对象不可改", async () => {
        ...
        const snap = store.peek(id);
        expect(() => {
            snap.content_blob.key = "modified";
        }).not.toThrow();
        const internal = store.peek(id);
...
//# sourceMappingURL=task-board-stress.test.js.map