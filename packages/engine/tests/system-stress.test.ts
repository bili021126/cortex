...
function selectiveFailAdapter(failAtCalls: Set<number>, failMsg = "Injected failure") {
  const adapter = new LlmAdapter({
    apiKey: "mock", baseUrl: "mock", chatModel: "mock", reasonerModel: "mock"});
  let callCount = 0;
  adapter.injectMock(async () => {
    callCount++;
    if (failAtCalls.has(callCount)) throw new Error(failMsg);
...
describe("场景 1：十层深?+ 随机失败注入", { timeout: 30_000 }, () => {
  // 单测试超?15s，链式调度不应超过此?
  ...
  it("10 层链中有 3 个失败层——Scheduler 应优雅完成且执行顺序正确", async () => {
    ...
    const agent = createAgent(
      ...
      selectiveFailAdapter(failAt, "Intentional failure"),
      new Toolkit(),
    );
    await agent.wakeup();
...
describe("场景 2：0 节点同层并行 + 池约", () => {
  ...
  it("20 同层并行 + maxInstances=3——mHC 流约束门控，所有节点排队等待后全部成功", async () => {
    ...
    const agent = createAgent(
      codeAgentConfig("test"),
      mockAdapter("Partial done"),
      new Toolkit(),
    );
    await agent.wakeup();
    scheduler.register(AgentType.Code, agent, "mock");

    const poolExhaustedErrors: string[] = [];
    observer.on(PipelinePriority.HIGH, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeSpawnFailed) {
        poolExhaustedErrors.push((e.payload as any).nodeId);
      }
    });
...
describe("场景 3：重规划预算耗尽", () => {
  it("maxReplanPerNode=3 + maxTotalReplans=3——单节点重复失败耗完全部预算后熔断", async () => {
    ...
    observer.on(PipelinePriority.HIGH, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeReplanQueued) {
        replanEvents.push({ attempt: (e.payload as any).attempt, type: "queued" });
      }
    });
  ...
  it("maxTotalReplans=3——两个不同节点的失败共享全局 budget", async () => {
    ...
    const replanByNode: Record<string, number> = { A: 0, B: 0 };
    observer.on(PipelinePriority.CRITICAL, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeReplan) {
        const nodeId = (e.payload as any).nodeId;
  ...
  it("预算耗尽?executeAll 快速终止——不依赖超时空转（D2 已修复）", async () => {
    ...
    const agent = createAgent(
      codeAgentConfig("test"),
      failAdapterFn("Persistent failure"),
      new Toolkit(),
    );
    await agent.wakeup();
    scheduler.register(AgentType.Code, agent, "mock");
  ...
  it("executeAll 调用——reset 防止状态泄漏", async () => {
    ...
    observer.on(PipelinePriority.CRITICAL, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeReplan) run1ReplanCount++;
    });
    ...
    observer.on(PipelinePriority.CRITICAL, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeReplan) run2ReplanCount++;
    });
...
describe("场景 7：多类型 Agent 并行", () => {
  it("Code/Review/Analysis 三类型节点各 5 个，全部正确路由", async () => {
    ...
    observer.on(PipelinePriority.HIGH, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeComplete) {
        const p = e.payload as any;
        if (p.nodeId.startsWith("impl-")) codeHandled.push(p.nodeId);
        else if (p.nodeId.startsWith("rev-")) reviewHandled.push(p.nodeId);
        else if (p.nodeId.startsWith("ana-")) analysisHandled.push(p.nodeId);
      }
    });
  ...
  it("三类?Agent + 混合节点——review 节点正确路由?Review Agent", async () => {
    ...
    const reviewHandled: string[] = [];
    observer.on(PipelinePriority.HIGH, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeComplete && (e.payload as any).nodeId.startsWith("r")) {
        reviewHandled.push((e.payload as any).nodeId);
      }
    });
...
describe("场景 7+? Scheduler 共享 AgentPool 并发", () => {
  it("3 Scheduler 同时 executeAll——共享池无竞态崩", async () => {
    ...
    const reports = await Promise.all(
      results.map((r) => r.scheduler.executeAll()),
    );
...
describe("场景 8?0 轮耐久", () => {
  it("50 ?executeAll——每?2 节点，总执行无衰减、无崩溃", async () => {
    ...
    const agent = createAgent(
      codeAgentConfig("endurance"),
      mockAdapter("endurance ok"),
      new Toolkit(),
    );
    await agent.wakeup();
  ...
  it("50 ?executeAll——每 10 轮采?heapUsed，验证无内存泄漏（增?< 2x 初始值）", async () => {
    ...
    const agent = createAgent(
      codeAgentConfig("endurance-leak"),
      mockAdapter("leak-check ok"),
      new Toolkit(),
    );
    await agent.wakeup();
...
describe("场景 12：removeSubtree + NodeRemoved 事件", () => {
  it("replan impactScope=subtree——子节点?removeSubtree 回收，NodeRemoved 事件触发", async () => {
    ...
    codeAdapter.injectMock(async () => {
      calls++;
      if (calls === 1) throw new Error("Parent node fails");
    ...
    observer.on(PipelinePriority.NORMAL, (e: ObservableEvent) => {
      if (e.type === PipelineEventType.NodeRemoved) {
        removedNodes.push((e.payload as any).nodeId);
      }
    });
...
describe("场景 13：MemoryStore 事件完整性", () => {
  ...
  it("50 条记忆读取+ 10 条链式链接——全程无 MemoryDbWriteFailed/MemoryWriteBlocked 等异常事件", async () => {
    ...
    observer.on(PipelinePriority.HIGH, (e: ObservableEvent) => {
      if (errorTypes.has(e.type as PipelineEventType)) {
