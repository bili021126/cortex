...
describe("MemoryStore 生命周期状态机", () => {
    ...
    it("用例2: 后端 write() 失败时异常向上传播（假阳性禁止原则）", async () => {
        ...
        vi.spyOn(store._backend, "write").mockRejectedValueOnce(new Error("BACKEND_WRITE_FAILURE"));
        // Act & Assert: 必须抛出
        await expect(store.write({
            ...
            content_hash: "",
            source: { agentType: AgentType.Code, taskId: "" }
        })).rejects.toThrow("BACKEND_WRITE_FAILURE");
        await store.close();
    });
    ...
    it("用例3: 后端 write() 失败时异常传播（observer 可感知）", async () => {
        ...
        vi.spyOn(store._backend, "write").mockRejectedValueOnce(new Error("BACKEND_FAILURE"));
        try {
            await store.write({
                kind: "TaskLog",
    ...
    it("用例6: 无 observer 时 close() 正常（不抛异常）", async () => {
        const noObsStore = new MemoryStore(new InMemoryMemoryStore(), undefined, mockEmbedder);
...
//# sourceMappingURL=memory-store-lifecycle.test.js.map