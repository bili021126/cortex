...
describe("MetaAgent", () => {
    ...
    it("setSafeReporter 注入后 JSON 解析失败走 reporter 而非 console.warn", async () => {
        ...
        const meta = new MetaAgent(adapter);
        const reporterCalls = [];
        const reporter = (ctx) => { reporterCalls.push(ctx); };
        ...
        const metaAgentWarns = warnSpy.mock.calls.filter((c) => typeof c[0] === "string" && c[0].includes("[meta-agent]"));
...
//# sourceMappingURL=meta-agent.test.js.map