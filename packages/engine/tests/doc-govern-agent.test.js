...
function mockEmbedder() {
    ...
    function vec(seed) { let v = new Array(dim), s = seed; for (let i = 0; i < dim; i++) {
        s = (1664525 * s + 1013904223) | 0;
        v[i] = s / 2147483647;
    } let n = 0; for (let x of v)
        n += x * x; n = Math.sqrt(n); for (let i = 0; i < dim; i++)
        v[i] /= n; return v; }
    return { async embedText(t) { return vec(hash(t)); }, async embedBatch(ts) { return ts.map(t => vec(hash(t))); } };
}
/** 创建 Mock Adapter */
...
/** 按类型创建 Mock Agent */
async function mockAgent(agentType, output) {
    ...
    switch (agentType) {
        ...
        case AgentType.Analysis:
            ...
            break;
        case AgentType.DocGovern:
            agent = createAgent(docGovernAgentConfig("Test"), adapter, tk);
            break;
...
//# sourceMappingURL=doc-govern-agent.test.js.map