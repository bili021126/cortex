...
describe("amendment-timeout", () => {
  ...
  describe("checkTimeout", () => {
    ...
    it("skips approved/rejected/applied proposals", () => {
      for (const status of ["approved", "rejected", "applied"] as const) {
    ...
    it("handles multiple proposals with mixed results", () => {
      ...
      const actions = checkTimeout(
        [freshProposal, expiredProposal, expiredDraft, approvedProposal],
        tmpDir,
        { judgmentTTLDays: 7, draftTTLDays: 14 },
      );

      expect(actions).toHaveLength(2);
      expect(actions.map((a: { proposalId: string }) => a.proposalId).sort()).toEqual(["AM-011", "AM-012"]);
    });
  ...
  describe("updateStaleCount", () => {
    it("creates counter file and sets count to 1 on first call", () => {
      ...
      const counters = JSON.parse(fs.readFileSync(counterPath, "utf-8")) as Record<string, number>;
...
function writeProposal(dir: string, proposal: AmendmentProposal, daysAgo?: number): void {
