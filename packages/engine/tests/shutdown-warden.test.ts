...
import type { ILifecycle, IMemoryStore, MemoryEntry, MemoryWriteInput, MemoryQuery, ReadMode, MemoryLink, LinkType, SemanticState } from "@cortex/shared";
import { LifecyclePhase } from "@cortex/shared";

/** 轻量 Lifecycle mock — 仅用于测试 ShutdownWarden 编排 */
class MockLifecycle implements ILifecycle {
  phase: LifecyclePhase = LifecyclePhase.Created;
  private _initFn: () => Promise<void> = async () => {};
  private _stopFn: () => Promise<void> = async () => {};
  ...
  setInit(fn: () => Promise<void>) { this._initFn = fn; }
  setStop(fn: () => Promise<void>) { this._stopFn = fn; }
}

/** 轻量 IMemoryStore mock */
function mockMemory(): IMemoryStore {
  let closed = false;
  return {
    isPersisted: false,
    size: 0,
