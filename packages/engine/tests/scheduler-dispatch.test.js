...
function makeMockAgent(status = AgentStatus.Awake) {
    return {
        type: AgentType.Code,
        status,
        wakeup: vi.fn().mockResolvedValue(undefined),
        execute: vi.fn().mockResolvedValue({ nodeId: "node-1", success: true, output: "ok" }),
    ...
    it("P0-1 regression: NodeFailed event fires exactly once per failure (singleton dedup)", async () => {
        ...
        observer.on(PipelinePriority.CRITICAL, (e) => {
            if (e.type === PipelineEventType.NodeFailed) {
                failedEvents.push({ type: e.type, nodeId: e.payload.nodeId });
            }
        });
...
describe("Scheduler._findMatchingAgent 密度平局打破", () => {
    ...
    function makeReviewAgent() {
        return {
            ...
            wakeup: vi.fn().mockResolvedValue(undefined),
            execute: vi.fn().mockResolvedValue({ nodeId: "", success: true, output: "reviewed" }),
            shutdown: vi.fn().mockResolvedValue(undefined)
        };
    }
    function makeCodeAgent() {
        return {
            type: AgentType.Code,
            status: AgentStatus.Awake,
            wakeup: vi.fn().mockResolvedValue(undefined),
            execute: vi.fn().mockResolvedValue({ nodeId: "", success: true, output: "coded" }),
            shutdown: vi.fn().mockResolvedValue(undefined)
        };
    }
    beforeEach(() => {
...
//# sourceMappingURL=scheduler-dispatch.test.js.map