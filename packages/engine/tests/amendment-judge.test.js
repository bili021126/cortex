...
describe("evaluateAmendment", () => {
    ...
    it("before 不在宪法中 → BLOCKED", () => {
        ...
        const result = evaluateAmendment(proposal, FIXTURE_CONSTITUTION);
        expect(result.verdict).toBe("BLOCKED");
        const structuralCheck = result.checks.find((c) => c.id === "structural-consistency");
    ...
    it("版本号回退 → BLOCKED", () => {
        ...
        const versionCheck = result.checks.find((c) => c.id === "version-continuity");
    ...
    it("声明的交叉引用不存在 → BLOCKED", () => {
        ...
        const result = evaluateAmendment(proposal, FIXTURE_CONSTITUTION);
        expect(result.verdict).toBe("BLOCKED");
        expect(result.blocking.some((b) => b.includes("§9.99"))).toBe(true);
    });
    ...
    it("add 类型且 before 为空 → 不检查结构一致性", () => {
        ...
        const result = evaluateAmendment(proposal, FIXTURE_CONSTITUTION);
        expect(result.verdict).not.toBe("BLOCKED");
        expect(result.checks.find((c) => c.id === "structural-consistency").passed).toBe(true);
    });
    // ─── 不可变原则动态提取 ─────────────────────────
    it("动态提取不可变原则——当前 fixture 有 3 条", () => {
        const proposal = makeProposal();
        const result = evaluateAmendment(proposal, FIXTURE_CONSTITUTION);
        const pc = result.checks.find((c) => c.id === "principle-immutability");
...
//# sourceMappingURL=amendment-judge.test.js.map