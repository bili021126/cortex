...
// ─── Mock Agent ────────────────────────────
async function mockAgentByType(agentType, success = true, output = `done by ${agentType}`) {
    ...
    switch (agentType) {
        case AgentType.Code:
            agent = createAgent(codeAgentConfig("test"), adapter, tk);
            break;
        case AgentType.Review:
            agent = createAgent(reviewAgentConfig("test"), adapter, tk);
            break;
        case AgentType.Analysis:
            agent = createAgent(analysisAgentConfig("test"), adapter, tk);
            break;
...
describe("topologicalSort", () => {
    it("无依赖节点全部在?0", () => {
        const nodes = [
            { id: "a", tags: ["implementation"], payload: "A", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
            { id: "b", tags: ["implementation"], payload: "B", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
        ];
    ...
    it("子节点在父节点之后", () => {
        const nodes = [
            { id: "a", parentId: undefined, tags: ["implementation"], payload: "A", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
            { id: "b", parentId: "a", tags: ["implementation"], payload: "B", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
            { id: "c", parentId: "a", tags: ["implementation"], payload: "C", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
        ];
        const layers = topologicalSort(nodes);
    ...
    it("三层嵌套", () => {
        const nodes = [
            { id: "r", parentId: undefined, tags: ["implementation"], payload: "Root", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
            { id: "m", parentId: "r", tags: ["implementation"], payload: "Mid", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
            { id: "l", parentId: "m", tags: ["implementation"], payload: "Leaf", type: "impl", status: "pending", needsMultiPerspective: false, claimedBy: [], results: [], createdAt: 0 },
        ];
        const layers = topologicalSort(nodes);
...
describe("Scheduler", () => {
    ...
    it("调度层事件被正确发布", async () => {
        const events = [];
        const push = (e) => events.push(e.type);
