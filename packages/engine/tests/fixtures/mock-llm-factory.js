...
export function mockTextAdapter(output) {
...
export function mockScriptAdapter(steps) {
    const adapter = createBaseAdapter();
    let stepIndex = 0;
    adapter.injectMock(async () => {
...
export class E2eMockFactory {
    ...
    forCode(output, toolScript) {
        if (toolScript)
            return mockScriptAdapter(toolScript);
    ...
    forReview(findings, defectCount = 0, withRunShell = false) {
        const steps = [];
        if (withRunShell) {
            steps.push({
    ...
    forMetaAgent(planNodes) {
        const planJson = JSON.stringify(planNodes.map((n, i) => ({ id: `plan-${i}`, ...n })), null, 2);
        return mockTextAdapter(`## Task Plan\n\n\`\`\`json\n${planJson}\n\`\`\`\n\nExecute the plan in order.`);
    }
    /** 为 DocGovernAgent 创建 mock——模拟"审计通过" */
    forDocGovern(auditOutput) {
        return mockScriptAdapter([
            {
 ...
 */
export function createE2eMockFactory() {
    return new E2eMockFactory();
}
/**
 * 创建完整的 Agent Mock 集合（用于快速搭建测试场景）
 */
export function createFullAgentMockSet(scenario) {
...
//# sourceMappingURL=mock-llm-factory.js.map