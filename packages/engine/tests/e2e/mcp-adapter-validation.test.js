...
// ════════════════════════════════════════════════════════════
/** 验证 Tool 接口完整性 */
function validateToolAdapter(adapter, serverId, rawDef) {
    const issues = [];
...
async function safeStart(client, timeoutMs = 60_000) {
    try {
        const startPromise = client.start();
        const result = await Promise.race([
            startPromise,
            new Promise((_, reject) => setTimeout(() => reject(new Error("start timeout")), timeoutMs)),
        ]);
        return { ok: true };
    }
    catch (e) {
        ...
        describe(`Server: ${spec.id} (${spec.formatNotes})`, () => {
            ...
            if (spec.sample) {
                ...
                it(`[E] 采样执行: ${sample.tool}`, async () => {
                    if (adapters.length === 0)
                        return;
                    const adapter = adapters.find((a) => a.name === `mcp:${spec.config.id}:${sample.tool}` ||
...
//# sourceMappingURL=mcp-adapter-validation.test.js.map