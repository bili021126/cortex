...
import { Scheduler, createAgent, codeAgentConfig, reviewAgentConfig } from "@cortex/engine";
...
describe("闭环协作 Mock E2E", () => {
    ...
    it("CodeAgent 实现 → ReviewAgent 发现缺陷 → 修复闭环", async () => {
        // 第一轮：实现 + 审查（发现 bug）
        {
            ...
            const reviewAdapter = factory.forReview("Bug found: add function subtracts instead of adding. Line 1.", 1);
    ...
    it("多 Agent 并行协作——Code + Review + Analysis 同时执行", async () => {
        ...
        const codeAgent = createAgent(codeAgentConfig("mock"), codeAdapter, new Toolkit());
        const reviewAgent = createAgent(reviewAgentConfig("mock"), reviewAdapter, new Toolkit());
        const analysisAgent = createAgent({ type: AgentType.Analysis, systemPrompt: "mock", memoryEnabled: false }, analysisAdapter, new Toolkit());
        await codeAgent.wakeup();
        await reviewAgent.wakeup();
...
//# sourceMappingURL=closed-loop-mock.test.js.map