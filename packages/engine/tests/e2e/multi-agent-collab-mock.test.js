...
import { Scheduler, createAgent, codeAgentConfig, reviewAgentConfig, analysisAgentConfig, docGovernAgentConfig } from "@cortex/engine";
...
describe("多 Agent 协作 Mock E2E", () => {
    ...
    it("Code + Review + Analysis 三路并行 → 全部完成", async () => {
        ...
        const codeAgent = createAgent(codeAgentConfig("mock"), factory.forCode("export const VERSION = '1.0.0';"), new Toolkit());
        const reviewAgent = createAgent(reviewAgentConfig("mock"), factory.forReview("No defects. Code is clean.", 0), new Toolkit());
        const analysisAgent = createAgent(analysisAgentConfig("mock"), factory.forAnalysis("Architecture is properly layered. All dependencies point inward."), new Toolkit());
    ...
    it("治理审计 → 发现违规 → Code + Fix 修复 → 再审计通过", async () => {
        ...
        const auditAdapter2 = factory.forDocGovern("## Re-Audit Report\n\n- [x] Hardcoded paths fixed\n- [x] console.error removed\n\n**Verdict: PASS**");
    ...
    it("5 Agent 全类型注册——每种标签只匹配正确 Agent", async () => {
        ...
        // 添加 4 个不同类型的节点
        board.addNode({
            id: "n-code", type: "implementation", tags: ["code"],
            needsMultiPerspective: false, status: "pending", claimedBy: [],
            payload: "Write code.", results: [], createdAt: Date.now()
        });
        board.addNode({
            id: "n-review", type: "code_review", tags: ["review"],
            needsMultiPerspective: false, status: "pending", claimedBy: [],
            payload: "Review code.", results: [], createdAt: Date.now()
        });
        board.addNode({
            id: "n-analysis", type: "analysis", tags: ["analysis"],
            needsMultiPerspective: false, status: "pending", claimedBy: [],
            payload: "Analyze.", results: [], createdAt: Date.now()
        });
        board.addNode({
            id: "n-doc", type: "constitution_check", tags: ["constitution_check"],
            needsMultiPerspective: false, status: "pending", claimedBy: [],
...
//# sourceMappingURL=multi-agent-collab-mock.test.js.map