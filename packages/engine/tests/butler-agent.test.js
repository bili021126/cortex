...
function makeEvent(type, priority, payload = {}) {
    return { type: type, priority, payload: payload, timestamp: Date.now() };
}
describe("ButlerAgent", () => {
    ...
    it("CRITICAL: node.failed → 格式化失败消息", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("[昔涟-CRITICAL]"));
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("n1"));
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("LLM timeout"));
    });
    it("CRITICAL: node.replan → 格式化重规划消息", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("[昔涟-CRITICAL]"));
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("n2"));
    ...
    it("HIGH: node.start → 格式化节点开始消息", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("[昔涟]"));
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("n3"));
    });
    it("HIGH: node.complete → 格式化节点完成消息", async () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("[昔涟]"));
        expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining("n4"));
    });
...
//# sourceMappingURL=butler-agent.test.js.map