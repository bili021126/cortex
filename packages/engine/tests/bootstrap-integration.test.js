...
describe("T4: MemoryStore 读写闭环", () => {
    it("write → read → close 完整生命周期", async () => {
        ...
        const episodicEntries = await memory.read({ kind: "EPISODIC", limit: 0 });
        expect(episodicEntries.length).toBeGreaterThanOrEqual(3);
        // 验证每条内容存在
        for (const item of items) {
            const found = episodicEntries.find((e) => e.content_blob?.value === item.content.value);
            expect(found).toBeDefined();
        }
        await memory.close();
    }, 10000);
