...
import { createAgent, codeAgentConfig, reviewAgentConfig, analysisAgentConfig, MetaAgent, createInspectorAgent, Scheduler } from "@cortex/engine";
...
async function mockAgent(agentType, adapter) {
    ...
    it("CodeAgent 产出 → ReviewAgent 审查（拓扑排序保证顺序）", async () => {
        ...
        observer.on(PipelinePriority.HIGH, (e) => {
            if (e.type === "node.start" || e.type === "node.complete") {
                events.push(`${e.type}:${e.payload.nodeId}`);
            }
        });
        // ── Act ──
...
describe("重规划 失败 → MetaAgent 重规划", () => {
    ...
    it("CodeAgent 失败 → MetaAgent 重规划 → 新节点执行", async () => {
        ...
        observer.on(PipelinePriority.CRITICAL, (e) => {
            if (e.type === "node.replan") {
                replanAttempt = e.payload.attempt;
            }
        });
        // ── Act ──
        const report = await scheduler.executeAll();
    ...
    it("超过 3 轮重规划 → 放弃重规划", async () => {
        ...
        observer.on(PipelinePriority.CRITICAL, (e) => {
            if (e.type === "node.replan") {
                replanCounts.push(e.payload.attempt);
            }
        });
        // ── Act ──
        const report = await scheduler.executeAll();
...
//# sourceMappingURL=multi-agent-collab.test.js.map