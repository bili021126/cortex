...
import { SkillRegistry, SkillTemplateEngine, } from "@cortex/skill-kit";
import { TaskBoard, AgentPool, PipelineObserver, } from "@cortex/scheduler";
import { Scheduler, MetaAgent, createAgent, fixAgentConfig } from "@cortex/engine";
...
describe("Skill Bootstrap Full Pipeline Integration", () => {
    ...
    it("full pipeline: register skill -> match by tags -> inject -> execute agent -> feedback", async () => {
        ...
        const matched = registry.queryByTags(["fix", "ci"]);
        ...
        const injected = templateEngine.render("[技能注入: {{ name }}]\n触发: {{ trigger }}\n步骤:\n{{#each steps}}- {{ this }}\n{{/each}}", { name: skill.name, trigger: skill.trigger, steps: skill.steps });
    ...
    it("Multiple skills registered -> non-overlapping matching", () => {
        registry.registerAll([
            makeSkill({ id: "s1", name: "Fix A", triggerTags: ["fix"], steps: ["a"] }),
            makeSkill({ id: "s2", name: "Review A", triggerTags: ["review"], steps: ["b"] }),
            makeSkill({ id: "s3", name: "Code A", triggerTags: ["code"], steps: ["c"] }),
        ]);
        expect(registry.queryByTags(["fix"])[0].id).toBe("s1");
        expect(registry.queryByTags(["review"])[0].id).toBe("s2");
        expect(registry.queryByTags(["code"])[0].id).toBe("s3");
    });
});
// ═══════════════════════════════════════════════════════════
...
describe("Skill Persistence via MemoryStore", () => {
    ...
    it("skills can be persisted to and loaded from MemoryStore", { timeout: 30000 }, async () => {
        ...
        expect(entries.some((e) => e.content_blob?.skill?.id === "persist-skill")).toBe(true);
    });
});
//# sourceMappingURL=skill-bootstrap-integration.test.js.map