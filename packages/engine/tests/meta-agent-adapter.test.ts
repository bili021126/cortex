...
import type { MetaAgent } from "@cortex/engine";
...
function makeMockMetaAgent(): MetaAgent {
  return {
...
describe("MetaAgentReplanAdapter", () => {
  it("requestReplan 转发到 MetaAgent.requestReplan", async () => {
    ...
    expect(meta.requestReplan).toHaveBeenCalledWith(
      node, "test reason", 1, undefined, 3,
    );
  });
  ...
  it("requestBoundaryReplan 转发到 MetaAgent.requestBoundaryReplan", async () => {
    ...
    expect(meta.requestBoundaryReplan).toHaveBeenCalledWith(
      node, "boundary reason", 2, undefined, 5,
    );
  });

  it("requestReplan 返回值与 MetaAgent 一致", async () => {
    const meta = makeMockMetaAgent();
    (meta.requestReplan as ReturnType<typeof vi.fn>).mockResolvedValue("replan_ok");
    const adapter = new MetaAgentReplanAdapter(meta);

    const result = await adapter.requestReplan(
      makeNode({ id: "n3", type: "analysis" }), "reason", 1,
    );
  ...
  it("requestBoundaryReplan 返回值与 MetaAgent 一致", async () => {
    const meta = makeMockMetaAgent();
    (meta.requestBoundaryReplan as ReturnType<typeof vi.fn>).mockResolvedValue("boundary_ok");
    const adapter = new MetaAgentReplanAdapter(meta);

    const result = await adapter.requestBoundaryReplan(
      makeNode({ id: "n4", type: "ops" }), "reason", 1,
    );
  ...
  it("currentDepth 参数被跳过（适配为 undefined）", async () => {
    ...
    const callArgs = (meta.requestReplan as ReturnType<typeof vi.fn>).mock.calls[0];
