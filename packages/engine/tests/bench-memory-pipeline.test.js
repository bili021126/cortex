...
import { BM25Index, } from "@cortex/memory-store";
import { HybridRetriever, cosineSimilarity, batchCosineSimilarity, } from "@cortex/memory-store";
import { CognitiveEngine, DEFAULT_COGNITIVE_CONFIG, fourierTimeDecay, ebbinghausRetention, BoundaryRegressor, } from "@cortex/memory-store";
...
function memoryVec(topic, memoryIndex) {
...
function toEntry(t) {
    return {
        id: t.id,
        source: { agentType: "Code", taskId: "task-" + t.id },
        kind: t.kind,
        summary: t.summary,
...
function mrr(predicted, groundTruthIds) {
...
describe("Bench ③: 认知引擎 vs 简单权重排序", () => {
    ...
    it("艾宾浩斯遗忘: 频繁访问的旧记忆 > 从未访问的新记忆", () => {
        ...
        const oldFrequent = {
            id: "old-freq",
            source: { agentType: "Code", taskId: "t" },
    ...
    it("CognitiveEngine.scoreAndRank 批量评分输出完整结构", () => {
        ...
        const emptyGetLinks = () => [];
        const emptyGetEntry = () => undefined;
        const scored = engine.scoreAndRank(entries, hybridScores, "登录 修复", BASE_TIME, emptyGetLinks, emptyGetEntry);
    ...
    it("联想链式激活: 源记忆扩散到关联记忆", () => {
        ...
        const getLinks = (id) => linkMap[id] ?? [];
        const getEntry = (id) => entryMap.get(id);
        const hybridScores = new Map();
...
describe("Bench ⑤: MemoryStore 端到端混合管线", () => {
    it("write → BM25 索引 → read 混合检索 全链路", async () => {
        ...
        for (const t of SYNTHETIC_POOL.slice(0, 5)) {
            const writeInput = {
                source: { agentType: "Code", taskId: "bench" },
                kind: t.kind,
                summary: t.summary,
                semantic_gist: t.semantic_gist,
        ...
        // 按关键词查询
        const results = await store.read({ keywords: ["登录", "异常"], limit: 5 }, "CSA");
    ...
    it("memory link 后混合检索可触发联想激活路径", async () => {
        ...
        // 查询
        const results = await store.read({ keywords: ["登录", "安全"], limit: 5 }, "CSA");
        expect(results.length).toBeGreaterThan(0);
...
//# sourceMappingURL=bench-memory-pipeline.test.js.map