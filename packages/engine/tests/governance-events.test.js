...
describe("GovernanceEventEmitter", () => {
    ...
    describe("emitAuditReport() 审计报告", () => {
        it("应发射 governance.audit_report 事件", () => {
            ...
            expect(observer.emittedEvents).toHaveLength(1);
            const payload = observer.emittedEvents[0].payload;
    ...
    describe("事件通用属性", () => {
        ...
        it("notificationType 根据 requiresDecision 设置", () => {
            ...
            expect(observer.emittedEvents[0].notificationType).toBe("DECISION_REQUIRED");
            emitter.emitAmendmentProposed({
                id: "b",
                summary: "s",
            });
            expect(observer.emittedEvents[1].notificationType).toBe("FYI");
        });
    });
});
//# sourceMappingURL=governance-events.test.js.map