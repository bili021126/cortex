...
import { AgentType, setAgentRegistry } from "@cortex/shared";
import { LlmAdapter } from "@cortex/llm";
import { SkillRegistry, TaskBoard, AgentPool, Scheduler, PipelineObserver, ConfirmGate, MetaAgent, createAgent } from "@cortex/engine";
...
function fail(label, d) { console.log(`  ❌ ${label}`); if (d)
    console.log(`     ${d}`); }
function log(label, v) { console.log(`  📋 ${label}: ${v}`); }
function dbg(m) { if (VERBOSE)
    console.log(`  🔍 ${m}`); }
// ── 工具注册 ──
function registerTools(toolkit, projectRoot) {
    // 所有工具基础路径为 packages/engine，Agent 调用 list_files(".") 即是 engine 根目录
    const engineRoot = path.join(projectRoot, "packages", "engine");
    const resolve = (p) => path.resolve(engineRoot, p);
    toolkit.register("read_file", async (params) => {
        let fp = params.file_path;
    ...
    toolkit.register("write_file", async (params) => {
        ...
        const content = params.content_blob;
    ...
    toolkit.register("replace_in_file", async (params) => {
        let fp = params.file_path;
        if (!path.isAbsolute(fp))
            fp = resolve(fp);
        // 只允许写入 tests/ 目录
        ...
        if (!oldText || oldText === newText)
            return { success: false, error: "需要 old_text 且 old_text ≠ new_text" };
        const raw = fs.readFileSync(fp, "utf-8");
        if (replaceAll) {
    ...
    toolkit.register("run_shell", async (params) => {
        let cmd = (params.command ?? "").replace(/^cd\s+\S+\s*(&&|;)\s*/i, "").trim();
        if (!cmd)
            return { success: false, error: "缺少 command" };
    ...
    toolkit.register("check_e2e_compile", async () => {
        try {
            ...
            const all = ((e.stderr ?? "") + (e.stdout ?? ""));
            const e2eLines = all.split("\n").filter((l) => l.includes("tests/manual/e2e/") && l.includes("error TS"));
    ...
    toolkit.register("list_files", async (params) => {
        const dp = resolve((params.dir_path ?? "."));
        if (!fs.existsSync(dp))
            return { success: false, error: `Dir not found: ${dp} (当前在 packages/engine，试试 tests/manual/e2e 或 src)` };
    ...
    toolkit.register("search_code", async (params) => {
        ...
        try {
            ...
            const out = execSync(`npx rg --no-heading -n "${q.replace(/"/g, '\\"')}" packages/engine/tests/ packages/engine/src/`, {
                cwd: projectRoot, timeout: 15_000, encoding: "utf-8", maxBuffer: 256 * 1024
            }).slice(0, 4000);
...
// ── 技能 ──
const TEST_FIX_SKILL = {
    id: "skill-e2e-compilation-fix",
    name: "E2E测试文件编译修复",
    triggerTags: ["fix", "test", "typescript"],
...
async function main() {
    ...
    const fixConfig = { type: AgentType.Fix, systemPrompt: FIX_PROMPT, maxLoops: 15 };
    const fixAgent = createAgent(fixConfig, chatAdapter, toolkit);
    await fixAgent.wakeup();
    scheduler.register(AgentType.Fix, fixAgent, llmCfg.chatModel);
    pass("希格雯 (Fix) 就绪");
    const reviewConfig = { type: AgentType.Review, systemPrompt: REVIEW_PROMPT, maxLoops: 5 };
    const reviewAgent = createAgent(reviewConfig, chatAdapter, toolkit);
    await reviewAgent.wakeup();
    scheduler.register(AgentType.Review, reviewAgent, llmCfg.chatModel);
    pass("刻晴 (Review) 就绪");
    const analysisConfig = { type: AgentType.Analysis, systemPrompt: ANALYSIS_PROMPT, maxLoops: 6 };
    const analysisAgent = createAgent(analysisConfig, chatAdapter, toolkit);
    await analysisAgent.wakeup();
    scheduler.register(AgentType.Analysis, analysisAgent, llmCfg.chatModel);
    pass("纳西妲 (Analysis) 就绪");
    const governConfig = { type: AgentType.DocGovern, systemPrompt: GOVERN_PROMPT, maxLoops: 3 };
    const governAgent = createAgent(governConfig, chatAdapter, toolkit);
    await governAgent.wakeup();
    scheduler.register(AgentType.DocGovern, governAgent, llmCfg.chatModel);
    pass("凝光 (DocGovern) 就绪");
    const loopConfig = { type: AgentType.Loop, systemPrompt: LOOP_PROMPT, maxLoops: 3 };
    const loopAgent = createAgent(loopConfig, chatAdapter, toolkit);
    await loopAgent.wakeup();
    scheduler.register(AgentType.Loop, loopAgent, llmCfg.chatModel);
    pass("莫娜 (Loop) 就绪");
    const apiConfig = { type: AgentType.Api, systemPrompt: API_PROMPT, maxLoops: 3 };
    const apiAgent = createAgent(apiConfig, chatAdapter, toolkit);
    await apiAgent.wakeup();
    scheduler.register(AgentType.Api, apiAgent, llmCfg.chatModel);
    pass("久岐忍 (Api) 就绪");
    const dataConfig = { type: AgentType.Data, systemPrompt: DATA_PROMPT, maxLoops: 3 };
    const dataAgent = createAgent(dataConfig, chatAdapter, toolkit);
    await dataAgent.wakeup();
    scheduler.register(AgentType.Data, dataAgent, llmCfg.chatModel);
    pass("艾尔海森 (Data) 就绪");
    // 战略双柱（reasoner 模型）
    const strategyConfig = { type: AgentType.Strategist, systemPrompt: STRATEGY_PROMPT, maxLoops: 3 };
    const strategistAgent = createAgent(strategyConfig, reasonerAdapter, toolkit);
    ...
    const directionConfig = { type: AgentType.Strategist, systemPrompt: DIRECTION_PROMPT, maxLoops: 3 };
    const directionAgent = createAgent(directionConfig, reasonerAdapter, toolkit);
    await directionAgent.wakeup();
    ...
    if (plan.length === 0) {
        ...
        const fallbackNodes = [
            {
                id: "fb-analysis", type: "analysis", tags: ["analysis", "research"],
            ...
            {
                id: "fb-fix-imports", type: "fix", tags: ["fix", "typescript"],
                needsMultiPerspective: false, status: "pending", claimedBy: [], results: [], createdAt: Date.now(),
                payload: [
            ...
            {
                id: "fb-fix-remaining", type: "fix", tags: ["fix", "typescript"],
                needsMultiPerspective: false, status: "pending", claimedBy: [], results: [], createdAt: Date.now(),
                payload: [
            ...
            {
                id: "fb-review", type: "review", tags: ["review", "audit"],
                needsMultiPerspective: false, status: "pending", claimedBy: [], results: [], createdAt: Date.now(),
    ...
    const episodic = allMem.filter((m) => m.kind === "TaskLog");
    const conceptual = allMem.filter((m) => m.kind === "Insight");
    ...
    try {
        ...
        const toStr = (b) => (b instanceof Buffer ? b.toString("utf-8") : String(b ?? ""));
        const errText = toStr(e.stderr) + toStr(e.stdout);
        const allErrLines = errText.split("\n").filter((l) => l.includes("error TS"));
        const e2eErrLines = allErrLines.filter((l) => l.includes("tests/manual/e2e/"));
...
//# sourceMappingURL=multi-agent-collab-e2e.js.map