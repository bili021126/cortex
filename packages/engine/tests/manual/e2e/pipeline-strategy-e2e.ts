...
import { AgentType, PipelinePriority, type TaskNode, type Tag } from "@cortex/shared";
...
function makeNode(id: string, payload: string, tags: Tag[] = [], strategy?: "react" | "direct"): TaskNode {
  return {
    id,
    type: "code",
    payload,
...
async function main() {
  ...
  board.addNode(makeNode(
    "react-1",
    "Read packages/engine/package.json and tell me the package name. Reply in 1 sentence.",
    ["code"] as Tag[],
    "react",
  ));
  ...
  board.addNode(makeNode(
    "mix-react",
    "Read packages/engine/src/index.ts and list 3 exported symbols. Reply concisely.",
    ["code"] as Tag[],
    "react",
  ));
  board.addNode(makeNode(
  ...
  if (mix1Ok) passed("react 节点完成");
  else { failed("react 节点", nMix1.results[0]?.error ?? `status=${nMix1.status}`); allPassed = false; }
  if (mix2Ok) passed("direct 节点完成");
  else { failed("direct 节点", nMix2.results[0]?.error ?? `status=${nMix2.status}`); allPassed = false; }
  ...
  if (reactMems.length >= 1) passed(`react 策略写入记忆: ${reactMems.length} 条`);
  else { failed("react 策略未写入记忆"); allPassed = false; }

  if (directMems.length >= 1) passed(`direct 策略写入记忆: ${directMems.length} 条`);
  else { failed("direct 策略未写入记忆"); allPassed = false; }
