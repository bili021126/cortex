...
import { evaluateAmendment, applyAmendment, loadPendingProposals, updateProposalStatus, summarizeGovernance } from "@cortex/governance";
// ══════════════════════════════════════════════
// 0. 常量
// ══════════════════════════════════════════════
...
/** 检查宪法是否包含指定文本 */
function constitutionContains(text) {
...
// 4. 裁决执行阶段（开拓者裁决 → 写入宪法）
// ══════════════════════════════════════════════
function applyPhase(judged) {
    header("Phase 2/3 — 开拓者裁决 → 写入宪法");
    // 按版本号升序排列（v2.5.11 先于 v2.5.12）
    const sorted = [...judged].sort((a, b) => a.proposal.version.localeCompare(b.proposal.version));
    const results = [];
    let allOk = true;
...
//# sourceMappingURL=governance-amendment-e2e.js.map