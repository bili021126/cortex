/**
 * @covers: 全量真实 E2E — 宪法 v3.0 全链路端到端验证（14 个旧测试浓缩）
 *
...
import type { TaskNode, ObservableEvent } from "@cortex/shared";
...
interface AgentTask { name: string; type: string; tags: string[]; payload: string; }
...
async function main() {
  ...
  try { (toolkit as any).gate?.bypassAll?.(); log("🔓 ConfirmGate bypass"); } catch { /* 不可用 */ }

  // Core-2 组件检查
  log("✅ Core-2 管道:");
  for (const [name, val] of [["TaskRouter", engine.taskRouter], ["SentinelFilter", engine.sentinelFilter], ["NotificationRt", engine.notificationRuntime], ["DecisionBridge", engine.decisionBridge], ["GovernanceEmit", engine.governanceEmitter], ["EnvRouter", engine.envRouter]] as [string, unknown][]) {
    log(`   ${val ? "✅" : "⚠️"} ${name}`);
  }

  // 事件收集
  const events: string[] = [];
  engine.observer.on(PipelinePriority.HIGH, (e: ObservableEvent) => events.push(e.type as string));

  // 甘雨规划
  log("\n🧠 甘雨规划...");
  if (!engine.metaAgent) { log("❌ MetaAgent 未初始化"); await engine.shutdown(); return; }
  ...
  } catch (e) { log(`❌ 甘雨规划失败: ${e}`); await engine.shutdown(); return; }
  log(`   ${planNodes.length} 个节点`);
  for (const n of planNodes) log(`     - ${n.type} [${n.tags.join(",")}] ${n.payload.slice(0, 55)}...`);
  if (planNodes.length === 0) { log("⚠️ 甘雨拒绝规划"); await engine.shutdown(); return; }
  ...
  for (let i = 0; i < planNodes.length; i++) {
    ...
    if (report.results.length === 0) {
      const n = board.getNode(node.id) as any;
