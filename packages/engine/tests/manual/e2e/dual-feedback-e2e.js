...
import { MetaAgent, TaskBoard, AgentPool, Scheduler, PipelineObserver, ConfirmGate, createAgent, codeAgentConfig, createInspectorAgent, } from "@cortex/engine";
...
function registerReadOnlyTools(toolkit, workspaceRoot) {
    ...
    toolkit.register("list_files", async (params) => {
        const dirPath = resolve((params.dir_path ?? "."));
    ...
    toolkit.register("search_code", async (params) => {
        const dir = resolve((params.dir_path ?? "."));
        const pattern = (params.pattern ?? "");
        if (!pattern)
            return { success: false, error: "search_code: 缺少 pattern" };
        try {
            const results = [];
            const walk = (d, depth) => {
                if (depth > 2)
...
class EventCollector {
    ...
    collect(observer) {
        ...
        observer.on(PipelinePriority.HIGH, (e) => {
            this.events.push({ type: e.type, priority: e.priority, payload: (e.payload ?? {}), timestamp: e.timestamp });
        });
...
async function main() {
    ...
    if (fs.existsSync(MEMORY_DB)) {
        ...
        try {
            fs.unlinkSync(MEMORY_DB + "-wal");
        }
        catch { /* 可能不存在 */ }
        try {
            fs.unlinkSync(MEMORY_DB + "-shm");
        }
        catch { /* 可能不存在 */ }
    }
    ...
    // ── 断言块 ──
    console.log("\n── 断言验证 ──");
    assert(typeof report.sessionId === "string" && report.sessionId.length > 0, `ExecutionReport.sessionId 非空: "${report.sessionId}"`);
    assert(report.sessionId.startsWith("run-"), `sessionId 以 "run-" 开头: ${report.sessionId}`);
    // sessionId 锚定：report ← → memory
    const memSessionId = memory.sessionId;
    // 注意：endSession() 在 executeAll 末尾已调用，sessionId 可能已被清除
    assert(memSessionId === undefined, `endSession() 后 memory.sessionId 已清除: ${memSessionId}`);
    ...
    assert(runMemories.length > 0, `getBySession() 可查询到本次 run 的记忆（${runMemories.length} 条）`);
    if (runMemories.length > 0) {
        for (const m of runMemories.slice(0, 3)) {
            console.log(`     [${m.kind}] ${m.summary.slice(0, 60)}... sessionId=${m.sessionId}`);
        }
        assert(runMemories.every((m) => m.sessionId === report.sessionId), "所有 run 记忆的 sessionId 与 ExecutionReport.sessionId 一致");
    }
    ...
    assert(nodeCompletes.length > 0, `至少 1 个 NodeComplete 事件被发射（MetaAgent 在 HIGH 优先级订阅）`);
    const schedulerDones = eventCollector.schedulerDones;
    assert(schedulerDones.length === 1, `恰好 1 个 SchedulerDone 事件: ${schedulerDones.length}`);
    // ═══════════════════════════════════════════════════
    ...
    // 断言块
    console.log("\n── 断言验证 ──");
    assert(typeof report2.sessionId === "string" && report2.sessionId.length > 0, `第二轮 sessionId 非空: "${report2.sessionId}"`);
    assert(report2.sessionId !== report.sessionId, `两轮 sessionId 不同（隔离）: "${report.sessionId}" vs "${report2.sessionId}"`);
    ...
    assert(round2Completes.length > 0, `第二轮至少 1 个 NodeComplete（MetaAgent HIGH 订阅验证）`);
    // 第二轮记忆隔离
    const round2Memories = memory.getBySession(report2.sessionId);
    console.log(`   第二轮 getBySession → ${round2Memories.length} 条记忆`);
    assert(round2Memories.every((m) => m.sessionId === report2.sessionId), "第二轮记忆的 sessionId 与本轮 report 一致");
    // 验证两轮记忆不混淆
    ...
    assert(!crossContamination || round1MemoriesCheck.length === 0 || round2Memories.length === 0, `两轮记忆无交叉污染（或某轮未产生记忆）`);
    // ═══════════════════════════════════════════════════
    // 结果汇总
    // ═══════════════════════════════════════════════════
    console.log("\n╔══════════════════════════════════════════════════╗");
...
//# sourceMappingURL=dual-feedback-e2e.js.map