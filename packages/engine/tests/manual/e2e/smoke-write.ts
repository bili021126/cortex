...
async function main() {
  ...
  for (const n of nodes) log(`     ${n.type} [${n.tags.join(",")}] ${n.payload.slice(0,60)}`);
  ...
  for (const r of rpt.results) log(`   ${r.success ? "✅" : "❌"} ${r.agentType}: ${(r.output ?? r.error ?? "").slice(0, 150)}`);

  // 验证
  const fp = path.join(root, "packages", "engine", "src", "core", "smoke-test.ts");
  const exists = fs.existsSync(fp);
  log(`\n📁 ${fp}: ${exists ? "✅ EXISTS" : "❌ NOT FOUND"}`);
  if (exists) log(`   📝 ${fs.readFileSync(fp, "utf-8").slice(0, 100)}`);
