...
import { TaskBoard, AgentPool, Scheduler, PipelineObserver, ConfirmGate, MetaAgent, createAgent, codeAgentConfig, reviewAgentConfig, analysisAgentConfig, opsAgentConfig, loopAgentConfig, docGovernAgentConfig, apiAgentConfig, dataAgentConfig, fixAgentConfig, createInspectorAgent, ButlerAgent } from "@cortex/engine";
...
function registerProjectTools(toolkit, projectRoot) {
    const resolve = (p) => {
...
function collectNewProposals(knownIds) {
...
async function main() {
    ...
    observer.on(PipelinePriority.HIGH, (e) => {
        const p = e.payload;
        const id = p?.nodeId ? `[${p.nodeId.slice(0, 20)}]` : "";
    ...
    if (newProposals.length === 0) {
        ...
        console.log("  执行结果摘要：");
        const docNodes = allNodes.filter(n => n.tags?.some((t) => t.includes("constitution") || t.includes("doc")));
...
//# sourceMappingURL=governance-full-agent-e2e.js.map