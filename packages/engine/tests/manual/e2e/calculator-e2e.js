...
import { AgentType, LinkType, PipelinePriority, setAgentToolPermissions } from "@cortex/shared";
import { LlmAdapter } from "@cortex/llm";
import { TaskBoard, AgentPool, createAgent, codeAgentConfig, reviewAgentConfig, createInspectorAgent, Scheduler, PipelineObserver, ConfirmGate, MetaAgent } from "@cortex/engine";
...
// ═══════════════════════════════════════════════
function registerCalculatorTools(toolkit, projectRoot, workspaceRoot) {
    const resolve = (p) => {
        // 绝对路径：直接使用
...
// 3. 种子记忆
// ═══════════════════════════════════════════════
async function seedMemories(memory, agentType) {
    // 先查后写，防止重复播种导致记忆膨胀
...
async function main() {
    ...
    // 确保 tsconfig.json 存在，Inspector 用 tsc --noEmit 需要
    ...
    console.log("   ✅ 3 位专家就绪 (阿贝多/安柏/刻晴)\n");
    // ── 手动任务图（Scheduler 按入板顺序执行）──
    console.log("🟢 [Phase 3] 构建任务图 (5 节点，顺序执行)...");
    const taskContext = [
        "你工作在 projects/calculator/ 下。所有文件读写限定在此目录。",
        "",
        "计算器系统：实现 Calculator 类，接收字符串表达式(如 \"2+3*4\")，支持+-*/、括号、优先级。",
        "除以零返回 NaN，非法字符抛 Error。",
    ].join("\n");
    const now = Date.now();
    // Scheduler 用 parentId 做拓扑分层——严格串行
    const nodes = [
        {
    ...
    for (const n of nodes)
        board.addNode(n);
    console.log(`   ✅ ${nodes.length} 个任务节点入板 (parentId 链式拓扑，逐层串行)`);
    for (const n of nodes) {
    ...
    // ── 执行 ──
    console.log("🟢 [Phase 4] Scheduler 执行 —— 三位专家开始协作...\n");
    console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
    ...
    console.log();
    // ── 验证计算器是否真的能运行 ──
    console.log("── 验证：用 npx tsx 运行计算器测试 ──");
    try {
        const { execSync } = await import("node:child_process");
        ...
        const stdout = e.stdout?.toString() ?? "";
        const stderr = e.stderr?.toString() ?? "";
        console.log(`   ❌ 计算器测试失败 (exit ${e.status ?? "?"})`);
        if (stdout)
            console.log(`   stdout:\n${stdout.slice(0, 400)}`);
        if (stderr)
            console.log(`   stderr:\n${stderr.slice(0, 400)}`);
    ...
    console.log("║  🎭 专家灵魂观察者                              ║");
    console.log("╚══════════════════════════════════════════════════╝\n");
    console.log("   1. 阿贝多（CodeAgent）在 think 阶段有没有引用");
    console.log("      「施工教训」「忘了 mock」「依赖路径」？");
    console.log("   2. 安柏（InspectorAgent）的勘察报告里编译/测试");
    console.log("      结果是真实的 tsc/vitest 输出，还是 LLM 编的？");
...
main().catch((err) => {
    console.error("❌ 计算器 E2E 失败:", err);
    process.exit(1);
});
//# sourceMappingURL=calculator-e2e.js.map