...
import { SkillRegistry, deriveStatus, registerSkillPipeline, TaskBoard, AgentPool, Scheduler, PipelineObserver, ConfirmGate, createAgent, } from "@cortex/engine";
...
function createTempProject() {
    ...
    fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "skill-e2e-test", private: true }, null, 2), "utf-8");
    fs.writeFileSync(path.join(src, "index.ts"), `export function add(a: number, b: number): number { return a + b; }\n`, "utf-8");
...
function registerTools(toolkit, projectRoot) {
    ...
    toolkit.register("run_shell", async (params) => {
        ...
        if (!cmd)
            return { success: false, error: "run_shell: 缺少 command" };
 ...
 */
function buildSkillContext(registry, tags) {
    const matched = registry.queryByTags(tags);
    if (matched.length === 0)
        return "（无匹配经验）";
    return matched
        .map((s, i) => `[经验 ${i + 1}] ${s.name} (weight=${s.weight}, status=${deriveStatus(s.weight, s.feedbackHistory)})\n` +
        `  触发: ${s.trigger}\n` +
...
// 6. 节点构建
// ══════════════════════════════════════════════
function makeFixNode(id, projectRoot, strategy) {
    return {
        id,
        type: "fix",
        tags: ["fix", "config"],
...
function createEventTracker(observer) {
    const events = {
        nodeStart: 0,
        nodeComplete: 0,
        nodeFailed: 0,
...
async function main() {
    ...
    try {
        ...
        pool1.register({ type: "fix", maxInstances: 1 });
        ...
        const recorded = skillRegistry.recordFeedback(CI_FIX_SKILL.id, "FixAgent", feedbackRating, feedbackSuggestion);
        ...
        pool2.register({ type: "code", maxInstances: 1 });
        ...
        evt.all.forEach((type, i) => {
            if (type === PipelineEventType.NodeStart)
                startIndices.push(i);
            if (type === PipelineEventType.NodeComplete)
                completeIndices.push(i);
        });
        if (startIndices.length > 0 && completeIndices.length > 0) {
            const allStartBeforeComplete = startIndices.every((si) => completeIndices.some((ci) => ci > si));
        ...
        const fixMems = allMem.filter((m) => m.kind === "TaskLog");
...
//# sourceMappingURL=skill-fix-e2e.js.map