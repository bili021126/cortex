...
function verify(pkgDir) {
    const r = { barrel: false, pkgJson: false, tests: 0, ciTags: 0, positioning: false, srcFiles: 0, compile: false, test: false };
    if (fs.existsSync(path.join(pkgDir, "src/index.ts")))
        r.barrel = true;
    if (fs.existsSync(path.join(pkgDir, "package.json")))
        r.pkgJson = true;
    if (fs.existsSync(path.join(pkgDir, "PACKAGE_POSITIONING.md")))
        r.positioning = true;
    // 统计实际 TypeScript 源文件
    const srcDir = path.join(pkgDir, "src");
    if (fs.existsSync(srcDir)) {
        try {
            r.srcFiles = fs.readdirSync(srcDir).filter((f) => f.endsWith(".ts") && f !== "index.ts").length;
        }
        catch { }
    }
    ...
    if (fs.existsSync(td)) {
        ...
        r.tests = tfs.length;
        for (const f of tfs) {
            if (fs.readFileSync(path.join(td, f), "utf-8").slice(0, 25).includes("@ci"))
                r.ciTags++;
        }
    }
    if (r.pkgJson) {
...
async function main() {
    ...
    setAgentToolPermissions(Object.fromEntries(["code", "review", "analysis", "ops", "loop", "doc-govern", "api", "data", "fix", "inspector"].map(t => [t, TOOLS])));
    try {
        toolkit.gate?.bypassAll?.();
        log("  ✅ ConfirmGate 已绕过");
    }
    catch {
        log("  ⚠️ ConfirmGate bypass 失败");
    }
    ...
    if (engine.memory) {
        try {
            const stats = engine.memory.stats?.() ?? {};
            const all = engine.memory.getAllEntries?.() ?? [];
            const pending = engine.memory.getPendingEntries?.() ?? [];
    ...
    if (engine.skillRegistry) {
        const skills = engine.skillRegistry.getAll?.() ?? [];
        log(`  技能: ${engine.skillRegistry.activeCount ?? skills.length} 活跃 | ${skills.length} 总数`);
    }
    // Agent 注册清单
    log("  ── Agent 注册 ──");
    if (engine.agents) {
        for (const [type, agent] of engine.agents) {
            const max = engine.pool?.configs?.get?.(type)?.maxInstances ?? "?";
    ...
    for (const augType of ["code", "review", "analysis", "ops", "inspector", "doc-govern", "loop", "api", "data"]) {
        try {
            engine.pool?.setMaxInstances?.(augType, 6);
        }
        catch { }
    }
    log("  ✅ Agent 并发上限 → 6");
    ...
    log("  ── 工具注册 ──");
    try {
        const defs = toolkit.listDefinitions?.("code") ?? [];
        const names = defs.map((d) => d.name ?? d);
    ...
    engine.observer.on(PipelinePriority.HIGH, (e) => {
        const t = e.type ?? "";
        events.push(t);
    ...
    const heartbeat = setInterval(() => {
        ...
        const claimed = allNodes.filter((n) => n.status === "claimed").length;
        const running = allNodes.filter((n) => n.status === "running" || n.status === "in_progress").length;
        ...
        for (const n of allNodes) {
            if (n.status === "claimed") {
                if (!claimedAt.has(n.id))
                    claimedAt.set(n.id, now);
                if (now - (claimedAt.get(n.id) ?? now) > 90_000) {
                    log(`[${new Date().toISOString().slice(11, 19)}] ⛔ ${n.id} 认领超时 强制释放`);
                    try {
                        engine.board.release(n.id, (Array.isArray(n.claimedBy) ? n.claimedBy[0] : n.claimedBy) ?? "");
                    }
                    catch { }
                    try {
                        engine.board.failNode(n.id);
                    }
                    catch { }
    ...
    let webCtx = "";
    try {
        const r = await toolkit.execute?.({ toolName: "web_search", params: { query: "TypeScript monorepo useful packages 2025", max_results: 3 } }, "analysis");
    ...
    // 从规划输出中提取甘雨的调度建议
    const planText = nodes.map(n => n.payload ?? "").join(" ");
    ...
    for (const n of nodes) {
        ...
        const depStr = deps?.length ? `  依赖:[${deps.map((d) => d.slice(-12)).join(",")}]` : "";
        log(`    [${n.type.padEnd(14)}] ${n.id.slice(-24)}${parent}${depStr}`);
        log(`       ${n.payload?.slice(0, 100)}`);
    }
    if (!nodes.length) {
        log("⚠️ 空计划");
        await engine.shutdown();
    ...
    // ── S8 全维度汇总 ────────────────────────
    const evt = (s) => events.filter(e => e.includes(s)).length;
    ...
    log(`  新包: ${newPkg ?? "(无)"}`);
    if (vrfy)
        log(`  结构: ${vrfy.barrel && vrfy.pkgJson && vrfy.tests > 0 && vrfy.ciTags > 0 && vrfy.positioning ? "✅" : "❌"}`);
    if (vrfy)
        log(`  编译: ${vrfy.compile ? "✅" : "❌"}  测试: ${vrfy.test ? "✅" : "❌"}`);
...
//# sourceMappingURL=solo-flight.js.map