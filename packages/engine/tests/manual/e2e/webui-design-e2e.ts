...
import { AgentType, PipelinePriority, type TaskNode, type LlmAdapterConfig } from "@cortex/shared";
...
const PERSONA: Record<string, { emoji: string; name: string }> = {
  "code":    { emoji: "🧪", name: "阿贝多" },
...
function p(agentType: string, msg: string): string {
...
function registerWebuiTools(toolkit: Toolkit, projectRoot: string) {
  ...
  toolkit.registerTool(new LocalTool(
    ...
    ReversibilityLevel.L0,
    async (params: Record<string, unknown>) => {
      const fp = resolve((params.file_path ?? params.path) as string);
      if (!fp) return { success: false, error: "read_file: 缺少 file_path 参数" };
  ...
  toolkit.registerTool(new LocalTool(
    ...
    ReversibilityLevel.L2,
    async (params: Record<string, unknown>) => {
      const fp = resolve((params.file_path ?? params.path) as string);
      const content = (params.content ?? params.content_blob ?? params.data ?? "") as string;
      if (!fp) return { success: false, error: "write_file: 缺少 file_path 参数" };
  ...
  const listHandler = async (params: Record<string, unknown>) => {
...
async function main() {
  ...
  const llmAdapterConfig: LlmAdapterConfig = {
    apiKey: API_KEY,
    baseUrl: BASE_URL,
...
只改有问题的地方，不要重新设计。`;
  ...
  if (planNodes.length === 0) {
    ...
    const codeOnly = planNodes.filter(
      n => n.type === "code" || n.tags?.includes("implementation")
    );
