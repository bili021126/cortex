/**
 * WebUI 计算器系统 —— MetaAgent 自规划 + 宵宫验证 E2E
 *
 * 用法: npx tsx tests/manual/webui-calculator-e2e.ts
 * 前提: 项目根目录 .env 已配置 DEEPSEEK_API_KEY
 *
 * 场景：甘雨（MetaAgent）接收用户意图，自规划任务树 →阿贝多写前端 →
 *       宵宫（BrowserAgent）用真实浏览器验证 →安柏复查 →刻晴审查
 *
 * 与 calculator-e2e.ts 的核心差异：
 *   - 不用硬编码节点，由 MetaAgent.plan() 自己拆解意图
 *   - BrowserAgent（宵宫）用 Playwright 操作真实浏览器
 *   - 验证 WebUI 而非 CLI 计算器
 */
...
function registerCalculatorTools(toolkit: Toolkit, projectRoot: string) {
  const resolve = (p: string) => {
    // 剥离 projects/calculator/ 前缀——projectRoot 已经包含它
...
async function main() {
  ...
  console.log("╔══════════════════════════════════════════════════╗");
  console.log("║  🌐 WebUI 计算器 —— MetaAgent 自规划 + 宵宫验证       ║");
  console.log("╚══════════════════════════════════════════════════╝\n");
  ...
  // CodeAgent（阿贝多）—— 炼金术士，写代码、修 bug、重构
  ...
  console.log("   ✅ 凝光 (DocGovern) —— 天权星");

  // LoopAgent（莫娜）—— 占星术士，模式提炼
  ...
  // ── MetaAgent 自规划 ──
  console.log("🟢 [Phase 3] 甘雨（MetaAgent）理解意图、自规划任务树...\n");

  const intent = [
    "从零实现一个 WebUI 计算器系统。代码全部放在 projects/calculator/webui/ 目录下。",
    "不要写任何 test.js 或测试文件——只需要 calculator.js 和 index.html 两个文件。",
    "",
    "⚠️ 关键约束：",
    "1. 所有文档必须用 write_file 工具输出到磁盘——不能只在脑子里分析，必须写出文件。",
    "2. HTML 页面元素的 ID 必须使用约定名称：输入框 #expression、按钮 #calculateBtn、结果区 #result。",
    "   不要用其他 ID，不要自行发挥。",
    "",
    "功能要求：",
    "- calculator.js —— 纯浏览器端 JavaScript，不用 Node.js API（不要用 require/fs/path）。",
    "  evaluate(str) 解析 +,-,*,/ 和括号表达式，遵循标准运算符优先级。",
    "  除以零返回 'NaN'，非法字符和语法错误抛出明确的 Error。",
    "- index.html —— 完整的用户界面。使用约定 ID：expression（输入框）、calculateBtn（按钮）、result（结果区）。",
    "  通过 <script src=\"calculator.js\"> 加载逻辑。必须能通过 file:// 协议直接打开。",
    "",
    "需要由阿贝多先用 write_file 写出这两个文件，然后安柏去侦察文件是否齐全、内容是否正确，",
    "接下来宵宫用浏览器验证三组表达式：2+3*4=14, (10-2)/4=2, 1/0=NaN），",
    "刻晴审查代码质量，纳西妲做架构分析并用 write_file 输出 architecture.md，",
    "凝光做合规审计并用 write_file 输出 audit-report.md，莫娜提炼模式，北斗检查部署就绪性。",
    "",
    "还要用 write_file 输出以下文档，每份独立成文件，放在 webui/ 目录下：",
  ...
  if (nodes.length === 0) {
    console.error("   ❌ MetaAgent 未生成任何任务节点");
    process.exit(1);
  }
  ...
  // ── 执行 ──
  console.log("🟢 [Phase 4] Scheduler 执行 —— 八位专家开始协作...\n");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n");
  ...
  for (const rel of checkFiles) {
    ...
    console.log(`   ${exists ? "✅" : "❌"} ${rel}  ${exists ? `(${size} bytes)` : "(未生成)"}`);
  }
  console.log();
  ...
  const docSpecs: Array<{ file: string; label: string; checks: Array<{ name: string; pattern: RegExp }> }> = [
    ...
    },
    {
      ...
      checks: [
        ...
        { name: "评估/建议", pattern: /评估|建议|优化|改进|可扩展/i },
      ],
    },
  ];
  ...
  // ── 宵宫侦察 ──
  console.log("── 宵宫（BrowserAgent）执行摘要 ──");
...
main().catch((err) => {
  console.error("❌ WebUI 计算器 E2E 失败:", err);
  // 即使失败也尝试清理
  process.exit(1);
});
