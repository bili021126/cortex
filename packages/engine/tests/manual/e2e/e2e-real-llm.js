...
import { MetaAgent, TaskBoard, AgentPool, createAgent, codeAgentConfig, reviewAgentConfig, analysisAgentConfig, docGovernAgentConfig, createInspectorAgent, opsAgentConfig, loopAgentConfig, Scheduler, PipelineObserver, ConfirmGate } from "@cortex/engine";
...
const REAL_TOOLS = {
    registerReal(toolkit, workspaceRoot) {
...
async function main() {
    ...
    const adapter = new LlmAdapter({
        ...
        reasoningEffort: llmCfg.reasoningEffort, // MetaAgent 按标签智能分配 max，其他 high 提速 2-3x
    });
    ...
    observer.on(PipelinePriority.HIGH, (e) => {
        console.log(`   📡 ${e.type}: ${JSON.stringify(e.payload.nodeId ?? e.payload).slice(0, 100)}`);
    });
    // ── 3e. 执行 ──
...
//# sourceMappingURL=e2e-real-llm.js.map