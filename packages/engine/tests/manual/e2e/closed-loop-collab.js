...
import { TaskBoard, AgentPool, Scheduler, PipelineObserver, ConfirmGate, MetaAgent, createAgent, codeAgentConfig, reviewAgentConfig, analysisAgentConfig, opsAgentConfig, loopAgentConfig, docGovernAgentConfig, apiAgentConfig, dataAgentConfig, fixAgentConfig, createInspectorAgent } from "@cortex/engine";
...
async function main() {
    ...
    const producedFiles = [];
    const walkProduced = (d) => {
        for (const entry of fs.readdirSync(d, { withFileTypes: true })) {
            if (entry.name === "node_modules" || entry.name === "dist" || entry.name === ".cortex")
                continue;
            const full = path.join(d, entry.name);
...
//# sourceMappingURL=closed-loop-collab.js.map