...
// 报告解析
// ═══════════════════════════════════════════════
function parseAgentReport(filePath, agentKey) {
    ...
    for (const line of lines) {
        ...
        overviewLines++;
        if (overview.length > 500 || overviewLines > 5)
            break;
    }
    // 提取关键判定行（包含 ✅ ❌ ⚠️ 的非表格行）
    const verdicts = [];
...
function parseFixList(filePath) {
    const result = { p0: [], p1: [], p2: [], p3: [], closed: [] };
    if (!fs.existsSync(filePath))
        return result;
    const content = fs.readFileSync(filePath, "utf-8");
    ...
    for (const section of sections) {
        ...
        if (section.startsWith("### P0"))
            result.p0 = items;
        else if (section.startsWith("### P1"))
            result.p1 = items;
        else if (section.startsWith("### P2"))
            result.p2 = items;
        else if (section.startsWith("### P3"))
            result.p3 = items;
 ...
 */
export function loadAuditContext(reportDir) {
    ...
    if (fs.existsSync(summaryPath)) {
        ...
        const cMatch = text.match(/完成[:：]\s*(\d+)/);
        if (cMatch)
            summary.completed = parseInt(cMatch[1]);
        const fMatch = text.match(/失败[:：]\s*(\d+)/);
        if (fMatch)
            summary.failed = parseInt(fMatch[1]);
        const dMatch = text.match(/耗时[:：]\s*([\d.]+s)/);
        if (dMatch)
            summary.duration = dMatch[1];
        const rMatch = text.match(/产出报告[:：]\s*(\d+)/);
        if (rMatch)
            summary.totalReports = parseInt(rMatch[1]);
        const pMatch = text.match(/通过\/闭合[:：]\s*(\d+)/);
        if (pMatch)
            summary.passCount = parseInt(pMatch[1]);
        const xMatch = text.match(/未完成[:：]\s*(\d+)/);
        if (xMatch)
            summary.failCount = parseInt(xMatch[1]);
        const wMatch = text.match(/黄灯\/残留[:：]\s*(\d+)/);
        if (wMatch)
            summary.warnCount = parseInt(wMatch[1]);
    }
 ...
 */
export function buildAuditContextForAgent(ctx, agentKey) {
    const parts = [];
    ...
    if (ownReports.length > 0) {
        ...
        for (const r of ownReports) {
            ...
            if (r.overview)
                parts.push(`   ${r.overview.slice(0, 300)}`);
 ...
 */
export function injectAuditContext(baseSystemPrompt, agentKey, ctx) {
 ...
 */
export function enhancePersonasWithAudit(basePrompts, ctx) {
 ...
 */
export function printAuditSummary(ctx) {
...
//# sourceMappingURL=audit-loader.js.map