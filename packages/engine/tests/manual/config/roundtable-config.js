...
export function getPersonaPrompts() {
    const result = {};
...
function buildPersonas(prompts) {
...
// ═══════════════════════════════════════════════
export const CODE_REVIEW_ROUNDTABLE = {
...
// ═══════════════════════════════════════════════
export const SOFT_CONSENSUS_ROUNDTABLE = {
...
//   - 讨论自然收束后，凝光输出「归因共识纪要」
// ═══════════════════════════════════════════════
export const ATTRIBUTION_ROUNDTABLE = {
 ...
 */
function extractConsensusItems(consensusText) {
    const items = [];
...
function validateConsensus(filePath) {
    if (!fs.existsSync(filePath))
        return;
    ...
    for (const item of CONFIRMED_CLOSED) {
        if (!p0p1Section.includes(item.keyword))
            continue;
        // 如果 excludeIf 也出现在同一区域，说明是引用而非违规（如 "node.failed" 出现在 "observer" 上下文中）
        if (item.excludeIf && p0p1Section.includes(item.excludeIf))
            continue;
...
export async function runMeeting(config, adapter, chatModel, dbDir, consensusOutputPath, seedMemories) {
...
//# sourceMappingURL=roundtable-config.js.map