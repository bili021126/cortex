...
// 审查逻辑
// ═══════════════════════════════════════════════
function auditPackageCount(packages) {
...
function auditNewPackages(packages) {
    const newPkgs = ["fsm-compiler", "plugin-runner", "policy-validator", "telemetry"];
    return newPkgs.map(name => {
        if (packages.has(name)) {
            return { id: `PKG-NEW-${name}`, severity: "PASS", category: "新包存在性", claim: `@cortex/${name} 应存在`, actual: "已存在" };
        }
        return { id: `PKG-NEW-${name}`, severity: "FAIL", category: "新包存在性", claim: `@cortex/${name} 应存在`, actual: "不存在" };
    });
}
function auditEnginePurity(root) {
...
function auditSplitPackages(packages) {
    ...
    return splitPkgs.map(name => {
        ...
        const tsFiles = hasSrc
            ? fs.readdirSync(path.join(pkgDir, "src"), { recursive: true }).filter(f => f.endsWith(".ts")).length
            : 0;
...
function auditDependencyAlignment(packages) {
    const findings = [];
    for (const [pkgName, expectedDeps] of Object.entries(CONSTITUTION_DEPS)) {
        ...
        if (missing.length === 0 && extra.length === 0) {
            ...
            const detail = [];
            if (missing.length > 0)
                detail.push(`缺少: ${missing.join(", ")}`);
            if (extra.length > 0)
                detail.push(`多余: ${extra.join(", ")}`);
            findings.push({
                id: `DEP-${pkgName.toUpperCase()}`,
...
function auditOldReplRemoval(root) {
...
function auditConfigStructure(root) {
...
function auditCompileCheck(root) {
    try {
...
function auditNewTuiExists(root) {
    ...
    const tsFiles = fs.readdirSync(tuiDir, { recursive: true }).filter(f => f.endsWith(".ts")).length;
...
async function main() {
    ...
    console.log("=".repeat(60));
    // 按严重度分组输出
    for (const sev of ["FAIL", "WARN", "PASS"]) {
...
//# sourceMappingURL=phase-assessment-audit.js.map