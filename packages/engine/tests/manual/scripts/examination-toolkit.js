...
export function registerExaminationTools(toolkit, rootDir, outputDir, softMode = false) {
    const resolve = (p) => {
    ...
    toolkit.register("read_file", async (params) => {
        ...
        if (fs.statSync(fp).isDirectory())
            return { success: false, error: `Path is a directory: ${fp}` };
        try {
    ...
    toolkit.register("list_dir", async (params) => {
        ...
        if (!fs.existsSync(fp))
            return { success: false, error: `Directory not found: ${fp}` };
        try {
    ...
    toolkit.register("search_code", async (params) => {
        ...
        const dirParam = params.directory ?? rootDir;
        ...
        try {
            ...
            const walk = (d, depth) => {
                ...
                for (const e of entries) {
                    ...
                    if (!/\.(ts|tsx|js|jsx|json|md|html|css)$/.test(e.name))
                        continue;
                    try {
    ...
    toolkit.registerTool(new LocalTool("write_file", ToolCategory.Write, "Write content to a file. Only paths under the examination output directory are permitted.", {
        type: "object",
        properties: {
        ...
        if (!normalizedFp.startsWith(normalizedOut + path.sep) && normalizedFp !== normalizedOut) {
            return {
                success: false,
                error: `写入被拒绝：审视实验中，所有写入操作仅限于 ${outputDir}/ 目录。\n` +
...
// ═══════════════════════════════════════════════
function registerSoftModeTools(toolkit, rootDir, resolve, maxOutputChars) {
    ...
    const UNIX_TO_WIN = {
        ...
        // 计数/统计
        "wc -l": (args) => `powershell -NoProfile -Command "(Get-Content ${args.trim()}).Count"`,
        "wc ": (args) => `powershell -NoProfile -Command "(Get-Content ${args.replace(/-[lwc]/g, '').trim()}).Count"`,
        // 文本处理
        sed: (args) => `powershell -NoProfile -Command "(Get-Content ${args.split(/\s+/).slice(1).join(' ').replace(/['\"]/g, '')}) -replace 'x', 'y'"`,
        // shell 判断
        "which ": (args) => `where ${args.trim()}`,
        // 文件查找
        "find ": (args) => {
            const pattern = args.match(/-name\s+["']?([^"'\s]+)["']?/)?.[1];
            const dir = args.split(/\s+/)[0] ?? ".";
            if (pattern)
                return `powershell -NoProfile -Command "Get-ChildItem -Path '${dir}' -Recurse -Filter '${pattern}' | Select-Object -First 50 FullName"`;
            return `powershell -NoProfile -Command "Get-ChildItem -Path '${dir}' -Recurse | Select-Object -First 50 FullName"`;
        }
    };
    /** 翻译单个命令段（不含管道）。大小写不敏感匹配。 */
    function adaptSegment(cmd, pipeIn) {
        if (!cmd)
            return cmd;
    ...
    toolkit.register("delete_file", async (params) => {
        const fp = resolve(params.file_path);
        if (!fs.existsSync(fp))
            return { success: false, error: `文件不存在: ${fp}` };
        try {
            fs.unlinkSync(fp);
...
//# sourceMappingURL=examination-toolkit.js.map