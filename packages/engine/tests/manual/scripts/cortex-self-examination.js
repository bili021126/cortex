...
import { AgentType, LinkType, PipelinePriority, PipelineEventType } from "@cortex/shared";
import { LlmAdapter } from "@cortex/llm";
import { TaskBoard, AgentPool, PipelineObserver, ConfirmGate } from "@cortex/scheduler";
import { createAgent, codeAgentConfig, reviewAgentConfig, createInspectorAgent, createBrowserAgent, analysisAgentConfig, docGovernAgentConfig, loopAgentConfig, opsAgentConfig, apiAgentConfig, dataAgentConfig, Scheduler, ButlerAgent, MetaAgent, StrategistAgent } from "@cortex/engine";
import { Toolkit, compressForRoundtable } from "@cortex/platform";
import { MemoryStore } from "@cortex/memory-store";
import { runMeeting, SOFT_CONSENSUS_ROUNDTABLE } from "../config/roundtable-config";
import { runCrossVerification, loadCrossVerifyPairs } from "./cross-verification";
...
// 2. 种子记忆——帮委员会快速了解项目
// ═══════════════════════════════════════════════
async function seedExaminationMemory(memory) {
    ...
    for (const entry of cortexConfig.seedMemories.entries) {
        const memId = await memory.write({
            source: { agentType: AgentType[entry.agentType], taskId: entry.taskId },
            kind: (entry.memoryType === "Conceptual" ? "Insight" : "TaskLog"),
            content_blob: entry.content,
            semantic_gist: entry.summary,
            content_hash: entry.taskId,
            summary: entry.summary
        });
        if (prevId && entry.linkTo) {
 ...
 */
async function seedPreviousReports(memory, outputDir, reportMaxChars) {
    ...
    for (const [key, { agentType, label }] of Object.entries(QUALITY_AGENTS)) {
        ...
        if (prevInjected.length > 0)
            continue;
        const files = fs.readdirSync(outputDir);
        const reportFile = files.find((f) => f.startsWith(key) && f.endsWith(".md") && f !== "self-examination-summary.md");
        if (!reportFile)
            continue;
...
function extractReportMeta(filePath) {
    try {
        ...
        for (const line of lines) {
            if (/✅|\[x\]|通过|已闭合|已修复|已完成/.test(line))
                passCount++;
            if (/❌|\s未完成|未修复|未开始/.test(line))
                failCount++;
            if (/⚠|⚠️|黄灯|部分|残留/.test(line))
                warningCount++;
        }
        return {
...
function generateExaminationSummary(outputDir, report, execDuration, fixListPath, isSoft = false) {
    ...
    if (fs.existsSync(outputDir)) {
        ...
        for (const f of files) {
            if (!f.endsWith(".md"))
                continue;
            if (f === "self-examination-summary.md")
                continue;
    ...
    for (const key of agentKeys) {
        const agent = cortexConfig.agents[key];
        if (agent?.display)
            agentDisplay[key] = { emoji: agent.display.emoji, label: agent.display.shortName };
    }
    for (const key of agentKeys) {
...
function writeExaminationSummary(outputDir, report, execDuration, fixListPath, summaryPath, isSoft = false) {
...
function agentName(type) {
    ...
    for (const [key, agent] of Object.entries(cortexConfig.agents)) {
        if (agent.type === type)
            return `${agent.display?.shortName ?? key} (${key.charAt(0).toUpperCase() + key.slice(1)})`;
    }
    return type;
}
async function main() {
    ...
    const taskBody = templatesLoaded
        ? templatesData.templates.flatMap((t, i) => buildTaskLines(t, i))
        : SOFT_MODE
            ? [
    ...
    // 第六层：输出规范（双模式共用）
    intentParts.push("以上七个节点全部设为根（无 parentId），全并行执行。", "每个节点的 payload 自包含——不需要读其他节点的结果才能开工。", "甘雨只做分派和最终的汇总摘要，不对专家的判断做二次加工。");
    ...
    console.log("╚══════════════════════════════════════════════════╝\n");
    for (const n of allNodes) {
        if (n.results.length === 0 || n.status === "pending")
            continue;
        const r = n.results[0];
    ...
    if (SOFT_MODE) {
        ...
        const crossVerifyPairs = loadCrossVerifyPairs(path.join(SCRIPTS_DIR, "..", "config"));
        ...
        const verifyFiles = await runCrossVerification(OUTPUT_DIR, crossVerifyPairs, verifierAgents, CHAT_MODEL);
        // 重新生成摘要以包含交叉验证产出
    ...
    // ═══════════════════════════════════════════════
    if (SOFT_MODE) {
        await runStrategyAnalysis(OUTPUT_DIR, strategistAgent, shuangningAgent, CHAT_MODEL);
    }
    // ═══════════════════════════════════════════════
    ...
    if (SOFT_MODE) {
        ...
        const agentReportMap = {};
        for (const k of consensusKeys) {
            const agent = cortexConfig.agents[k];
            if (agent?.display)
        ...
        if (fs.existsSync(OUTPUT_DIR)) {
            ...
            for (const [key, info] of Object.entries(agentReportMap)) {
                const reportFile = files.find((f) => f.includes(key) && f.endsWith(".md") && !f.includes("summary") && !f.includes("roundtable"));
                if (!reportFile) {
        ...
        try {
            await runMeeting(SOFT_CONSENSUS_ROUNDTABLE, adapter, CHAT_MODEL, DB_DIR, CONSENSUS_OUTPUT);
...
//# sourceMappingURL=cortex-self-examination.js.map