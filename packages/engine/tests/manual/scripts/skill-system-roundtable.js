...
import { getPersonaPrompts, QUALITY_RULES, } from "../config/roundtable-config";
...
// 种子记忆：技能系统现状
// ═══════════════════════════════════════════════
function buildSeedMemories(projectRoot) {
    const memories = [];
    // 读取技能沉淀机制设计文档
    ...
    if (fs.existsSync(constitutionPath)) {
        ...
        // 提取技能相关段落
        const skillLines = content.split("\n").filter((l) => /技能|skill|SkillRegistry|SkillTemplate|沉淀|闭环/i.test(l));
    ...
    for (const f of skillFiles) {
        try {
            ...
            skillSummaries.push(`- ${skill.id}: ${skill.name} | status=${skill.status} | adoption=${skill.adoptionCount} | rejection=${skill.rejectionCount} | agentType=${skill.agentType}`);
        }
        catch { /* skip */ }
    }
    memories.push({
...
// 会议配置
// ═══════════════════════════════════════════════
function buildMeetingConfig(allPersonas) {
    // 确保昔涟在参会列表中（如果不在就添加）
...
async function runCyreneRoundtable(config, adapter, chatModel, reasonerModel, dbDir, consensusOutputPath, seedMemories) {
    ...
    const converge = { requested: false, triggeredBy: "", triggeredAt: "" };
    try {
        console.log(`\n${"=".repeat(60)}`);
        ...
        for (let ri = 0; ri < config.rounds.length; ri++) {
            ...
            for (let turn = 1; turn <= round.maxTurns; turn++) {
                ...
                for (const persona of config.personas) {
                    ...
                    // 读取记忆
                    const recentMems = await memory.read({ kind: "TaskLog" });
                    ...
                    if (persona.name === "昔涟") {
                        ...
                        try {
                            ...
                            const cyrenePrompt = [
                                ...
                                .join("\n");
                            const cyreneChunks = [];
                            const cyreneRes = await adapter.chatStream(chatModel, [
                                { role: "system", content: persona.systemPrompt },
                                { role: "system", content: QUALITY_RULES },
                                { role: "user", content: cyrenePrompt },
                            ], undefined, (chunk) => { cyreneChunks.push(chunk); });
                        ...
                        const res = await adapter.chatStream(modelForAgent, [
                            { role: "system", content: persona.systemPrompt },
                            { role: "system", content: QUALITY_RULES },
                ...
                if (turn >= round.minTurns && !converge.requested) {
                    ...
                    if (cycleSubstantive < threshold) {
                        console.log(`  ⏹ 第${turn}次实质发言不足 (${cycleSubstantive}/${config.personas.length})，本轮终止`);
                        break;
                    }
                }
                // 收束后不再循环
        ...
        if (consensusOutputPath) {
            if (lastNingSpeech) {
                ...
                const historySection = oldContent
                    ...
                    : "";
                fs.writeFileSync(consensusOutputPath, header + (lastNingSpeech ?? "") + historySection, "utf-8");
                console.log(`  📝 共识清单已覆写: ${consensusOutputPath}`);
            }
            else {
...
async function main() {
    ...
    const personas = Object.entries(personaPrompts)
...
//# sourceMappingURL=skill-system-roundtable.js.map