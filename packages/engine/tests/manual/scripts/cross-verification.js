...
export function loadCrossVerifyPairs(configDir) {
 ...
 * @returns 产出的验证文件路径列表
 */
export async function runCrossVerification(outputDir, pairs, agents, chatModel) {
    const producedFiles = [];
    ...
    for (const pair of pairs) {
        ...
        // 查找报告文件
        const reportFile = dirFiles.find((f) => f.includes(pair.reportFilePattern) && f.endsWith(".md"));
        if (!reportFile) {
        ...
        const taskNode = {
            ...
            status: "pending",
            tags: ["verification", pair.verifierKey],
...
//# sourceMappingURL=cross-verification.js.map