...
async function deepseekChat(baseUrl, apiKey, model, messages) {
...
// ═══════════════════════════════════════════════
function buildSystemPrompt(targetUrl) {
    const persona = fs.readFileSync(path.join(ROOT, "prompts", "yoimiya", "system.md"), "utf-8");
    const reactProtocol = `
...
function parseAction(output) {
    ...
    switch (type) {
        case "navigate":
            if (parts[1])
                return { type: "navigate", url: parts[1].trim() };
            break;
        case "click":
            if (parts[1])
                return { type: "click", selector: parts[1].trim() };
            break;
        case "type":
            if (parts[1] && parts[2])
                return { type: "type", selector: parts[1].trim(), text: parts.slice(2).join("|").trim() };
            break;
        case "scroll":
        ...
        case "extract":
            if (parts[1])
                return { type: "extract", what: parts.slice(1).join("|").trim() };
            break;
        case "done":
...
async function main() {
    ...
    const TARGET_URL = "https://rcnwa456pqsg.feishu.cn/wiki/ZDOvwxeqpi7xjZkyxJLcGoocn8d?sheet=6KGlri";
    console.log("╔══════════════════════════════════════════════╗");
    ...
    try {
        ...
        for (let round = 1; round <= MAX_ROUNDS; round++) {
            ...
            const output = await deepseekChat(BASE_URL, API_KEY, CHAT_MODEL, [
            ...
            if (intentMatch)
                console.log(`  🎯 ${intentMatch[1].trim()}`);
            // 解析动作
...
//# sourceMappingURL=yoimiya-feishu.js.map