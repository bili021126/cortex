...
async function main() {
    ...
    for (const [n, v] of [["TaskRouter", engine.taskRouter], ["Sentinel", engine.sentinelFilter], ["NotifyRt", engine.notificationRuntime], ["Bridge", engine.decisionBridge], ["GovEmit", engine.governanceEmitter], ["EnvRt", engine.envRouter]])
        log(`   ${v ? "✅" : "⚠️"} ${n}`);
    const events = [];
    ...
    for (const r of rpt.results)
        log(`   ${r.success ? "✅" : "❌"} ${r.agentType}: ${(r.output ?? "").slice(0, 80)}`);
    log(`\n📊 ${rpt.completed} ok | events: ${events.length}`);
    await engine.shutdown();
    log("✨ Done");
}
main().catch((e) => { log(`💥 ${e}`); process.exit(1); });
//# sourceMappingURL=solo-flight-real.js.map