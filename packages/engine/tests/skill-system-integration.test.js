...
function mockLoopAgentOutput(skills) {
...
describe("Skill System Integration", () => {
    ...
    it("full lifecycle: LoopAgent output → register → MetaAgent query", () => {
        ...
        // 阶段4：MetaAgent 查询
        // fix 类型查询
        const fixSkills = registry.queryByTags(["fix"]);
        ...
        // 按标签查询
        const patternScanSkills = registry.queryByTags(["skill_precipitate"]);
        expect(patternScanSkills.length).toBe(1);
        // 不匹配的标签
        const noMatch = registry.queryByTags(["browser"]);
    ...
    it("should filter deprecated skills from queries", () => {
        ...
        const byTag = registry.queryByTags(["implementation"]);
    ...
    it("should unregister and clean indexes", () => {
        ...
        expect(registry.queryByTags(["fix"]).length).toBe(1);
...
//# sourceMappingURL=skill-system-integration.test.js.map