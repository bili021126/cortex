...
describe("MemoryStore 并发写入压力", () => {
    ...
    it("10 Agent 并发写 100 条记忆——无崩溃、ID 格式合法", async () => {
        ...
        const writeOps = Array.from({ length: agentCount }, (_, ai) => Array.from({ length: writesPerAgent }, (_, wi) => store.write({
            kind: "TaskLog",
        ...
        // 所有成功写入返回合法 ID
        expect(fulfilled.every((r) => typeof r.value === "string"
            && /^[0-9a-f]{8}-/.test(r.value))).toBe(true);
    ...
    it("并发写入 + 并发关闭——不抛异常", async () => {
        // 同时写入和关闭
        const writes = Array.from({ length: 5 }, (_, i) => store.write({
            kind: "TaskLog",
            content_blob: { idx: i },
...
//# sourceMappingURL=memory-concurrency.test.js.map