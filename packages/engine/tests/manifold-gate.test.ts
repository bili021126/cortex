// @ci: unit
/**
 * ManifoldGate 流约束门控 —— 全场景单元测试
 *
 * 验证 mHC 流形约束的核心行为：
 * - 同类型 Agent 并发数 ≤ maxInstances
 * - FIFO 公平唤醒
 * - 超时优雅失败
 * - RLM 子任务不参与流约束
...
describe("ManifoldGate — 流约束核心", () => {
  ...
  it("超时后返回 false 且不从队列唤醒下一个", async () => {
    ManifoldGate.register("test-type", 1);
  ...
  it("release 在 active=0 时不做负值", () => {
    ManifoldGate.register("test-type", 3);
    ManifoldGate.release("test-type"); // 应安全 no-op
  ...
  it("reset 清空所有状态", async () => {
    ...
    ManifoldGate.reset();
    expect(ManifoldGate.active("test-type")).toBe(0);
    expect(ManifoldGate.max("test-type")).toBe(1); // 重置后退回默认
  });
});
...
describe("Scheduler + ManifoldGate 集成", () => {
  ...
  it("同类型超池节点被流控等待后全部成功（mHC 约束不丢节点）", async () => {
    ...
    // 同一层 5 个 code 节点——旧行为：2 成功 + 3 池耗尽失败
