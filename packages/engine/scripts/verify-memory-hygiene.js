...
async function main() {
    ...
    const agingEntry = store._storage.insert({
        kind: "TaskLog",
    ...
    console.log(`  ${INFO} L3 aging insert id: ${agingId} (total: ${store._storage.memories.size})`);
    ...
    if (agedEntry) {
        ...
        assert(aged > 0 && aged < 1.0 && aged > 0.5, `权重老化生效: 原始 1.0 → ${aged.toFixed(4)} (期望在 0.5~1.0 之间，14天未访问 ≈0.9025)`);
    }
    else {
    ...
    // ── L4: 内存总量软上限 ──────────────────────────
    console.log("\n── L4 内存总量软上限 ──");
    console.log(`  ${INFO} 当前记忆量: ${store._storage.memories.size} 条 (上限 10000)`);
    ...
    try {
        ...
        const vec = await embedText("端口 3000 被占用");
        assert(vec.length === 384, `embedText 生成 384d 向量: 实际 ${vec.length}d`);
        // 验证归一化 (L2 norm ≈ 1.0)
        const norm = Math.sqrt(vec.reduce((sum, v) => sum + v * v, 0));
        assert(Math.abs(norm - 1.0) < 0.01, `向量 L2 归一化: norm=${norm.toFixed(4)} (期望 ≈1.0)`);
        // 两个语义相似文本的余弦相似度应 > 0.8
        ...
        for (let i = 0; i < 384; i++)
            dot += vec1[i] * vec2[i];
        assert(dot >= 0.8, `语义相似文本余弦相似度: ${dot.toFixed(4)} (期望 ≥0.8)`);
        // 语义无关文本的余弦相似度应 < 0.5
        const vec3 = await embedText("今天天气真好适合出去玩");
        let dot2 = 0;
        for (let i = 0; i < 384; i++)
            dot2 += vec1[i] * vec3[i];
        assert(dot2 < 0.5, `语义无关文本余弦相似度: ${dot2.toFixed(4)} (期望 <0.5)`);
        ...
        assert(dedupHit, `向量相似去重命中: 写入"端口3000被占用" → 返回已有 id`);
    }
    catch (e) {
...
//# sourceMappingURL=verify-memory-hygiene.js.map