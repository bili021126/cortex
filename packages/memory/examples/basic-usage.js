...
// ============================================================
import { InMemoryMemoryStore, FileBasedMemoryStore, MemoryStoreRegistry, } from "@cortex/memory";
...
async function exampleFileBasedStore() {
...
async function exampleRegistry() {
    ...
    console.log(`默认存储名称: ${defaultStore.constructor.name}`);
    // 获取工厂存储（触发惰性初始化）
    ...
    console.log(`切换后默认: ${newDefault.constructor.name}`);
    // 列出所有名称
