// ============================================================
// Cyrene-Agent 记忆系统 — 近期注入记忆追踪（适配版）
//
// 从 Cyrene-Agent src/main/memory/recent-injected-memory.ts 提取。
// 纯内存模块，无 Electron/IPC 依赖。
// ============================================================
...
function prune(now: number): void {
  ...
  for (let i = recentInjections.length - 1; i >= 0; i--) {
    if ((recentInjections[i]?.injectedAt ?? 0) < minTime) {
