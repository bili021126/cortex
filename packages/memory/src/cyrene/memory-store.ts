...
// 閫傞厤锛氱Щ闄?Electron/IPC 渚濊禆銆傝矾寰勬敼涓烘瀯閫犳敞鍏ャ€?
// 绉婚櫎 EventBus 浜嬩欢锛圕ortex 涓嶄娇鐢?ipcMain锛夈€?
// ============================================================

import * as fs from "fs"
import * as path from "path"
import type {
  ConflictLog, L0Profile, L1Profile, L2Memory, L2SyncStatus,
  MemoryConflictResolution, MemoryEvidence, MemoryStoreData, ReflectionLog,
...
function cloneDefaultStore(): MemoryStoreData {
  return {
    ...DEFAULT_STORE,
    l0: { ...DEFAULT_L0 },
    l1: { ...DEFAULT_L1 },
...
export function repairMigrations(store: Partial<MemoryStoreData>): MemoryStoreData {
  return {
    schemaVersion: CURRENT_SCHEMA_VERSION,
    l0: { ...DEFAULT_L0, ...store.l0 },
    l1: { ...DEFAULT_L1, ...store.l1 },
    l2: Array.isArray(store.l2) ? store.l2.map((memory: L2Memory) => ({
      ...memory,
    ...
    conflictLogs: Array.isArray(store.conflictLogs) ? store.conflictLogs.map((log: ConflictLog) => ({
      ...log,
...
export class MemoryStoreManager {
  ...
  constructor(filePath?: string) {
    this.filePath = filePath ?? path.join(process.cwd(), "data", "memory.json")
  }
  ...
  async updateL2RecallStats(id: string, delta = 1): Promise<void> {
    const store = await this.load()
    const mem = store.l2.find((m: L2Memory): boolean => m.id === id)
    if (!mem) return
  ...
  async deleteL2(id: string): Promise<void> {
    const store = await this.load()
    store.l2 = store.l2.filter((m: L2Memory): boolean => m.id !== id)
    await this.save(store)
  ...
  async markL2Conflict(id: string, conflictRagId: string): Promise<L2Memory | null> {
    ...
    if (!mem.isPinned && mem.status === "active") mem.status = "aging"
    await this.save(store)
    appendMemoryTrace({
      op: "l2.conflict.mark",
      layer: "L2",
  ...
  async getEvidenceByMemoryId(memoryId: string): Promise<MemoryEvidence[]> {
    const store = await this.load()
    return (store.evidence ?? []).filter((evidence: MemoryEvidence): boolean => evidence.memoryId === memoryId)
  }
  ...
  async appendReflectionLog(log: Omit<ReflectionLog, "id" | "createdAt">): Promise<void> {
    ...
    if (store.reflectionLogs.length > 50) store.reflectionLogs = store.reflectionLogs.slice(-50)
    await this.save(store)
    appendMemoryTrace({
  ...
  async appendConflictLog(log: Omit<ConflictLog, "id" | "createdAt">): Promise<ConflictLog> {
    ...
    if (store.conflictLogs.length > 100) store.conflictLogs = store.conflictLogs.slice(-100)
    await this.save(store)
    appendMemoryTrace({
      op: "conflict.log.add",
      layer: "L2",
      ...
      details: { conflictLogId: entry.id, targetL2Id: entry.targetL2Id, detector: entry.detector, conflictStatus: entry.status },
    })
    return entry
  }
  ...
  async scoreConflictLog(
    ...
    const store = await this.load()
    const log = (store.conflictLogs ?? []).find((entry: ConflictLog): boolean => entry.id === id)
    if (!log) return null
    ...
    appendMemoryTrace({
      ...
      details: { conflictLogId: log.id, targetL2Id: log.targetL2Id, conflictScore: log.conflictScore, resolverPriority: log.resolverPriority, scoringSignals: log.scoringSignals },
    })
    if (shouldQueue) {
      appendMemoryTrace({
        ...
        l2Id: log.sourceL2Id,
        ragId: log.sourceRagId,
        details: { conflictLogId: log.id, targetL2Id: log.targetL2Id, resolverPriority: log.resolverPriority, conflictScore: log.conflictScore },
      })
    }
    return log
  }
  ...
  async getResolverQueue(limit = 20): Promise<ConflictLog[]> {
    ...
    return (store.conflictLogs ?? [])
      ...
      .sort((a: ConflictLog, b: ConflictLog): number => {
        const priorityDiff = (RESOLVER_PRIORITY_RANK[b.resolverPriority ?? "none"] ?? 0) - (RESOLVER_PRIORITY_RANK[a.resolverPriority ?? "none"] ?? 0)
  ...
  async applyResolverResolution(conflictLogId: string, resolution: MemoryConflictResolution): Promise<ConflictLog | null> {
    ...
    if (!log) return null
    const newMemory = store.l2.find((memory: L2Memory): boolean => memory.id === log.sourceL2Id)
    const oldMemory = store.l2.find((memory: L2Memory): boolean => memory.id === log.targetL2Id)
    ...
    if (resolution.actions.oldMemoryStatus) {
      ...
      if (resolution.actions.oldMemoryStatus === "superseded" && resolutionMemoryId) oldMemory.supersededBy = resolutionMemoryId
      if (resolution.actions.oldMemoryStatus === "merged" && resolutionMemoryId) oldMemory.mergedInto = resolutionMemoryId
    }
    if (resolution.actions.newMemoryStatus) {
      newMemory.status = resolution.actions.newMemoryStatus
      if (resolution.actions.newMemoryStatus === "superseded" && resolutionMemoryId) newMemory.supersededBy = resolutionMemoryId
      if (resolution.actions.newMemoryStatus === "merged" && resolutionMemoryId) newMemory.mergedInto = resolutionMemoryId
    }
    ...
    appendMemoryTrace({
      ...
      l2Id: log.sourceL2Id,
      ragId: log.sourceRagId,
      details: { conflictLogId: log.id, targetL2Id: log.targetL2Id, resolutionType: log.resolutionType, resolutionMemoryId, conflictStatus: log.status },
    })
    return log
  }
  ...
  async updateL2Status(ids: string[], status: L2Memory["status"]): Promise<void> {
    ...
    for (const mem of store.l2) {
      if (ids.includes(mem.id)) mem.status = status
    }
    await this.save(store)
    appendMemoryTrace({ op: "l2.status.batch", layer: "L2", status: "ok", details: { ids, memoryStatus: status } })
  }
  ...
  async decayL2Weights(delta = 1): Promise<number> {
    ...
    for (const mem of store.l2) {
      ...
      mem.status = mem.weight >= 30 ? "active" : mem.weight >= 10 ? "aging" : "archived"
      changed += 1
    }
    if (changed > 0) await this.save(store)
    appendMemoryTrace({ op: "l2.decay", layer: "L2", status: changed > 0 ? "ok" : "skip", details: { delta, changed } })
    return changed
  }
  ...
  async addL2Batch(inputs: L2Input[]): Promise<L2Memory[]> {
    ...
    await this.save(store)
    appendMemoryTrace({ op: "l2.add.batch", layer: "L2", status: "ok", details: { ids: results.map((item: L2Memory): string => item.id), count: results.length } })
    for (const memory of results) {
      const evidenceId = memory.evidenceIds?.[memory.evidenceIds.length - 1]
      appendMemoryTrace({ op: "evidence.add", layer: "L2", status: "ok", l2Id: memory.id, details: { evidenceId, sourceStatus: "active" } })
    }
    return results
  }
}
