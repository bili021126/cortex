// ============================================================
// Cyrene-Agent 记忆系统 — MemoryManager（适配版）
//
// 从 Cyrene-Agent src/main/memory/memory-manager.ts 提取。
...
import { MemoryCandidate, L2Memory, L0_FIELD_DESCRIPTIONS } from "./memory-types.js"
import { findPossibleConflictCandidate } from "./memory-conflict.js"
import { scoreMemoryConflict, type ConflictEvidenceLevel } from "./memory-conflict-score.js"
import { wasRecentlyInjectedMemory } from "./recent-injected-memory.js"
...
export class MemoryManager {
  ...
  constructor(deps?: MemoryManagerDeps) {
    this.deps = deps ?? {
      addMemory: async () => { throw new Error("addMemory not configured") },
  ...
  private async writeL2(candidate: MemoryCandidate): Promise<void> {
    ...
    try {
      ragId = await this.deps.addMemory(candidate.content, "user_memory", {
        triggerText: candidate.triggerText,
  ...
  private async detectAndMarkConflicts(content: string, newL2Id: string, newRagId: string, triggerText: string): Promise<void> {
    ...
    const similarEntries = await this.deps.searchMemoryEntries(content, "user_memory", 5, { recordRecall: false })
    ...
    for (const entry of similarEntries) {
      ...
      if (typeof l2Id === "string" && l2Id.length > 0) entriesByL2Id.set(l2Id, entry)
    }
