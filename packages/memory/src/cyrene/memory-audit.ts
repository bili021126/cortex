// ============================================================
// Cyrene-Agent 记忆系统 — 记忆审计（适配版）
//
// 从 Cyrene-Agent src/main/memory/memory-audit.ts 提取。
// 纯函数，无 Electron/IPC 依赖。
// ============================================================

import type { L2Memory, MemoryEvidence, MemoryStoreData } from "./memory-types.js"
...
export function auditMemoryStore(store: MemoryStoreData): MemoryAuditFinding[] {
  ...
  const evidenceById = new Map<string, MemoryEvidence>(evidence.map((item: MemoryEvidence) => [item.id, item]))
...
export function auditMemoryFile(filePath: string, fs: { readFileSync: (p: string, enc: string) => string }): MemoryAuditReport {
  const parsed = JSON.parse(fs.readFileSync(filePath, "utf8")) as MemoryStoreData
  return {
    filePath,
