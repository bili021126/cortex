...
describe("InMemoryMemoryStore", () => {
    ...
    describe("read / query", () => {
        ...
        it("should filter by metadata", async () => {
            ...
            expect(results.length).toBe(1);
            expect(results[0].content_blob.env).toBe("test");
        });
    });
    ...
    describe("transactions", () => {
        ...
        it("should throw when writing to a committed transaction", async () => {
            ...
            await expect(store.writeWithin(txn, createSampleInput())).rejects.toThrow("Transaction not found or completed");
        });
    ...
    describe("readWithin", () => {
        ...
        it("should throw when reading within a rolled-back transaction", async () => {
            ...
            await expect(store.readWithin(txn, {})).rejects.toThrow("Transaction not found or completed");
        });
    });
    ...
    describe("error handling - invalid input", () => {
        ...
        it("should throw when semantic_gist is empty", async () => {
            await expect(store.write(createSampleInput({ semantic_gist: "" }))).rejects.toThrow("semantic_gist");
        });
        it("should throw when content_blob is missing", async () => {
            const input = createSampleInput();
            delete input.content_blob;
    ...
    describe("concurrent operations", () => {
        it("should handle concurrent writes", async () => {
            const promises = Array.from({ length: 10 }, (_, i) => store.write(createSampleInput({ summary: `并发写入${i}` })));
...
//# sourceMappingURL=InMemoryMemoryStore.test.js.map