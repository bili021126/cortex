...
import type { IMemoryStore } from "@cortex/memory";

// ─── Mock Store ───────────────────────────────

class MockStore implements IMemoryStore {
  ...
  async get(_id: string): Promise<undefined> {
    return undefined;
  }
  peek(_id: string): Readonly<import("@cortex/shared").MemoryEntry> | undefined {
    return undefined;
  }
  has(_id: string): boolean {
    return false;
  }
  async read(): Promise<import("@cortex/shared").MemoryEntry[]> {
    return [];
  }
  getLinks(): import("@cortex/shared").MemoryLink[] {
    return [];
  }
  getBySession(): import("@cortex/shared").MemoryEntry[] {
    return [];
  }
  getPending(): import("@cortex/shared").MemoryEntry[] {
    return [];
  }
  hasPending(): boolean {
    return false;
  ...
  beginSession(_externalId?: string): string {
    return "mock-session";
  }
...
describe("MemoryStoreRegistry", () => {
  ...
  describe("working with InMemoryMemoryStore", () => {
    it("should integrate with InMemoryMemoryStore", async () => {
      ...
      const retrieved = await registry.get("mem") as InMemoryMemoryStore;
