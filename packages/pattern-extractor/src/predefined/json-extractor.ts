...
// ============================================================
// §1 JsonExtractorOptions — 提取器专有选项
// ============================================================
 ...
 * ```
 */
export type JsonExtractorOptions = PatternExtractorOptions &
...
// ============================================================
// §2 InternalOptions — 内部合并选项类型
// ============================================================

/**
 * InternalOptions —— 构造函数注入与 extract() 运行时选项合并后的内部类型。
 *
 * 所有字段均为必填，由构造函数或 extract() 方法填充默认值。
 * 使用独立接口以避免对泛型约束 Record<string, unknown> 的依赖。
 */
 ...
 * NamingStyle —— 键命名风格枚举。
 *
 * 用于统计 JSON 对象键的命名约定分布。
 */
type NamingStyle = "camelCase" | "snake_case" | "PascalCase" | "kebab-case" | "other";

/**
 * PropertyType —— 属性值类型枚举。
 *
 * 对应 JSON 规范中的六种值类型。
 */
type PropertyType = "string" | "number" | "boolean" | "null" | "object" | "array";

/**
 * JsonAnalysisStats —— JSON 结构分析统计汇总。
 *
 * 由 _analyzeJson() 递归遍历过程中累积填充。
 * 所有字段均为只读语义，仅通过专用方法修改。
 */
interface JsonAnalysisStats {
  /** 命名风格计数器 */
  namingCounts: Record<NamingStyle, number>;
  /** 所有遇到的键名列表（用于正反例） */
  allKeys: string[];
  /** 最大嵌套深度 */
  maxDepth: number;
  /** 遍历深度累加（用于计算平均深度） */
  totalDepthSum: number;
  /** 总访问节点数 */
  visitedNodes: number;
  /** 属性类型计数器 */
  ...
  /** 同质数组计数器（元素类型一致的数组） */
  homogeneousArrays: number;
  /** 每个对象的属性数量列表（用于计算属性分布） */
  propertyCountsPerObject: number[];
  /** 字符串长度累加（用于估算数据量） */
 ...
 */
function createEmptyStats(): JsonAnalysisStats {
  return {
    namingCounts: {
      camelCase: 0,
      snake_case: 0,
 ...
 * ```
 */
function detectNamingStyle(key: string): NamingStyle {
  // 空键或非字母开头
  if (key.length === 0) {
    return "other";
  }
 ...
 * @returns 属性类型枚举值
 */
function detectPropertyType(value: JsonValue): PropertyType {
  if (value === null) {
    return "null";
  }
 ...
 * @returns 唯一 ID 字符串
 */
function buildPatternId(nameSlug: string): string {
...
function buildNamingPattern(
  ...
  // 找占比最高的命名风格
  let dominantStyle: NamingStyle = "other";
  let dominantCount = 0;

  const styleEntries = Object.entries(stats.namingCounts) as [NamingStyle, number][];
  ...
  // 找到正反例
  const positiveExample = stats.allKeys.find(
    (k) => detectNamingStyle(k) === dominantStyle,
  ) ?? stats.allKeys[0];

  const negativeExample = stats.allKeys.find(
    (k) => detectNamingStyle(k) !== dominantStyle,
  );

  const styleDescriptionMap: Record<NamingStyle, string> = {
  ...
  const styleNameMap: Record<NamingStyle, string> = {
    camelCase: "camelCase",
  ...
  // 构建示例
  const examples: PatternBody["examples"] = [
    {
      code: JSON.stringify(
  ...
  return {
    ...
    description: [
      ...
      ...Object.entries(stats.namingCounts)
        ...
        .map(
          ([style, count]) =>
            `- ${style}：${count} 个（${Math.round((count / totalKeys) * 100)}%）`,
        ),
      "",
      "**建议**：",
...
function buildStructurePattern(
  ...
  const confidence = Math.min(
    0.3 + (stats.visitedNodes / (stats.visitedNodes + 10)) * 0.5,
    0.9,
  );
  ...
  if (avgPropsPerObject > 0) {
    rules.push(
      `平均每对象含 ${avgPropsPerObject.toFixed(1)} 个属性` +
 ...
 * @returns PatternDefinition 或 undefined
 */
function buildTypeDistributionPattern(
  stats: JsonAnalysisStats,
  source: string,
  minSampleSize: number,
): PatternDefinition | undefined {
  ...
  const confidence = Math.min(
    0.3 + (totalValues / (totalValues + 20)) * 0.5,
    0.85,
  );

  const typeNameMap: Record<PropertyType, string> = {
    string: "字符串（string）",
    number: "数字（number）",
    boolean: "布尔值（boolean）",
    null: "空值（null）",
 ...
 * @returns PatternDefinition 或 undefined
 */
function buildArrayHomogeneityPattern(
  stats: JsonAnalysisStats,
  source: string,
  minSampleSize: number,
): PatternDefinition | undefined {
  ...
  const homogeneityRatio =
    stats.arraysAnalyzed > 0
  ...
  implements PatternExtractor<string, JsonExtractorOptions>
{
  ...
  readonly description =
    "基于 JSON 结构分析的模式提取器，从 JSON 内容中提取键命名约定、对象结构深度、属性类型分布和数组元素同质性等模式";
  ...
  extract(input: string, options?: JsonExtractorOptions): ExtractionResult {
    ...
    try {
      parsedValue = JSON.parse(trimmedInput) as JsonValue;
    ...
    try {
      ...
      diagnostics.push(
        `[json-extractor] 结构分析异常: ${errorMessage}`,
      );

      if (this._options.enableDiagnostics) {
        diagnostics.push(
          `[json-extractor] 已访问节点数: ${stats.visitedNodes}`,
        );
      }
    }
    ...
    const mergedOptions: InternalOptions = {
      extractNamingPatterns:
        options?.extractNamingPatterns ?? this._options.extractNamingPatterns,
      extractStructurePatterns:
        options?.extractStructurePatterns ??
        this._options.extractStructurePatterns,
      extractTypePatterns:
        options?.extractTypePatterns ?? this._options.extractTypePatterns,
      extractArrayPatterns:
        options?.extractArrayPatterns ?? this._options.extractArrayPatterns,
    ...
    diagnostics.push(
      `[json-extractor] 分析完成: ${stats.visitedNodes} 节点, ${stats.allKeys.length} 键, ${stats.arraysAnalyzed} 数组`,
    );
    ...
    if (mergedOptions.extractNamingPatterns) {
      const namingPattern = buildNamingPattern(
        stats,
        source,
      ...
      if (namingPattern !== undefined) {
        ...
        diagnostics.push(
          `[json-extractor] 命名模式: "${namingPattern.name}" (置信度: ${namingPattern.confidence})`,
        );
      } else {
        diagnostics.push(
          "[json-extractor] 命名模式: 样本不足或分布无主导风格，跳过",
        );
      }
    }

    if (mergedOptions.extractStructurePatterns) {
      const structurePattern = buildStructurePattern(
        stats,
        source,
        mergedOptions.minSampleSize,
      );
      if (structurePattern !== undefined) {
        ...
        diagnostics.push(
          `[json-extractor] 结构模式: "${structurePattern.name}" (置信度: ${structurePattern.confidence})`,
        );
      } else {
        diagnostics.push(
          "[json-extractor] 结构模式: 样本不足，跳过",
        );
      }
    }

    if (mergedOptions.extractTypePatterns) {
      const typePattern = buildTypeDistributionPattern(
        stats,
        source,
        mergedOptions.minSampleSize,
      );
      if (typePattern !== undefined) {
        ...
        diagnostics.push(
          `[json-extractor] 类型模式: "${typePattern.name}" (置信度: ${typePattern.confidence})`,
        );
      } else {
        diagnostics.push(
          "[json-extractor] 类型模式: 样本不足，跳过",
        );
      }
    }

    if (mergedOptions.extractArrayPatterns) {
      const arrayPattern = buildArrayHomogeneityPattern(
        stats,
        source,
        mergedOptions.minSampleSize,
      );
      if (arrayPattern !== undefined) {
        ...
        diagnostics.push(
          `[json-extractor] 数组模式: "${arrayPattern.name}" (置信度: ${arrayPattern.confidence})`,
        );
      } else {
        diagnostics.push(
          "[json-extractor] 数组模式: 样本不足，跳过",
        );
      }
    }
    ...
    diagnostics.push(
      `[json-extractor] 提取完成: ${patterns.length} 个模式, 耗时 ${durationMs}ms`,
    );
 ...
 */
export const JSON_EXTRACTOR_ANCHOR =
  "[@cortex/pattern-extractor] JsonPatternExtractor v0.1.0 — JSON 结构模式提取器";
