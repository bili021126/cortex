...
// ============================================================
// §1 MarkdownExtractorOptions — 提取器专有选项
// ============================================================
 ...
 * ```
 */
export type MarkdownExtractorOptions = PatternExtractorOptions &
...
// ============================================================
// §2 SkillTemplateCandidate — 中间候选类型
// ============================================================

/**
 * SkillTemplateCandidate —— 从 Markdown 中提取出的技能模板候选。
 *
 * 这是 PatternDefinition 的中间产物——字段映射：
 * - SkillTemplate.id → PatternDefinition.id
 * - SkillTemplate.name → PatternDefinition.name
 * - SkillTemplate.trigger → PatternDefinition.description（trigger 文本内联）
 * - SkillTemplate.tags（triggerTags）→ PatternDefinition.tags
 * - SkillTemplate.steps → PatternDefinition.body（steps 结构化存储）
 * - SkillTemplate.expectedOutput / status 等 → elements
 *
 * 此类不暴露为公开 API——仅供 extract() 内部使用，
 * 最终统一转换为 PatternDefinition[] 返回。
 */
interface SkillTemplateCandidate {
  /** 从 Regex match 或 heading 中提取的原始名称 */
  name: string;

  /** 触发标签列表 */
  tags: string[];

  /** 触发条件描述文本（如 "需要从已产出的 pattern.md 文件中提取..."） */
  triggerText: string;
  ...
  /** 原始来源段落文本（用于 diagnostic 追踪） */
  rawText: string;

  /** 提取策略名称——记录是哪种策略命中的 */
  strategy: "json-block" | "p0-p9" | "pattern-paragraph" | "full-file";

  /** 基础置信度（策略自评） */
  baseConfidence: number;

  /** 在文件中的源码行号范围 [startLine, endLine] */
  ...
  implements PatternExtractor<string, MarkdownExtractorOptions>
{
  ...
  readonly description =
    "基于 Markdown 结构分析的模式提取器，按照 4 级回退策略从 pattern.md / patterns.md 文件中提取技能模板候选模式。策略优先级：JSON 块提取 → P0-P9 格式提取 → 模式段落提取 → 全文回退";

  /** 合并后的选项（含默认值） */
  private readonly _options: Required<MarkdownExtractorOptions>;
  ...
  extract(
    ...
    const allCandidates: SkillTemplateCandidate[] = [];

    // 策略 1：JSON 块提取
    if (opts.strategyJsonBlock) {
      const jsonCandidates = this._extractJsonBlocks(
        trimmedInput,
        diagnostics,
        opts,
      );
    ...
    if (opts.strategyP0P9Format) {
      const p0p9Candidates = this._extractP0P9Format(
        trimmedInput,
        diagnostics,
        opts,
      );
    ...
    if (opts.strategyPatternParagraph) {
      const paraCandidates = this._extractPatternParagraphs(
        trimmedInput,
        diagnostics,
        opts,
      );
    ...
    // 策略 4：全文回退（仅当无其他产出时）
    if (
      opts.strategyFallbackFullFile &&
      allCandidates.length === 0
    ) {
      const fallback = this._extractFullFileFallback(
        trimmedInput,
        diagnostics,
      );
      if (fallback) allCandidates.push(fallback);
    }

    // ── 步骤 3：去重归并 ──

    let merged: SkillTemplateCandidate[] = allCandidates;
    ...
    const filtered = merged.filter(
      (c) => c.baseConfidence >= opts.minConfidence,
    );
    ...
    const patterns = trimmed.map((c, i) =>
      this._candidateToPatternDefinition(c, i),
    );
  ...
  private _extractJsonBlocks(
    input: string,
    diagnostics: string[],
    _opts: Required<MarkdownExtractorOptions>,
    ...
    while ((match = MarkdownPatternExtractor.JSON_BLOCK_RE.exec(input)) !== null) {
      ...
      try {
        ...
        diagnostics.push(
          `[${this.name}] JSON 块解析失败 (offset ${match.index}): ` +
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        if (!rawName) {
          diagnostics.push(
            `[${this.name}] JSON 块条目[${i}]缺少 name 字段，跳过`,
          );
          continue;
        }
        ...
        const triggerText =
          typeof obj.trigger === "string" ? obj.trigger : "";
        ...
        if (rawId) baseConfidence += 0.1;
        if (triggerText) baseConfidence += 0.1;
        if (tags.length > 0) baseConfidence += 0.1;
        if (steps.length > 0) baseConfidence += 0.1;
        if (expectedOutput) baseConfidence += 0.05;
      ...
      diagnostics.push(
        `[${this.name}] JSON block extracted ${items.length} candidates ` +
   ...
   */
  private _extractTagsFromJson(obj: Record<string, unknown>): string[] {
    const raw =
      Array.isArray(obj.triggerTags)
        ? obj.triggerTags
   ...
   */
  private _extractStepsFromJson(obj: Record<string, unknown>): string[] {
    const raw =
  ...
  private _extractP0P9Format(
    input: string,
    diagnostics: string[],
    opts: Required<MarkdownExtractorOptions>,
  ): SkillTemplateCandidate[] {
    const candidates: SkillTemplateCandidate[] = [];
    ...
    for (const section of sections) {
      ...
      const pMatch = MarkdownPatternExtractor.P_NUMBER_RE.exec(
        section.heading,
      );
      if (!pMatch) continue;
      ...
      if (steps.length < opts.minStepsForP0P9) {
        diagnostics.push(
          `[${this.name}] P-number section "${section.heading}" ` +
      ...
      if (triggerText) baseConfidence += 0.15;
      if (tags.length > 0) baseConfidence += 0.15;
      if (steps.length >= 3) baseConfidence += 0.15;
      if (expectedOutput) baseConfidence += 0.1;
   ...
   */
  private _extractTagsFromPNSection(body: string): string[] {
   ...
   */
  private _extractStepsFromPNSection(body: string): string[] {
    // 格式 1: "Recipe:" / "Steps:" 节后的 bullet 列表
    const recipeMatch = body.match(
    ...
    const numberedGlobal = body.match(
      /^[\t ]*(?:\d+[.、)\]]\s*)(.+)/gm,
    );
  ...
  private _extractPatternParagraphs(
    ...
    for (const section of sections) {
      ...
      if (MarkdownPatternExtractor.P_NUMBER_RE.test(section.heading)) continue;

      const body = section.body;
      if (body.length < 30) continue; // 太短的段落跳过
      ...
      diagnostics.push(
        `[${this.name}] pattern-paragraph heuristic: ` +
   ...
   */
  private _countStepSignals(body: string): number {
    const lines = body.split("\n");
    let count = 0;
    for (const line of lines) {
      if (
        MarkdownPatternExtractor.LIST_ITEM_RE.test(line) &&
   ...
   */
  private _hasTagListSignal(body: string): boolean {
   ...
   * 从自由格式段落中提取标签。
   */
  private _extractTagsFromPatternParagraph(body: string): string[] {
   ...
   * 从自由格式段落中提取步骤列表。
   */
  private _extractStepsFromPatternParagraph(body: string): string[] {
    const lines = body.split("\n");
    return lines
      .filter(
        (line) =>
  ...
  private _splitByHeadings(
    input: string,
    levels: number[],
  ): { heading: string; body: string; startLine: number; endLine: number }[] {
    const sections: {
      heading: string;
      body: string;
      startLine: number;
  ...
  private _mergeByStrategyPriority(
    candidates: SkillTemplateCandidate[],
    diagnostics: string[],
  ): SkillTemplateCandidate[] {
    const map = new Map<string, SkillTemplateCandidate>();
    ...
    if (candidates.length !== map.size) {
      diagnostics.push(
        `[${this.name}] 去重归并: ${candidates.length} → ${map.size}`,
      );
    }
  ...
  private _mergeOptions(
    base: Required<MarkdownExtractorOptions>,
    overrides: MarkdownExtractorOptions,
  ): Required<MarkdownExtractorOptions> {
    const merged: Required<MarkdownExtractorOptions> = { ...base };
