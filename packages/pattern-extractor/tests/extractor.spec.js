...
import { PatternKind, PACKAGE_ANCHOR, } from "../src/extractor.js";
// ============================================================
// §1 extractor.ts 基本类型
// ============================================================
...
describe("extractor.ts - PatternExtractorOptions", () => {
    ...
    it("enableDiagnostics 为布尔值", () => {
        const enabled = { enableDiagnostics: true };
        const disabled = { enableDiagnostics: false };
...
describe("extractor.ts - PatternExtractor 接口", () => {
    ...
    it("canHandle 应支持语言和种类的复合判断", () => {
        const extractor = {
            ...
            canHandle: function (language, kind) {
    ...
    it("extract 的泛型参数应能传递不同的输入类型", () => {
        ...
        const node = { type: "InterfaceDeclaration" };
...
describe("extractor.ts - PatternDefinition 接口", () => {
    const createPatternDef = (overrides) => ({
        id: "pd-001",
        kind: PatternKind.Structural,
        name: "测试模式定义",
...
describe("JsonPatternExtractor - extract 基本功能", () => {
    ...
    it("应提取结构深度模式", async () => {
        ...
        if (result.success) {
            ...
            const depthPattern = structPatterns.find((p) => p.id === "json-extractor-structure-depth");
    ...
    it("应提取属性类型分布模式", async () => {
        ...
        expect(result.success).toBe(true);
        if (result.success) {
            const typePatterns = result.patterns.filter((p) => p.id === "json-extractor-type-distribution");
    ...
    it("应提取数组同质性模式", async () => {
        ...
        expect(result.success).toBe(true);
        if (result.success) {
            const arrayPatterns = result.patterns.filter((p) => p.id === "json-extractor-array-homogeneity");
...
describe("JsonPatternExtractor - 运行时选项覆盖", () => {
    it("extract() 时传入的选项应覆盖构造选项", async () => {
        ...
        if (resultEnabled.success) {
            const namingPatterns = resultEnabled.patterns.filter((p) => p.kind === PatternKind.Naming);
            expect(namingPatterns.length).toBeGreaterThanOrEqual(1);
        }
    });
    it("minSampleSize 可通过运行时选项调整", async () => {
        ...
        if (resultNo.success) {
            const namingPatterns = resultNo.patterns.filter((p) => p.kind === PatternKind.Naming);
        ...
        if (resultYes.success) {
            const namingPatterns = resultYes.patterns.filter((p) => p.kind === PatternKind.Naming);
            expect(namingPatterns.length).toBeGreaterThanOrEqual(1);
        }
    });
});
...
describe("JsonPatternExtractor - Pattern 字段质量", () => {
    ...
    it("产出的命名模式应包含正反例", async () => {
        ...
        expect(result.success).toBe(true);
        if (result.success) {
            const namingPattern = result.patterns.find((p) => p.kind === PatternKind.Naming);
...
describe("JsonPatternExtractor - JSON_EXTRACTOR_ANCHOR", () => {
    it("应导出包锚点常量", async () => {
        const { JSON_EXTRACTOR_ANCHOR } = await import("../src/predefined/json-extractor.js");
