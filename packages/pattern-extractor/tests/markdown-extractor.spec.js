...
import { MarkdownPatternExtractor, PatternKind, } from "../src/index.js";
// ============================================================
// §0 测试数据工厂
// ============================================================
/** 构造 ```json ... ``` 围栏包裹的文本 */
...
const VALID_SKILL_JSON = {
    ...
    expectedOutput: "CI 门禁结果报告：build ✅ / typecheck ✅ / test ✅ (N/N passed) / lint ✅ — 或具体失败原因",
...
describe("MarkdownPatternExtractor - Constructor & 基础属性", () => {
    ...
    it("自定义构造选项应覆盖默认值", () => {
        ...
        expect(result.diagnostics.some((d) => d.includes("JSON block"))).toBe(false);
    });
    it("支持关闭所有策略", () => {
        const ext = new MarkdownPatternExtractor({
            strategyJsonBlock: false,
...
describe("MarkdownPatternExtractor - 策略 1: JSON 块提取", () => {
    ...
    it("应提取单个 SkillTemplate JSON 块", () => {
        ...
        expect(p.sourceSpan.endLine).toBeGreaterThanOrEqual(p.sourceSpan.startLine);
        expect(p.elements.length).toBeGreaterThan(0);
        // tags 映射为 elements
        expect(p.elements.some((e) => e.type === "tag" && e.name === "test")).toBe(true);
    });
    ...
    it("应返回来自 JSON 块中的多个 ```json 块", () => {
        const input = jsonFence({ ...VALID_SKILL_JSON, name: "Block A" }) +
            "\n\nSome text\n\n" +
    ...
    it("应处理无效 JSON 块（解析失败不中断）", () => {
        ...
        expect(result.diagnostics.some((d) => d.includes("解析失败"))).toBe(true);
    });
    it("应跳过没有 name 字段的 JSON 对象", () => {
        ...
        expect(result.success).toBe(true);
        expect(result.patterns).toHaveLength(0);
        expect(result.diagnostics.some((d) => d.includes("缺少 name"))).toBe(true);
    });
...
describe("MarkdownPatternExtractor - 策略 2: P0-P9 格式提取", () => {
    ...
    it("应提取多个 P 编号段落", () => {
        const input = pSection(10, { name: "P10: CI Gate" }) +
            "\n\n" +
    ...
    it("步骤数不足 minStepsForP0P9 的段落应被过滤", () => {
        ...
        expect(result.success).toBe(true);
        expect(result.patterns).toHaveLength(0);
        expect(result.diagnostics.some((d) => d.includes("steps") && d.includes("< min"))).toBe(true);
    });
...
describe("MarkdownPatternExtractor - 去重归并", () => {
    it("同名候选应保留置信度最高的策略优先级版本", () => {
        ...
        const merged = result.patterns.find((p) => p.name.includes("P10") || p.name.includes("CI Gate"));
        if (merged) {
    ...
    it("disableMerge 应保留所有原始候选", () => {
        ...
        // 不归并 → 各自保留
        expect(result.patterns.length).toBeGreaterThanOrEqual(2);
        expect(result.diagnostics.some((d) => d.includes("去重归并"))).toBe(false);
    });
});
// ============================================================
// §9 置信度过滤 + 最大候选数
...
describe("MarkdownPatternExtractor - extract() 运行时选项覆盖", () => {
    ...
    it("extract options 应逐字段合并而非全量替换", () => {
        ...
        // 运行时只启用 P0-P9
        const result = ext.extract(pSection(10, { name: "P10: Test", steps: ["a", "b"] }), {
            strategyP0P9Format: true,
            minStepsForP0P9: 1,
        });
        expect(result.success).toBe(true);
...
//# sourceMappingURL=markdown-extractor.spec.js.map