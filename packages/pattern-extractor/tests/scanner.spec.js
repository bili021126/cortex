...
import { DEFAULT_SCAN_OPTIONS, DEFAULT_SCANNER_NAME, DEFAULT_SCANNER_DESCRIPTION, } from "../src/scanner.js";
...
describe("PatternScanner 接口（结构验证）", () => {
    ...
    it("canScan 应支持可选的 kind 参数", () => {
        const scanner = {
            ...
            canScan: (language, kind) => {
                if (!language)
                    return false;
                if (!kind)
                    return scanner.supportedLanguages.includes(language);
                return (scanner.supportedLanguages.includes(language) &&
...
describe("ScanOptions", () => {
    ...
    it("targetKinds 可设置为部分种类", () => {
        const partial = { targetKinds: [PatternKind.Naming] };
...
describe("ScanResult 判别联合", () => {
    ...
    const createSampleSummary = (overrides) => ({
        totalFiles: 1,
        filesScanned: 1,
        filesFailed: 0,
        rawPatterns: 1,
...
describe("ScanDiagnostic", () => {
    ...
    it("支持三种严重级别", () => {
        const info = { severity: "info", message: "信息", source: "s", timestamp: 0 };
        const warning = { severity: "warning", message: "警告", source: "s", timestamp: 0 };
        const error = { severity: "error", message: "错误", source: "s", timestamp: 0 };
