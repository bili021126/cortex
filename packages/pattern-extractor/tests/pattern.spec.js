...
describe("Pattern 接口（结构验证）", () => {
    ...
    it("应能携带 sourceSpan 定位信息", () => {
        const span = { startLine: 10, endLine: 35 };
        const pattern = {
    ...
    it("sourceSpan 支持可选的 startColumn 和 endColumn", () => {
        const span = { startLine: 10, endLine: 35, startColumn: 1, endColumn: 80 };
        const pattern = {
            ...createMinimalPattern(),
            sourceSpan: span,
        };
...
describe("SourceSpan", () => {
    it("startLine 和 endLine 是必需字段，应为正整数", () => {
        const span = { startLine: 1, endLine: 42 };
        expect(span.startLine).toBe(1);
        expect(span.endLine).toBe(42);
    });
    it("startColumn 和 endColumn 为可选字段", () => {
        const withColumns = { startLine: 1, endLine: 42, startColumn: 5, endColumn: 80 };
        const withoutColumns = { startLine: 1, endLine: 42 };
    ...
    it("仅提供 startLine 和 endLine 时不应报错", () => {
        const span = { startLine: 10, endLine: 20 };
...
describe("PatternExample", () => {
    it("code 和 isPositive 是必需字段", () => {
        const example = { code: "示例", isPositive: true };
        expect(example.code).toBe("示例");
        expect(example.isPositive).toBe(true);
    });
    it("description 为可选字段", () => {
        const withDesc = { code: "例", isPositive: true, description: "说明" };
        const withoutDesc = { code: "例", isPositive: true };
...
describe("PatternElement", () => {
    it("name、type、isPrimary 是必需字段", () => {
        const el = { name: "Agent", type: "interface", isPrimary: true };
        expect(el.name).toBe("Agent");
        expect(el.type).toBe("interface");
        expect(el.isPrimary).toBe(true);
    });
    it("signature 为可选字段", () => {
        const withSig = { name: "Agent", type: "interface", signature: "export interface Agent", isPrimary: true };
        const withoutSig = { name: "Agent", type: "interface", isPrimary: false };
...
describe("IPatternExtractor 接口", () => {
    ...
    it("canHandle 方法应返回布尔值", () => {
        const extractor = {
            ...
            canHandle: (language, kind) => language === "typescript" && kind === PatternKind.Structural,
        };
...
describe("IPatternValidator 接口", () => {
    ...
    it("validateMany 应返回与输入等长的结果列表", () => {
        const validator = {
            ...
            validateMany: (patterns) => patterns.map(() => ({ valid: true, errors: [], warnings: [] })),
        };
        const patterns = [
...
describe("ValidationResult / ValidationError", () => {
    ...
    it("ValidationError 区分 error 和 warning 严重程度", () => {
        const errorEntry = { field: "id", message: "缺少 id", severity: "error" };
        const warningEntry = { field: "name", message: "名称过长", severity: "warning" };
...
describe("IPatternMerger 接口", () => {
    it("merge 应接受 patterns 和可选的 threshold 参数", () => {
        ...
        const p1 = { id: "p-1", kind: PatternKind.Structural, name: "same", description: "d", tags: [], language: "ts", confidence: 0.9, source: "s", body: { rules: ["r"] }, elements: [{ name: "e", type: "t", isPrimary: true }], extractor: "e", extractedAt: 0, usageCount: 0, weight: 1 };
        const p2 = { id: "p-2", kind: PatternKind.Structural, name: "same", description: "d", tags: [], language: "ts", confidence: 0.8, source: "s", body: { rules: ["r"] }, elements: [{ name: "e", type: "t", isPrimary: true }], extractor: "e", extractedAt: 0, usageCount: 0, weight: 1 };
        const p3 = { id: "p-3", kind: PatternKind.Naming, name: "different", description: "d", tags: [], language: "ts", confidence: 0.7, source: "s", body: { rules: ["r"] }, elements: [{ name: "e", type: "t", isPrimary: true }], extractor: "e", extractedAt: 0, usageCount: 0, weight: 1 };
...
describe("PipelineStageContext", () => {
    ...
    it("metadata 可传递任意类型数据", () => {
        ...
        expect(ctx.metadata.tags).toEqual(["a", "b"]);
        expect(ctx.metadata.config.enable).toBe(true);
    });
});
// ============================================================
// §11 配置选项类型
