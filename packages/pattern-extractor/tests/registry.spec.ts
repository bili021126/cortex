...
import { PatternKind, type PatternExtractor } from "../src/extractor.js";
...
function createMockExtractor(
  ...
  return {
    ...
    canHandle: (language: string, kind: PatternKind) =>
      languages.includes(language) && kinds.includes(kind),
  };
}
...
describe("PatternExtractorRegistry - register / unregister", () => {
  let registry: PatternExtractorRegistry;
  ...
  it("注册同名提取器应覆盖旧实例", () => {
    ...
    // 旧的语言索引应被清理
    const tsExtractors = registry.queryByLanguageAndKind(
      "typescript",
      PatternKind.Structural,
    );
    expect(tsExtractors).toHaveLength(0);
    // 新的语言索引应就绪
    const pyExtractors = registry.queryByLanguageAndKind(
      "python",
      PatternKind.Naming,
    );
  ...
  it("unregister 后注册表应完全清理该提取器的所有索引", () => {
    registry.register(
      createMockExtractor("multi-lang", {
    ...
    expect(
      registry.queryByLanguageAndKind("javascript", PatternKind.Naming),
    ).toHaveLength(0);
    expect(
      registry.queryByLanguageAndKind("python", PatternKind.Structural),
    ).toHaveLength(0);
  });
...
describe("PatternExtractorRegistry - queryByLanguageAndKind", () => {
  ...
  it("应精确匹配语言和种类的交集", () => {
    ...
    //   交集 = {"ast-ext"}
    const result = registry.queryByLanguageAndKind(
      "typescript",
      PatternKind.Structural,
    );
  ...
  it("注册相同名称的提取器后索引应更新而不是累积", () => {
    // 先注册一个通用提取器
    registry.register(
      createMockExtractor("dynamic-ext", {
        languages: ["*"],
        kinds: [PatternKind.Structural],
      }),
    );
    ...
    expect(
      registry.queryByLanguageAndKind("typescript", PatternKind.Structural),
    ).toHaveLength(1); // ast-ext (精确匹配 typescript) — regex-ext 不匹配因精确匹配存在时无"*"回退
    ...
    expect(
      registry.queryByLanguageAndKind("typescript", PatternKind.Structural),
    ).toHaveLength(1); // 仍是 ast-ext

    // python+Naming 应包含 dynamic-ext
    expect(
      registry.queryByLanguageAndKind("python", PatternKind.Naming),
    ).toHaveLength(1);
    expect(
      registry.queryByLanguageAndKind("python", PatternKind.Naming)[0].name,
    ).toBe("dynamic-ext");
  });
});
...
describe("PatternExtractorRegistry - clear", () => {
  ...
  it("clear 后所有查询应返回空结果", () => {
    ...
    expect(
      registry.queryByLanguageAndKind("ts", PatternKind.Structural),
    ).toEqual([]);
  });
...
describe("PatternExtractorRegistry - 边界情况", () => {
  ...
  it("注册空种类的提取器应无种类索引", () => {
    ...
    expect(registry.size).toBe(1);
    expect(
      registry.queryByLanguageAndKind("anything", PatternKind.Structural),
    ).toHaveLength(0);
  });
  ...
  it("支持多种语言和多种种类的提取器应在多维度查询中返回", () => {
    ...
    // 不支持的种类查不到
    expect(
      registry.queryByLanguageAndKind("a", PatternKind.Dataflow),
    ).toHaveLength(0);
  });
...
describe("PatternExtractorRegistry - 索引一致性", () => {
  ...
  it("_byKind 不应包含空集合（验证内部实现正确清理）", () => {
    ...
    expect(
      registry.queryByLanguageAndKind("*", PatternKind.Architectural),
    ).toEqual([]);
  });
  ...
  it("多次注册-注销周期后索引应保持正确", () => {
    for (let cycle = 0; cycle < 5; cycle++) {
      registry.register(
        createMockExtractor(`cycle-${cycle}`, {
    ...
    for (let cycle = 0; cycle < 5; cycle++) {
      expect(
        registry.queryByLanguageAndKind(`lang-${cycle}`, PatternKind.Structural),
      ).toHaveLength(1);
    }
    ...
    // 偶数周期的仍可查询
    expect(
      registry.queryByLanguageAndKind("lang-0", PatternKind.Structural),
    ).toHaveLength(1);
    expect(
      registry.queryByLanguageAndKind("lang-2", PatternKind.Structural),
    ).toHaveLength(1);
    expect(
      registry.queryByLanguageAndKind("lang-4", PatternKind.Structural),
    ).toHaveLength(1);

    // 奇数周期的已不可查询
    expect(
      registry.queryByLanguageAndKind("lang-1", PatternKind.Structural),
    ).toHaveLength(0);
    expect(
      registry.queryByLanguageAndKind("lang-3", PatternKind.Structural),
    ).toHaveLength(0);
  });
});
