...
import { accumulateToolCalls, finalizeToolCalls, } from "../src/tool-call-stream.js";
describe("accumulateToolCalls", () => {
    ...
    it("空 deltas 返回原数组浅拷贝", () => {
        const acc = accumulateToolCalls([{ id: "x", function: { name: "t", arguments: "{}" } }], []);
        expect(acc).toHaveLength(1);
        expect(acc[0].id).toBe("x");
    });
    it("无 index 默认槽位 0", () => {
        let acc = accumulateToolCalls([], [
            { id: "call_x", function: { name: "ls", arguments: "" } },
        ]);
        acc = accumulateToolCalls(acc, [
            { function: { arguments: "{}" } },
        ]);
...
//# sourceMappingURL=tool-call-stream.test.js.map