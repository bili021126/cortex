...
import { queryLoop, extractHistory, agentTalkPersona, } from "@cortex/tui";
// ── Mock 工厂 ──────────────────────────────────────────────
function mockBridge(streamResult) {
    const defaultResult = {
    ...
    return {
        // LlmStreamBridge
        streamChat: vi.fn().mockImplementation(async (_model, _messages, _tools, onChunk) => {
            // 模拟流式 chunk
        ...
        executeAll: vi.fn().mockResolvedValue({ executed: 0, failed: 0 }),
    };
}
/**
 ...
 */
function mockBridgeWithTools(rounds) {
    let callCount = 0;
    const bridge = mockBridge();
    bridge.streamChat.mockImplementation(async (_model, _messages, _tools, onChunk) => {
        callCount++;
        if (callCount <= rounds) {
            // 返回 tool_calls 让循环继续
            return {
...
describe("queryLoop — 基本流程", () => {
    ...
    it("streamChat 被调用且传入正确的 model", async () => {
        ...
        const [model] = bridge.streamChat.mock.calls[0];
...
describe("queryLoop — 历史注入", () => {
    it("history 参数注入到 streamChat 的 messages 中", async () => {
        ...
        const callArgs = bridge.streamChat.mock.calls[0];
...
describe("queryLoop — hooks", () => {
    ...
    it("onPostProcessOutput 可以修改最终输出", async () => {
        ...
        const hooks = mockHooks({
            onPostProcessOutput: vi.fn(async (output) => output + " [modified]"),
        });
        const gen = queryLoop({
            input: "test",
            bridge,
    ...
    it("onPreModelRequest 可以修改 messages", async () => {
        const bridge = mockBridge();
        const hooks = mockHooks({
            onPreModelRequest: vi.fn(async (msgs) => {
                // 追加一条自定义消息
...
describe("queryLoop — 最大工具轮次", () => {
    it("达到 MAX_TOOL_ROUNDS(10) 后停止并调用 onMaxToolRounds", async () => {
        ...
        let callCount = 0;
        const bridge = mockBridge();
        bridge.streamChat.mockImplementation(async (_model, _messages, _tools, _onChunk) => {
            callCount++;
            return {
                content: "",
                tool_calls: [
...
describe("queryLoop — 模式 system prompt", () => {
    async function getSystemPrompt(mode, agent) {
        const bridge = mockBridge();
        const hooks = mockHooks();
        const gen = queryLoop({
            input: "test",
    ...
    it("party 模式包含群聊说明", async () => {
        const sysPrompt = await getSystemPrompt("party", "code");
        expect(sysPrompt).toContain("群聊模式");
        expect(sysPrompt).toContain("[格式]");
    });
    it("command 模式包含命令说明且不包含 agent 角色", async () => {
        const sysPrompt = await getSystemPrompt("command", "code");
...
//# sourceMappingURL=query-loop.test.js.map