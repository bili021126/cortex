...
import { EngineBridge, } from "@cortex/cli";
import { getFormatter, } from "@cortex/cli";
...
function ok(r) {
    if (!r.success)
        throw new Error(`预期成功，实际失败: ${r.error}`);
}
...
describe("C. cortex run — 输入→调度→输出", () => {
    ...
    it("C5. 纯文本输入走引擎调度路径", async () => {
        ...
        try {
            ...
            bridge.agentPool.register({ type: "analysis", maxInstances: 2 });
            bridge.agentPool.spawn("analysis", "test-instance");
            const { createRunHandler } = await import("@cortex/cli");
            const handler = createRunHandler(bridge);
            const result = await handler([tmpFile], {}, ctx);
...
describe("D. cortex agent — AgentPool 集成", () => {
    ...
    it("D2. list 列出已注册 Agent 类型", async () => {
        ...
        try {
            await bridge.ensureInitialized();
            bridge.agentPool.register({ type: AgentType.Fix, maxInstances: 2 });
            bridge.agentPool.register({ type: AgentType.Review, maxInstances: 2 });
            const { createAgentHandler } = await import("@cortex/cli");
            const handler = createAgentHandler(bridge);
    ...
    it("D3. spawn 创建 Agent 实例", async () => {
        ...
        try {
            await bridge.ensureInitialized();
            bridge.agentPool.register({ type: AgentType.Code, maxInstances: 2 });
            const { createAgentHandler } = await import("@cortex/cli");
            const handler = createAgentHandler(bridge);
    ...
    it("D4. spawn 达到上限时拒绝", async () => {
        ...
        try {
            await bridge.ensureInitialized();
            bridge.agentPool.register({ type: AgentType.Ops, maxInstances: 1 });
    ...
    it("D5. destroy 回收实例", async () => {
        ...
        try {
            await bridge.ensureInitialized();
            bridge.agentPool.register({ type: AgentType.Data, maxInstances: 2 });
...
describe("F. cortex memory — 记忆 CRUD 闭环", () => {
    ...
    it("F2. write → read 完整读写闭环", async () => {
        ...
        try {
            ...
            const handler = createMemoryHandler(bridge);
            // 写入
            const writeResult = await handler(["write", "greeting", "hello world"], { type: "episodic" }, ctx);
            ...
            // 读取（需要解析 ID）
            const readResult = await handler(["read", "greeting"], {}, ctx);
    ...
    it("F5. freeze 调用不存在的 freeze 方法（预期失败）", async () => {
        ...
        try {
            ...
            const handler = createMemoryHandler(bridge);
            // 先写入一条记忆
            const writeResult = await handler(["write", "freeze-key", "freeze-value"], {}, ctx);
            assertValidResult(writeResult);
            if (!writeResult.success)
                return; // 写入失败时跳过
            // 提取记忆 ID
...
describe("I. 输出格式契约", () => {
    it("I1. text 格式输出为非空字符串", () => {
        ...
        const result = { success: true, output: "hello", exitCode: 0 };
    ...
    it("I4. formatInfo 任意格式均返回非空字符串", () => {
        for (const f of ["text", "json", "color"]) {
...
describe("J. 错误处理与边界", () => {
    ...
    it("J2. 未找到的文件返回 exitCode=1", async () => {
        ...
        try {
            const { createRunHandler } = await import("@cortex/cli");
            const handler = createRunHandler(bridge);
            const result = await handler(["/nonexistent/path/file.txt"], {}, ctx);
            expect(result.success).toBe(false);
            expect(result.exitCode).toBe(1);
        }
        finally {
...
//# sourceMappingURL=cli-engine-integration.test.js.map