...
import { AgentType, getAgentTags, getAgentToolPermissions, AGENT_CHINESE_ROLE, CHINESE_NAME_TO_TYPE, } from "@cortex/shared";
// ── 辅助 ────────────────────────────────────────────
...
function ok(result) {
    if (!result.success)
        throw new Error(`Command failed: ${result.error ?? "unknown"}`);
}
/** 验证子命令路由返回预期的错误信息 */
function assertUnknownSubcommand(result, expectedAvailable) {
    ...
    it("分发到正确的处理器", async () => {
        ...
        r.register({
            ...
            handler: async (args) => {
                if (args[0] === "list") {
                    called = true;
                    return { success: true, data: ["a1"], exitCode: 0 };
                }
...
describe("命令注册表完整性", () => {
    it("13 个命令均可注册", () => {
        ...
        for (const n of names)
            r.register({ name: n, description: n, handler: noop });
...
describe("Agent 中英文名映射", () => {
    ...
    it("中文名到 AgentType 双向映射一致", () => {
        ...
        for (const cn of chineseNames) {
            if (cn === "钟离")
                continue; // strategist 双柱映射（钟离/霜凝→同一个type）
