...
function mockHooks(overrides) {
    return {
        onPreToolUse: vi.fn().mockResolvedValue("allow"),
        onPostToolUse: vi.fn(),
        ...overrides,
    };
}
function readCall(name = "read_file", id = "r1") {
    return { name, arguments: { path: "/tmp/x" }, id };
}
function writeCall(name = "write", id = "w1") {
    return { name, arguments: { path: "/tmp/x", content: "hello" }, id };
}
function bashCall(name = "bash", id = "b1") {
...
describe("streamExecuteTools", () => {
    ...
    describe("权限 deny", () => {
        it("L1 读操作被 deny 时，写入 denied 消息不执行", async () => {
            ...
            const hooks = mockHooks({
                onPreToolUse: vi.fn().mockResolvedValue("deny"),
            });
            const gen = streamExecuteTools([readCall("read_file", "r1")], AGENT, mockBridge(), messages, hooks);
        ...
        it("L3 写操作被 deny 时，写入 denied 消息不执行", async () => {
            ...
            const gen = streamExecuteTools([writeCall("write", "w1")], AGENT, mockBridge(), messages, hooks);
            const { result } = await collectEvents(gen);
            const results = result;
            expect(results[0].output).toBe("denied by hook");
    ...
    describe("权限 skip", () => {
        it("skip 时注入 [skipped by user] 消息并标记 success=true", async () => {
            const messages = [];
            const hooks = mockHooks({
                onPreToolUse: vi.fn().mockResolvedValue("skip"),
            });
            const gen = streamExecuteTools([readCall("glob", "r1")], AGENT, mockBridge(), messages, hooks);
            const { result } = await collectEvents(gen);
            const results = result;
    ...
    describe("读操作执行", () => {
        it("allow 时 yield tool_start 事件", async () => {
            ...
            const gen = streamExecuteTools([readCall("read_file", "r1")], AGENT, bridge, [], mockHooks());
        ...
        it("执行后 yield tool_result 事件", async () => {
            ...
            const gen = streamExecuteTools([readCall("grep", "r1")], AGENT, bridge, [], mockHooks());
            const { events } = await collectEvents(gen);
            const resultEv = events.find((e) => e.type === "tool_result");
            expect(resultEv).toBeDefined();
            expect(resultEv.output).toBe("hello world");
        });
        ...
        it("读操作结果注入 messages", async () => {
            ...
            const gen = streamExecuteTools([readCall("read_file", "r1")], AGENT, bridge, messages, mockHooks());
    ...
    describe("写操作执行", () => {
        it("L2 写操作串行执行", async () => {
            ...
            const gen = streamExecuteTools([writeCall("write", "w1"), writeCall("update_memory", "w2")], AGENT, bridge, [], mockHooks());
            const { events, result } = await collectEvents(gen);
            const results = result;
        ...
        it("L3 不可逆操作正确标记 level=3", async () => {
            const gen = streamExecuteTools([bashCall("bash", "b1")], AGENT, mockBridge(), [], mockHooks());
            const { result } = await collectEvents(gen);
            const results = result;
    ...
    describe("混合批次", () => {
        it("读写混合：先完成所有读，再写", async () => {
            ...
            const bridge = mockBridge({
                executeToolCall: vi.fn().mockImplementation(async (name) => {
    ...
    describe("推理内容传递", () => {
        it("reasoningContent 注入到 assistant 消息", async () => {
            const messages = [];
            const gen = streamExecuteTools([readCall("read_file", "r1")], AGENT, mockBridge(), messages, mockHooks(), "I should read this file");
            await collectEvents(gen);
            const asstMsg = messages.find((m) => m.role === "assistant");
            expect(asstMsg).toBeDefined();
            expect(asstMsg.reasoning_content).toBe("I should read this file");
        });
    });
    describe("错误处理", () => {
        it("executeToolCall 抛异常时调用 onToolError 并返回失败结果", async () => {
            ...
            const gen = streamExecuteTools([readCall("read_file", "r1")], AGENT, bridge, [], hooks);
            const { result } = await collectEvents(gen);
            const results = result;
            expect(results[0].success).toBe(false);
...
//# sourceMappingURL=streaming-tool-executor.test.js.map