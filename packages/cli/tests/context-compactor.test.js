...
import { estimateTokens, compactMessages, } from "@cortex/tui";
// ── 工厂函数 ──────────────────────────────────────────────
...
function asstWithTools(content, toolCalls) {
    return {
        role: "assistant",
        content,
...
function tool(id, content) {
    return { role: "tool", content, tool_call_id: id };
}
function userTurn(question, answer) {
...
describe("L3 — 压缩旧工具调用对", () => {
    ...
    it("将旧 assistant(tool_calls)+tool 对压缩为摘要", async () => {
        ...
        // 摘要消息存在
        const summaryMsg = result.messages.find((m) => m.role === "assistant" && m.content.includes("[已执行工具"));
        expect(summaryMsg).toBeDefined();
    });
    it("保留区内的 tool 对不被压缩", async () => {
        const triggerOptsKeep = {
            ...triggerOpts,
            contextLimit: 128000,
            targetRatio: 0.001,
            keepRecentTurns: 1,
