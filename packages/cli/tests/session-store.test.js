...
function msg(role = "user", content = "hello") {
    return { role, content };
}
...
describe("tui/session-store", () => {
    ...
    describe("saveSession", () => {
        ...
        it("truncates history exceeding MAX_HISTORY to last 200 entries", () => {
            const bigHistory = Array.from({ length: 300 }, (_, i) => msg("user", `message-${i}`));
        ...
        it("does not truncate history at exactly MAX_HISTORY", () => {
            const exactHistory = Array.from({ length: MAX_HISTORY }, (_, i) => msg("user", `msg-${i}`));
        ...
        it("preserves history under MAX_HISTORY unchanged", () => {
            const smallHistory = [msg("user", "one"), msg("assistant", "two")];
    ...
    describe("loadSession", () => {
        ...
        it("returns null when mode is missing", () => {
            const cortexDir = path.join(tmpDir, ".cortex");
            fs.mkdirSync(cortexDir, { recursive: true });
            fs.writeFileSync(path.join(tmpDir, SESSION_REL), JSON.stringify({ agent: "meta", history: [] }), "utf-8");
            expect(loadSession(tmpDir)).toBeNull();
        });
        it("returns null when mode is not a string", () => {
            const cortexDir = path.join(tmpDir, ".cortex");
            fs.mkdirSync(cortexDir, { recursive: true });
            fs.writeFileSync(path.join(tmpDir, SESSION_REL), JSON.stringify({ mode: 123, agent: "meta", history: [] }), "utf-8");
            expect(loadSession(tmpDir)).toBeNull();
        });
        it("returns null when history is missing", () => {
            const cortexDir = path.join(tmpDir, ".cortex");
            fs.mkdirSync(cortexDir, { recursive: true });
            fs.writeFileSync(path.join(tmpDir, SESSION_REL), JSON.stringify({ mode: "chat", agent: "meta" }), "utf-8");
            expect(loadSession(tmpDir)).toBeNull();
        });
        it("returns null when history is not an array", () => {
            const cortexDir = path.join(tmpDir, ".cortex");
            fs.mkdirSync(cortexDir, { recursive: true });
            fs.writeFileSync(path.join(tmpDir, SESSION_REL), JSON.stringify({ mode: "chat", agent: "meta", history: "oops" }), "utf-8");
            expect(loadSession(tmpDir)).toBeNull();
        });
        it("returns null for invalid mode value", () => {
            const cortexDir = path.join(tmpDir, ".cortex");
            fs.mkdirSync(cortexDir, { recursive: true });
            fs.writeFileSync(path.join(tmpDir, SESSION_REL), JSON.stringify({ mode: "bogus", agent: "meta", history: [] }), "utf-8");
            expect(loadSession(tmpDir)).toBeNull();
        });
        it.each(VALID_MODES)("accepts valid mode: %s", (mode) => {
            const cortexDir = path.join(tmpDir, ".cortex");
            fs.mkdirSync(cortexDir, { recursive: true });
            fs.writeFileSync(path.join(tmpDir, SESSION_REL), JSON.stringify({
                mode,
                agent: AgentType.Meta,
                history: [],
                talkTrio: false,
    ...
    describe("round-trip (save → load)", () => {
        ...
        it("truncates oversized history on save, then loads truncated version", () => {
            const bigHistory = Array.from({ length: 250 }, (_, i) => msg(i % 2 === 0 ? "user" : "assistant", `msg-${i}`));
