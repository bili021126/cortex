 ...
 * @see CLI 设计文档 §4.16
 */
import { HealthChecker } from "@cortex/doctor";
...
function _parseDoctorOpts(options) {
...
/** 根据健康报告状态判定退出码 */
function _doctorExitCode(report) {
    if (report.status === "healthy")
        return 0;
    if (report.status === "warning")
        return 0; // warning 不阻断，但建议关注
...
/** 计算总健康分（所有检查器评分的平均值） */
function computeTotalScore(report) {
...
function formatTextReport(report) {
    ...
    for (const check of report.checks) {
        ...
        // 列出 error/fatal 级别发现
        const criticalFindings = check.findings.filter((f) => f.severity === "error" || f.severity === "fatal");
