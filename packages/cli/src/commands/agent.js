...
import { AGENT_CHINESE_ROLE, AgentStatus, AgentType, CHINESE_NAME_TO_TYPE, getAgentTags, getAgentToolPermissions } from "@cortex/shared";
import * as fs from "node:fs";
import * as path from "node:path";
/** Agent 实例持久化文件（跨进程共享 spawn/list 状态） */
const INSTANCES_FILE = path.join(process.cwd(), ".cortex", "agent-instances.json");
function readPersistedInstances() {
    try {
...
function writePersistedInstances(instances) {
...
export function createAgentHandler(bridge) {
...
async function dispatchAgent(bridge, args, options) {
...
function safePool(pool) {
    return {
        count: (type) => pool?.count?.(type) ?? 0,
        getStatuses: (type) => pool?.getStatuses?.(type) ?? [],
        hasAwake: (type) => pool?.hasAwake?.(type) ?? false,
        spawn: (type, instanceId) => pool?.spawn?.(type, instanceId) ?? false,
        destroy: (type, instanceId) => pool?.destroy?.(type, instanceId),
        register: (config) => pool?.register?.(config),
    };
}
/** 将单个 Strategist 实例追加为行 */
function _appendStrategistRow(ctx, id, agent) {
    const statusStr = agent.status === AgentStatus.Awake ? "awake" : String(agent.status);
    const tags = getAgentTags()[AgentType.Strategist] ?? [];
    if (ctx.statusFilter && statusStr !== ctx.statusFilter)
        return;
    if (ctx.verbose) {
...
function _buildAgentRows(ctx) {
    ...
    for (const type of agentTypes) {
        ...
        if (verbose) {
            const permissions = (getAgentToolPermissions()[type] ?? []).join(", ");
...
function handleAgentList(pool, options, bridge) {
    ...
    const rows = _buildAgentRows({
        ...
        tally: (inc, awake) => { totalInstances += inc; if (awake)
            totalAwake++; },
    });
    return {
        success: true,
...
function _resolveStrategistInspect(bridge, typeName) {
...
function _inspectRegularAgent(pool, agentType) {
    const p = safePool(pool);
    return {
        agentType,
        ...
        permissions: getAgentToolPermissions()[agentType] ?? [],
    };
}
function handleAgentInspect(pool, typeName, bridge) {
    if (!typeName) {
...
function _spawnInstances(p, agentType, config) {
    const { typeName, count } = config;
    let spawned = 0;
    for (let i = 0; i < count; i++) {
        const instanceId = `${typeName}-${Date.now()}-${i}`;
        if (p.spawn(agentType, instanceId))
            spawned++;
    }
    return spawned;
}
function handleAgentSpawn(pool, typeName, options) {
    if (!typeName) {
...
function handleAgentDestroy(pool, typeName, options) {
    if (!typeName) {
    ...
    if (instanceId) {
        ...
        if (persisted[agentType] && persisted[agentType] > 0) {
            persisted[agentType]--;
            if (persisted[agentType] === 0)
                delete persisted[agentType];
            writePersistedInstances(persisted);
        }
