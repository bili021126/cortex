...
function _buildCommandList(commands, aliases) {
...
/** 总览帮助文本模板 */
function _buildOverview(commands, aliases) {
...
/** 构建单个命令的帮助文本 */
function _buildCommandHelp(cmd) {
    const lines = ["", `命令: cortex ${cmd.name}`];
    if (cmd.alias)
        lines.push(`别名: ${cmd.alias}`);
    ...
    if (cmd.subcommands && Object.keys(cmd.subcommands).length > 0) {
        ...
        for (const [subName, sub] of Object.entries(cmd.subcommands)) {
            ...
            if (sub.usage)
                lines.push(`    用法: ${sub.usage}`);
        }
    }
    return lines.join("\n");
}
export function createHelpHandler(registry) {
    const handler = async (args, _options, _context) => {
        const cmdName = args[0];
        if (cmdName) {
            const cmd = registry.find(cmdName);
            if (cmd)
                return { success: true, output: _buildCommandHelp(cmd), exitCode: 0 };
