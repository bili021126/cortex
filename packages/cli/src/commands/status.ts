  ...
  const handler: CommandHandler = async (args, _options, _context): Promise<CommandResult> => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        lines.push(`  daemon:    ✅ 在线${j?.uptime ? `（uptime ${Math.round(j.uptime / 60)}min）` : ""}`);
      } else {
        lines.push(`  daemon:    ⚠️ 响应异常（HTTP ${res.status}）`);
      }
    } catch {
      lines.push("  daemon:    ❌ 离线（127.0.0.1:3210 不可达）");
    }
    ...
    try {
      ...
      if (fsMod.existsSync(reportPath)) {
        ...
        lines.push(`  评测:      ✅ ${passed}/${report.results.length} 通过（${report.generatedAt}）`);
      } else {
        lines.push("  评测:      ⚪ 无报告（cortex eval --run 生成）");
      }
    } catch {
      lines.push("  评测:      ⚪ 无报告");
    }
    ...
    try {
      ...
      lines.push(`  桌面端:    ${count > 0 ? `✅ 运行中（${count} 进程）` : "⚪ 未运行"}`);
    } catch {
      lines.push("  桌面端:    ⚪ 未知");
    }
