...
/** 读取任务文件内容 */
function _readTaskFile(filePath) {
    return fs.readFileSync(path.resolve(filePath), "utf-8");
}
/** 构建 CLI 提交的 TaskNode */
function _createSubmittedTaskNode(content, agentType, label) {
    return {
...
export function createTaskHandler(bridge) {
...
/** 等待模式：立即执行并返回结果 */
async function _handleWaitExecution(svc, taskNode) {
    const report = await svc.scheduler.executeAll();
    const taskResult = report.results.find((r) => r.nodeId === taskNode.id);
    return {
...
async function handleTaskSubmit(svc, filePath, options) {
    if (!filePath) {
        return { success: false, error: "请指定任务文件。用法: cortex task submit <file>", exitCode: 1 };
    }
    let content;
    try {
        content = _readTaskFile(filePath);
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        return { success: false, error: `读取文件失败: ${msg}`, exitCode: 1 };
    }
    const agentType = (options["agent"] ?? options["a"]);
    ...
    if (options["wait"] || options["w"])
        return await _handleWaitExecution(svc, taskNode);
    return {
        success: true,
...
function handleTaskList(board, options) {
...
/** 格式化任务状态输出字符串 */
function _formatTaskStatus(node, verbose) {
    return [
        `任务 ID:    ${node.id}`,
        `状态:       ${node.status}`,
        `类型:       ${node.type}`,
...
function handleTaskStatus(board, taskId, options) {
    if (!taskId) {
...
function handleTaskCancel(board, taskId) {
    if (!taskId) {
...
async function handleTaskRedo(svc, taskId) {
    if (!taskId) {
    ...
    const report = await svc.scheduler.executeAll();
    const redoResult = report.results.find((r) => r.nodeId === taskId);
    return {
        success: report.completed > 0,
