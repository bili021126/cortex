...
function handleDocConvert(filePath, options, _context) {
    if (!filePath) {
    ...
    try {
        ...
        const documentMode = options["document"];
...
/** doc serve 的请求处理器工厂 */
function _createDocRequestHandler(rootDir, port) {
    return (req, res) => {
...
function handleDocServe(dirPath, options, _context) {
...
/** 检查 Markdown 标题层级跳跃 */
function _checkHeadings(content) {
    const issues = [];
...
function handleDocCheck(filePath, options, _context) {
    if (!filePath) {
    ...
    try {
        ...
        const rules = (options["rules"] ?? "links,headings").split(",");
        const issues = [];
        if (rules.includes("headings"))
            issues.push(..._checkHeadings(content));
        if (rules.includes("links"))
            issues.push(..._checkLinks(content));
        return {
