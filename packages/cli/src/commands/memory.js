...
export function createMemoryHandler(bridge) {
    const handler = async (args, options, _context) => {
...
async function handleMemoryWrite(memory, args) {
    const { rawKey, rawValue, options } = args;
    if (!rawKey || !rawValue) {
        return { success: false, error: "请指定 key 和 value。用法: cortex memory write <key> <value>", exitCode: 1 };
    }
    const memoryType = options["type"]?.toUpperCase() ?? "EPISODIC";
    const agentType = options["agent"] ?? "butler";
    ...
    const id = await memory.write({
        ...
        source: { agentType: agentType, taskId: "" },
        weight,
        content_hash: "",
    });
    return {
...
async function handleMemorySearch(memory, query, options) {
    ...
    return {
        ...
        data: entries,
        output: entries.length > 0
            ? entries.map((e) => `[${e.id}] ${e.summary} (${e.kind}, w:${e.weight})`).join("\n")
            : `未找到匹配 "${query}" 的记忆`,
        exitCode: 0,
    };
}
function handleMemoryLink(memory, args) {
    const { src, tgt, options } = args;
    if (!src || !tgt) {
        return { success: false, error: "请指定源和目标 ID。用法: cortex memory link <src> <tgt>", exitCode: 1 };
    }
    const linkType = options["type"]?.toUpperCase() ?? "DERIVED_FROM";
    const link = memory.link(src, tgt, LinkType[linkType] ?? LinkType.DerivedFrom);
    if (!link) {
...
function handleMemoryArchive(memory, id) {
    if (!id)
        return { success: false, error: "请指定记忆 ID。用法: cortex memory archive <id>", exitCode: 1 };
    const ok = memory.archive(id);
    return { success: ok, output: ok ? `✓ 记忆已归档: ${id}` : `归档失败: ${id}`, exitCode: ok ? 0 : 1 };
}
function handleMemoryFreeze(memory, id) {
    if (!id)
        return { success: false, error: "请指定记忆 ID。用法: cortex memory freeze <id>", exitCode: 1 };
    const ok = memory.freeze(id);
    return { success: ok, output: ok ? `✓ 记忆已冻结: ${id}` : `冻结失败: ${id}`, exitCode: ok ? 0 : 1 };
}
function handleMemoryObliterate(memory, id) {
    if (!id)
        return { success: false, error: "请指定记忆 ID。用法: cortex memory obliterate <id>", exitCode: 1 };
    const ok = memory.obliterate(id);
    return { success: ok, output: ok ? `✓ 记忆已湮灭: ${id}` : `湮灭失败: ${id}`, exitCode: ok ? 0 : 1 };
}
async function handleMemoryFlush(memory) {
    await memory.flush();
    return { success: true, output: "✓ 持久化已刷新", exitCode: 0 };
}
/** 聚合记忆条目统计——按类型、状态分组并计算平均权重 */
function _aggregateStats(entries, memorySize, isPersisted) {
    const byType = {};
    const byState = {};
    let totalWeight = 0;
...
async function handleMemoryStats(memory, options) {
