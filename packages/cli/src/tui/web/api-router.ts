...
export class APIRouter {
  ...
  private _handleExecute(req: IncomingMessage, res: ServerResponse): boolean {
    ...
    // 异步处理请求体完成后执行，捕获异常返回 500
    this._handleExecuteAsync(req, res, body).catch((err) => {
      if (!res.headersSent) {
        this._sendJson(res, 500, { error: "Internal server error", detail: err instanceof Error ? err.message : String(err) });
      }
    });
    return true;
  }
