...
export async function tuiReplHandler(
  ...
  rl.on("line", async (line) => {
    ...
    try { await _dispatchTuiMode(session, input, bridge, registry, context, cmdCtx.projectRoot); }
...
async function _dispatchTuiMode(
  ...
  switch (m) {
    case "command":
      ...
      break;
    case "chat": {
      const result = await consumeGenerator(chatMode(input, bridge, a, h), tuiEventBus);
      if (result) { h.push({ role: "user", content: input }, { role: "assistant", content: result }); writeln(result); }
      break;
    }
    case "talk": {
      if (session.talkTrio) {
        const result = await consumeGenerator(talkTrioMode(input, bridge, h), tuiEventBus);
        if (result) { h.push({ role: "user", content: input }, { role: "assistant", content: result }); writeln(result); }
      } else {
        const result = await consumeGenerator(talkMode(input, bridge, session.agent, h), tuiEventBus);
        if (result) { h.push({ role: "user", content: input }, { role: "assistant", content: result }); writeln(result); }
      }
      break;
    }
    case "plan": {
      ...
      const result = await consumeGenerator(planMode(input, bridge, a, ps, h), tuiEventBus);
      if (result) { savePlanState(projectRoot, ps); writeln(result); }
      break;
    }
    case "party": {
      const result = await consumeGenerator(partyMode(input, bridge, session.partyRoster, h), tuiEventBus);
      if (result) { h.push({ role: "user", content: input }, { role: "assistant", content: result }); writeln(result); }
      break;
    }
  }
