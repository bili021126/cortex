...
export async function* queryLoop(p: QueryLoopParams): AsyncGenerator<TuiEvent, string, void> {
  ...
  while (toolRound < MAX_TOOL_ROUNDS) {
    ...
    if (signal?.aborted || (streamError && streamError.name === "AbortError")) {
      const se = streamError as unknown as Error | null;
      const errMsg = se?.message?.slice(0, 120) ?? String(streamError);
      process.stderr.write(`[query-loop] interrupted@stream abort=${signal?.aborted ?? false} streamError=${se?.name ?? ""} msg=${errMsg}\n`);
      yield { type: "interrupted", agent } as TuiEvent;
      return finalOutput;
    }
    if (streamError) {
