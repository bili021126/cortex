...
import { ENV_DEEPSEEK_BASE_URL, ENV_DEEPSEEK_CHAT_MODEL, ENV_DEEPSEEK_CYRENE_CHAT_MODEL, ENV_DEEPSEEK_REASONER_MODEL, ENV_DEEPSEEK_REASONING_EFFORT, ENV_DEEPSEEK_API_KEY, ENV_DEEPSEEK_CYRENE_API_KEY, ENV_DEEPSEEK_CHAT_API_KEY, ENV_DEEPSEEK_REASONER_API_KEY, ENV_PM_MASTER_KEY, DEFAULT_LLM_BASE_URL, DEFAULT_LLM_CHAT_MODEL, DEFAULT_LLM_REASONER_MODEL, LLM_KEY_NAMES, } from "@cortex/config";
/** 按优先级解析 API Key：vault → 专用环境变量 → DEEPSEEK_API_KEY 兜底 */
function resolveKey(pmStore, pmKey, envVarName, fallbackKey) {
    if (pmStore) {
        try {
 ...
 * 仅 API Key 按三路独立解析。
 */
export async function bootstrapLlm() {
    const llms = new Map();
    // 共享的模型/URL 配置
    ...
    const llmReasoningEffort = process.env[ENV_DEEPSEEK_REASONING_EFFORT] || undefined;
    ...
    const adapter = (key, label, chatModelOverride, extra) => new LlmAdapter({
        apiKey: key,
        baseUrl: llmBaseUrl,
    ...
    if (cyreneKey)
        llms.set(LLM_KEY_NAMES.CYRENE, adapter(cyreneKey, "cyrene", llmCyreneChatModel));
    ...
    if (chatKey)
        llms.set(LLM_KEY_NAMES.CHAT, adapter(chatKey, "chat"));
    ...
    if (reasonerKey)
        llms.set(LLM_KEY_NAMES.REASONER, adapter(reasonerKey, "reasoner", undefined, { reasoningEffort: llmReasoningEffort }));
    return llms;
}
/** 检查是否有任何 API Key 可用 */
export function hasAnyLlmKey() {
    return !!(process.env[ENV_DEEPSEEK_API_KEY] ||
        process.env[ENV_DEEPSEEK_CYRENE_API_KEY] ||
        process.env[ENV_DEEPSEEK_CHAT_API_KEY] ||
        process.env[ENV_DEEPSEEK_REASONER_API_KEY]);
}
/** 启用 API 审计日志 */
export function enableLlmAudit() {
