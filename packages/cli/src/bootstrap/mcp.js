...
import { SearchAggregator, McpSearchBackend, DdgSearchBackend, } from "@cortex/platform";
import { ENV_CORTEX_NO_SEARCH, ENV_VITEST, FILE_CORTEX_AGENTS_JSON, resolveConfigDataDir, loadConfigDomain, } from "@cortex/config";
/**
 * 初始化 MCP 后端与搜索聚合器。
 *
...
function _loadMcpConfigs(configRoot) {
    // 1) 优先 mcpServers 域（config 包 data/mcp-servers.json）
    try {
        const dataDir = resolveConfigDataDir();
        const readFile = (fp) => nodeFs.readFileSync(fp, "utf-8");
        const servers = loadConfigDomain("mcpServers", readFile, dataDir);
        if (servers) {
    ...
    if (nodeFs.existsSync(configPath)) {
        ...
        if (searchProviders?.backends && Array.isArray(searchProviders.backends)) {
            return searchProviders.backends
...
/** 启动 MCP 后端并构建搜索聚合器 */
async function _startMcpBackends(configs, toolkit) {
...
export async function bootstrapMcp(toolkit, configRoot) {
