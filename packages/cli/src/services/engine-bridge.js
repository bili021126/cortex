...
import { Scheduler, bootstrapEngine, } from "@cortex/engine";
...
export class EngineBridge {
    ...
    _pool = new MiniAgentPool();
    config;
    dbPath;
    engineConfig;
    _bootstrapConfig;
    constructor(config, dbPath, engineConfig) {
        this.config = config;
        this.dbPath = dbPath;
     ...
     */
    async rebootstrapIfNeeded(workspaceRoot) {
    ...
    async _ensureBootstrapped() {
        if (this.ctx.bootstrapped)
            return this.ctx;
        if (!this._bootstrapConfig) {
            throw new Error("[EngineBridge] ensureBootstrapped() 需要先调用 setBootstrapConfig() 设置 LlmAdapter/Toolkit");
        }
        const { llms, toolkit, projectRoot, dbPath, engineConfig, workspaceRoot } = this._bootstrapConfig;
     ...
     */
    async ensureBootstrappedContext() {
     ...
     * @returns LLM 完整响应
     */
    async streamChat(model, messages, tools, onChunk, opts) {
        const l = this.llm;
        if (!l)
            throw new Error("LLM 未配置——请设置 DEEPSEEK_API_KEY 环境变量");
        return await l.chatStream(model, messages, tools, onChunk, opts?.reasoningEffort);
    }
    /** 提交任务节点到 TaskBoard（ICortexApi） */
    async submitTask(node) {
     ...
     * @returns 完整执行报告
     */
    async executeWithStream(nodes, onEvent) {
        const ctx = await (this.ctx.bootstrapped ? this._ensureBootstrapped() : this.ensureInitialized());
     ...
     * @returns LLM 返回的文本内容
     */
    async directChat(systemPrompt, messages, opts) {
        const l = this.llm;
        if (!l)
            throw new Error("LLM 未配置——请设置 DEEPSEEK_API_KEY 环境变量");
        const model = opts?.model ?? l.chatModel;
        const resp = await l.chat(model, [{ role: "system", content: systemPrompt }, ...messages], undefined, opts?.reasoningEffort);
        return resp.content ?? "";
    }
    ...
    async getMemoryStore() {
        ...
        if (!ctx.memoryStore)
            throw new Error("Engine not initialized: memoryStore missing");
        return ctx.memoryStore;
    }
    async getScheduler() {
        const ctx = await this.ensureInitialized();
        if (!ctx.scheduler)
            throw new Error("Engine not initialized: scheduler missing");
        return ctx.scheduler;
    }
    async getTaskBoard() {
        const ctx = await this.ensureInitialized();
        if (!ctx.taskBoard)
            throw new Error("Engine not initialized: taskBoard missing");
        return ctx.taskBoard;
    }
    async getObserver() {
        const ctx = await this.ensureInitialized();
        if (!ctx.pipelineObserver)
            throw new Error("Engine not initialized: pipelineObserver missing");
        return ctx.pipelineObserver;
    }
    async getConfirmGate() {
        const ctx = await this.ensureInitialized();
        if (!ctx.confirmGate)
            throw new Error("Engine not initialized: confirmGate missing");
