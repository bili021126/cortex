...
import { AgentType, TAG_VOCABULARY, AGENT_TAGS, AGENT_CHINESE_ROLE, CHINESE_NAME_TO_TYPE, AGENT_DISPLAY, AGENT_DISPLAY_BY_TYPE, AGENT_DISPLAY_FALLBACK, CHAT_AGENT_ALIASES, AGENT_TOOL_PERMISSIONS, resolveAgentPermissions, getAgentTags, setAgentTags, getAgentToolPermissions, setAgentToolPermissions, setAgentRegistry, buildChineseRoleMap, } from "@cortex/shared";
...
describe("agent-registry — AGENT_TOOL_PERMISSIONS / resolveAgentPermissions", () => {
    ...
    it("Resolver: 未知 AgentType 返回空数组", () => {
        const perms = resolveAgentPermissions("nonexistent");
...
describe("agent-registry — 运行时覆写", () => {
    ...
    it("setAgentRegistry 同时注入 tags + toolPermissions", () => {
        setAgentRegistry({ code: ["registry_tag"] }, { code: ["registry_perm"] }, ["registry_tag"]);
        const tags = getAgentTags();
        const perms = getAgentToolPermissions();
...
describe("agent-registry — 集成：AGENT_DEFS 统一派生", () => {
    ...
    it("AGENT_DISPLAY_BY_TYPE 与 AGENT_DISPLAY (string-key) 完全对齐", () => {
        for (const agentType of Object.values(AgentType)) {
            ...
            const byString = AGENT_DISPLAY[agentType];
    ...
    it("CHAT_AGENT_ALIASES 包含所有 AgentType 的 string-key", () => {
        for (const agentType of Object.values(AgentType)) {
            expect(CHAT_AGENT_ALIASES[agentType]).toBe(agentType);
        }
    });
    ...
    it("AGENT_DISPLAY (string-key) 键集合与 AgentType 枚举值一致", () => {
        const enumValues = Object.values(AgentType).filter(v => typeof v === "string");
