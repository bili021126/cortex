...
import { AgentType, AgentStatus, ReversibilityLevel, LinkType, PipelinePriority, LockType, PlatformKind, TAG_VOCABULARY, AGENT_TAGS, AGENT_TOOL_PERMISSIONS, getTagVocabulary, } from "@cortex/shared";
...
describe("@cortex/shared v2.0 types", () => {
    ...
    it("SemanticState (v3) is three-state: Active/Archived/Obliterated", () => {
        const validStates = ["Active", "Archived", "Obliterated"];
    ...
    it("NodeResult captures success/failure with optional output and error", () => {
        const success = { nodeId: "n1", success: true, output: "ok" };
        const failure = { nodeId: "n2", success: false, error: "fail" };
    ...
    it("RiskDomain covers file_write/shell_exec/network/config_change", () => {
        const domains = ["file_write", "shell_exec", "network", "config_change"];
