...
function addPackage(
  ...
  // package.json 处理
  const defaultPkgJson: Record<string, unknown> = {
    name: `@cortex/${name}`,
    version: "0.1.0",
    private: true,
    type: "module",
  ...
  let finalPkgJson: Record<string, unknown>;
  ...
  if (overrides?.hasPositioningDoc !== false) {
    fs.writeFileSync(
      path.join(pkgDir, "PACKAGE_POSITIONING.md"),
      `# @cortex/${name} — 定位文档\n\n补足说明...\n`,
      "utf-8",
    );
  }
...
describe("HealthChecker", () => {
  ...
  it("检测缺少 name 字段的包", async () => {
    ...
    expect(
      pkgCheck!.findings.some((f) => f.message.includes("缺少字段: name")),
    ).toBe(true);
  });
  ...
  it("检测缺少 scripts 字段的包", async () => {
    ...
    expect(pkgCheck!.passed).toBe(false);
    expect(
      pkgCheck!.findings.some((f) => f.message.includes("缺少字段: scripts")),
    ).toBe(true);
  });
  ...
  it("检测 scripts 缺少 build 字段", async () => {
    ...
    expect(pkgCheck!.passed).toBe(false);
    expect(
      pkgCheck!.findings.some((f) => f.message.includes("缺少字段: scripts.build")),
    ).toBe(true);
  });
  ...
  it("检测 type 字段不为 module", async () => {
    ...
    expect(pkgCheck).toBeDefined();
    expect(pkgCheck!.passed).toBe(false);
    const typeFindings = pkgCheck!.findings.filter(
      (f) => f.message.includes("type") && f.message.includes('"module"'),
    );
  ...
  it("检测缺少 PACKAGE_POSITIONING.md 的包", async () => {
    ...
    expect(
      posCheck!.findings.some((f) => f.id.startsWith("POS-DOC-")),
    ).toBe(true);
  });
  ...
  it("检测测试文件首行缺少 @ci 标注", async () => {
    ...
    expect(
      headerCheck!.findings.some((f) => f.id.startsWith("TEST-HDR-")),
    ).toBe(true);
  });
  ...
  it("registerChecker 可以注册新检查器", () => {
    ...
    const customChecker: IChecker = {
      name: "custom-check",
      description: "自定义测试检查器",
      async check(_projectRoot: string): Promise<CheckResult> {
        return {
          checker: this.name,
          passed: true,
          findings: [],
  ...
  it("registerChecker 同名覆盖已有检查器", () => {
    ...
    const overrideChecker: IChecker = {
