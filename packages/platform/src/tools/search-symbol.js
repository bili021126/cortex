...
export function createTool(ctx) {
    return new LocalTool("search_symbol", ToolCategory.Search, "Scan a TS/JS source file and return a structured symbol table (functions, classes, interfaces, enums, type aliases, exports) with names, positions, and signatures.", {
        type: "object",
        properties: {
        ...
        const kindFilter = params.kind_filter?.toLowerCase() ?? "all";
        try {
...
function extractSymbols(filePath, content, kindFilter) {
    ...
    const filterSet = filterAll ? new Set() : new Set(kindFilter.split(",").map((s) => s.trim()));
    const push = (entry) => {
    ...
    const visitor = (node) => {
        ...
        // export 声明 (re-exports)
            ...
            if (clause && ts.isNamedExports(clause)) {
                ...
                push({
                    name: node.moduleSpecifier ? `from "${node.moduleSpecifier.text}"` : "(bare re-export)",
                    kind: "export",
                    line: pos.line,
                    col: pos.col,
                    exported: true,
...
//# sourceMappingURL=search-symbol.js.map