...
export function createTool(ctx) {
    return new LocalTool("resolve_import", ToolCategory.Read, "Resolve an import specifier to an absolute file path. Given a source file and its import statement (e.g. './foo', '@/utils/bar'), returns the resolved target file path with candidate extensions tried.", {
        type: "object",
        properties: {
        ...
        const sourceFile = ctx.resolvePath(params.source_file);
        const specifier = params.import_specifier.trim();
        if (!specifier) {
        ...
        try {
            ...
            const result = await tryResolve(ctx, sourceDir, specifier);
            return {
                success: true,
...
async function tryResolve(ctx, sourceDir, specifier) {
    const tried = [];
    // 1. 相对路径 / 绝对路径
