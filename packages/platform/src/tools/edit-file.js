...
export function createTool(ctx) {
    return new LocalTool("edit_file", ToolCategory.Write, "Perform a precise search-and-replace in a file. Finds the exact old_text string and replaces it with new_text. Unlike write_file (full overwrite), this only touches the matched portion. Use replace_all=true to replace all occurrences.", {
        type: "object",
        properties: {
            file_path: {
                type: "string",
        ...
        try {
            ...
            if (!replaceAll && occurrences > 1) {
                const preview = content.slice(Math.max(0, content.indexOf(oldText) - 40), content.indexOf(oldText) + oldText.length + 40);
                return {
                    success: false,
