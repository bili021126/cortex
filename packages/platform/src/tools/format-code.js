...
const ALLOWED_FORMATTERS = ["prettier", "eslint", "biome"];
export function createTool(ctx) {
    return new LocalTool("format_code", ToolCategory.Write, "Format code in a file or directory using prettier, eslint, or biome. Safer than running shell commands directly.", {
        type: "object",
        properties: {
            file_path: {
                type: "string",
        ...
        // 白名单校验
        const selected = formatter === "both"
            ? ["prettier", "eslint"]
            : ALLOWED_FORMATTERS.includes(formatter)
                ? [formatter]
                : [];
