...
export function createTool(ctx) {
    return new LocalTool("search_code", ToolCategory.Search, "Search for code patterns in the project.", {
        type: "object",
        properties: {
        ...
        try {
            ...
            try {
                output = await ctx.fs.execFile("rg", ["--line-number", "--max-count", "30", "--no-heading", query], { cwd: searchRoot, timeout: ctx.toolTimeouts.searchCode });
            }
            catch (e) {
                const err = e;
                ...
                if (err.status === 1) {
                    ...
                    if (!process.env.VITEST) {
                        console.warn(`[toolkit] search_code: rg failed (exit ${err.status ?? "?"}), falling back to grep. stderr: ${stderr.slice(0, 200)}`);
                    }
                    try {
 ...
 */
async function grepFallback(ctx, rootDir, query) {
    const results = [];
    const lowerQuery = query.toLowerCase();
    const walk = async (dir, depth) => {
        if (depth > 4 || results.length > 30)
            return;
        let entries;
        try {
