...
export function createTool(ctx) {
    return new LocalTool("parse_ast", ToolCategory.Read, "Parse a source file and return its AST (Abstract Syntax Tree). Uses TypeScript Compiler API for .ts/.tsx/.js/.jsx files; tree-sitter for other languages (pending).", {
        type: "object",
        properties: {
        ...
        const maxDepth = Math.min(params.max_depth ?? 6, 12);
...
/** 按文件扩展名选择解析器 */
function getParserByExtension(ext) {
...
function parseAstWithTS(content, filePath) {
...
function serializeAST(sourceFile, maxDepth, includeText, sourceText) {
...
/** 递归 AST 节点 → 纯 JSON */
function nodeToJSON(node, depth, maxDepth, includeText, lines) {
...
/** 收集 AST 顶层统计信息 */
function collectASTStats(sourceFile) {
    const stats = {
        totalNodes: 0,
        functions: 0,
        classes: 0,
...
//# sourceMappingURL=parse-ast.js.map