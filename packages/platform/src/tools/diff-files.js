...
export function createTool(ctx) {
    return new LocalTool("diff_files", ToolCategory.Read, "Compare two files and return a unified diff. Useful for reviewing changes, verifying edits, or comparing outputs. Pure in-memory LCS algorithm—no external diff dependency.", {
        type: "object",
        properties: {
            file_a: {
                type: "string",
        ...
        const pathA = ctx.resolvePath(params.file_a);
        const pathB = ctx.resolvePath(params.file_b);
...
function computeUnifiedDiff(linesA, linesB, pathA, pathB, context) {
...
function buildHunks(edits, context, linesA, linesB) {
    const hunks = [];
    let i = 0;
