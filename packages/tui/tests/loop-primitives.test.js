...
function createMockBridge() {
    ...
    return {
        ...
        emit: vi.fn((e, d) => handlers.get(e)?.forEach(fn => fn(d))),
        send: vi.fn(),
        write: vi.fn(),
        end: vi.fn(),
    };
...
describe("Tui loop primitives", () => {
    it("mock bridge emit → on 同步", () => {
        ...
        let msg = "";
        bridge.on("message", (d) => { msg = d.text; });
    ...
    it("mock bridge on 可被多次调用（多 handler）", () => {
        const bridge = createMockBridge();
        const a = [], b = [];
        bridge.on("num", (n) => a.push(n));
        bridge.on("num", (n) => b.push(n * 2));
        bridge.emit("num", 3);
        expect(a).toEqual([3]);
        expect(b).toEqual([6]);
    });
});
//# sourceMappingURL=loop-primitives.test.js.map