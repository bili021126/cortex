...
describe("TuiEventBus", () => {
    function emit(bus, overrides = {}) {
        bus.emit({ type: "tool_result", agent: "code", tool: "read_file", success: true, output: "data", durationMs: 0, ...overrides });
    }
    ...
    it("通配符 * 匹配所有事件", () => {
        ...
        emit(bus);
        bus.emit({ type: "tool_start", agent: "fix", tool: "search_code" });
