 ...
 */
export function loadPlanState(projectRoot) {
    try {
 ...
 */
export function savePlanState(projectRoot, state) {
    try {
 ...
 */
export function clearPlanState(projectRoot) {
    try {
        const filePath = nodePath.join(projectRoot, PLAN_STATE_FILE);
        if (nodeFs.existsSync(filePath)) {
 ...
 * - .approve → 执行计划
 */
export async function* planMode(input, bridge, agent, planState, history) {
    const hooks = {
 ...
 *   3. 文本中嵌入的 JSON 数组片段
 */
function _extractPlanNodes(text) {
    // 尝试 1：整个文本就是 JSON 数组
    try {
        const direct = JSON.parse(text);
        if (Array.isArray(direct))
            return _normalizeNodes(direct);
    }
    catch { /* 继续 */ }
