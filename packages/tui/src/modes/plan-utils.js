 ...
 * 返回绝对路径或 null。
 */
export function extractWorkspacePath(input) {
    const patterns = [
...
/** 展示意图解析结果 */
export function displayClarification(cl) {
    const actionIcon = {
 ...
 */
export async function clarifyAndConfirm(input, metaAgent, askUser) {
    if (!askUser)
        return input; // 无交互能力时跳过确认
 ...
 */
function _matchChineseConfirm(raw) {
    const patterns = [
...
export function formatPlanTree(nodes) {
    ...
    const icon = (node) => {
        ...
        if (t === "code" || t === "implementation")
            return "🔨";
        if (t === "review")
            return "🔍";
        if (t === "analysis" || t === "research")
            return "🧠";
        if (t === "fix" || t === "bugfix")
            return "💊";
        if (t === "inspect" || t === "inspector")
            return "🔭";
        if (t === "ops" || t === "deploy")
            return "⚓";
        if (t === "doc-govern" || t === "audit")
            return "📜";
        if (t === "browser")
            return "🎆";
        if (t === "loop" || t === "pattern_scan")
            return "🔮";
        return "📌";
    };
    const render = (node, depth, isLast, prefix) => {
