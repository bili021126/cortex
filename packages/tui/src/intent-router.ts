...
export function classifyIntent(input: string): UserIntent {
  ...
  // L3: 系统命令——以已知命令名开头
  const cmdFirst = /^(ls|dir|cat|find|grep|cd|pwd|mkdir|rm |cp |mv |git |npm |node |tsc |vitest |pnpm |yarn |npx |code |open |start |stop |clear|cls|echo |type |more|sort|where|which|chmod|chown|df |du |ps |kill |top |ping |curl|wget|ssh |scp |docker |kubectl|helm )/i;
  if (cmdFirst.test(input)) return "command";

  // 命令后缀（中文指令）
  const cmdSuffix = /^(打开|启动|停止|重启|关闭|清[除理屏]|列出|查看|搜索|查找|运行|执行|测试|构建|编译|部署|安装|卸载|更新|升级|回滚|切换|进入|退出|保存|删除|移动|复制|备份|恢复) /.test(input);
  ...
  // plan 审批/确认——计划待审批时的"执行""好的""确认"等都走 task
  if (/^(执行|好的|确认|可以|行|开始|跑|go|yes|ok|approve|run|start)/i.test(input)) return "task";

  // 闲聊——包含"聊天""聊""说话""交流"等
  if (/聊[天聊]|说话|交流|谈心|解闷/.test(input) && input.length < 40) return "chat";

  // L2: task 关键词——明确的开发任务表达
