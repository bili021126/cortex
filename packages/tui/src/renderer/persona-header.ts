...
/**
 * 遗留兼容：渲染单人角色头（直接 writeln 到终端）。
 * 新代码请使用 personaHeader.update()。
 */
export function renderPersonaHeader(agent: AgentTypeEnum, mode: ReplMode): void {
  const rows = renderSingleLine(agent, mode, Math.min(terminalWidth(), 80));
  for (const row of rows) process.stdout.write(row + "\n");
}

/**
 * 遗留兼容：渲染多人角色头（直接 writeln 到终端）。
 * 新代码请使用 personaHeader.updateMulti()。
 */
export function renderMultiPersonaHeader(agents: AgentTypeEnum[], modeLabel: string): void {
  const rows = renderMultiLine(agents, modeLabel, Math.min(terminalWidth(), 80));
  for (const row of rows) process.stdout.write(row + "\n");
}
