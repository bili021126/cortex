...
export class FsmValidator {
     ...
     */
    checkConnectivity(states, transitions, initialState) {
    ...
    validate(ast) {
        ...
        for (const stateId of ast.stateMap.keys()) {
            if (finalSet.has(stateId))
                continue; // final states are allowed to have no outgoing
        ...
        const eventMap = new Map(); // state → (event → transitionId)
        let deterministic = true;
        for (const [stateId, transitions] of ast.adjacencyList) {
            const eventToTransition = new Map();
            for (const t of transitions) {
