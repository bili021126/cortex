...
import { StateMachine, TransitionError, GuardError } from "../src/runtime/state-machine.js";
...
describe("StateMachine -- advanced", () => {
    it("should support custom transition table via options", () => {
        ...
        const machine = new StateMachine(basicDef, "idle", { transitionTable: customTable });
    ...
    it("should pass context through to transition record", () => {
        const machine = new StateMachine(basicDef, "idle");
        const ctx = { userId: "u1", reason: "manual" };
        machine.dispatch("start", ctx);
    ...
    it("should throw TransitionError on invalid event for current state", () => {
        ...
        expect(() => machine.dispatch("finish")).toThrow(TransitionError);
    });
    it("should retain error cause in TransitionError when guard rejects", () => {
        ...
        const machine = new StateMachine({ ...guardDef, transitions: [guardDef.transitions[0]] }, "idle", { guards });
        expect(() => machine.dispatch("start")).toThrow(TransitionError);
        try {
            machine.dispatch("start");
        }
        catch (e) {
    ...
    it("should throw GuardError when guard function throws", () => {
        ...
        const machine = new StateMachine({ ...guardDef, transitions: [guardDef.transitions[0]] }, "idle", { guards });
        expect(() => machine.dispatch("start")).toThrow(GuardError);
    });
    it("should execute sync action on transition", () => {
        let sideEffect = "";
        const actions = new ActionRegistry();
        actions.register("onGo", (ctx) => {
            sideEffect = `went to ${ctx.target ?? "b"}`;
        });
        const machine = new StateMachine(actionDef, "a", { actions });
        machine.dispatch("go", { target: "B" });
    ...
    it("should fire-and-forget async actions in sync dispatch", async () => {
        ...
        expect(() => machine.dispatch("go")).not.toThrow();
    ...
    it("should return false from can() for undefined transitions", () => {
        const machine = new StateMachine(basicDef, "idle");
        expect(machine.can("nonexistent")).toBe(false);
    });
    ...
    it("should handle isFinal correctly with multiple final states", () => {
        ...
        const machine = new StateMachine(multiFinalDef, "start");
...
describe("HistoryRecorder", () => {
    ...
    it("should handle empty recorder gracefully", () => {
        ...
        expect(recorder.getFrom("a")).toEqual([]);
        expect(recorder.getTo("b")).toEqual([]);
        expect(recorder.getByEvent("c")).toEqual([]);
