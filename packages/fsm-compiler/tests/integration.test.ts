...
const taskNodeDef: FsmDefinition = {
  id: "task_node",
...
type TaskState = "pending" | "running" | "paused" | "completed" | "failed" | "cancelled";
type TaskEvent = "execute" | "pause" | "resume" | "complete" | "fail" | "cancel";
...
describe("Full Pipeline: Task Node FSM", () => {
  ...
  it("should execute the happy path through the StateMachine runtime", () => {
    const machine = new StateMachine<TaskState, TaskEvent>(taskNodeDef, "pending");
  ...
  it("should enforce guard conditions", () => {
    ...
    const machine = new StateMachine<TaskState, TaskEvent>(taskNodeDef, "pending", { guards });
  ...
  it("should execute actions on transitions", () => {
    ...
    actionRegistry.register("onFail", (ctx) => {
      actions.push(`failed: ${(ctx as { reason?: string }).reason ?? "unknown"}`);
    });
    ...
    const machine = new StateMachine<TaskState, TaskEvent, any>(
      taskNodeDef, "pending",
      { guards, actions: actionRegistry },
    );
  ...
  it("should create a complete audit trail with HistoryRecorder", () => {
    const recorder = new HistoryRecorder<TaskState, TaskEvent, { step: number }>({ maxRecords: 10 });
    const machine = new StateMachine<TaskState, TaskEvent, any>(taskNodeDef, "pending");

    // Run through a sequence, recording each step
    const steps = [
      { event: "execute" as TaskEvent, ctx: { step: 1 } },
      { event: "pause" as TaskEvent, ctx: { step: 2 } },
      { event: "resume" as TaskEvent, ctx: { step: 3 } },
      { event: "complete" as TaskEvent, ctx: { step: 4 } },
    ];

    for (const { event, ctx } of steps) {
      machine.dispatch(event, ctx);
      // Manually record into HistoryRecorder for audit trail
      recorder.record(machine.history[machine.history.length - 1] as any);
    }
  ...
  it("should serialize and restore machine state correctly across full lifecycle", () => {
    ...
    const restored = StateMachine.deserialize<TaskState, TaskEvent, void>(snapshot, taskNodeDef);
...
describe("Error recovery and edge cases", () => {
  it("should not change state on invalid transition attempt", () => {
    ...
    expect(() => machine.dispatch("complete" as any)).toThrow();
    expect(machine.current).toBe("pending");
  });
