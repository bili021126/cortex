...
describe('TaskBoard', () => {
    ...
    it('claim 普通节点成功', () => {
        ...
        const claimed = board.claim('n1', 'inspector');
    ...
    it('claim 重复认领返回 null', () => {
        board.addNode(makeNode('n1', { tags: ['audit'] }));
        board.claim('n1', 'review');
        const second = board.claim('n1', 'code');
        expect(second).toBeNull();
    });
    it('release 普通节点回到 pending', () => {
        board.addNode(makeNode('n1', { tags: ['audit'] }));
        board.claim('n1', 'review');
        const ok = board.release('n1', 'review');
        expect(ok).toBe(true);
        const node = board.getNode('n1');
    ...
    it('complete 普通节点置为 done', () => {
        board.addNode(makeNode('n1', { tags: ['audit'] }));
        board.claim('n1', 'review');
        board.complete('n1', 'review', true, 'ok');
        const node = board.getNode('n1');
    ...
    it('complete 失败节点置为 failed', () => {
        board.addNode(makeNode('n1', { tags: ['audit'] }));
        board.claim('n1', 'review');
        board.complete('n1', 'review', false, undefined, '出错了');
        const node = board.getNode('n1');
    ...
    describe('multi-perspective', () => {
        it('多种 Agent 可并行认领', () => {
            ...
            const c1 = board.claim('mp', 'review');
            expect(c1).not.toBeNull();
            const c2 = board.claim('mp', 'inspector');
            expect(c2).not.toBeNull();
            expect(board.getNode('mp').claimedBy).toHaveLength(2);
        });
        it('同类型不可重复认领', () => {
            board.addNode(makeNode('mp', { needsMultiPerspective: true, tags: ['audit'] }));
            board.claim('mp', 'review');
            const second = board.claim('mp', 'review');
            expect(second).toBeNull();
        });
        it('等齐所有认领后自动 done', () => {
            board.addNode(makeNode('mp', { needsMultiPerspective: true, tags: ['audit', 'inspect'] }));
            board.claim('mp', 'review');
            board.claim('mp', 'inspector');
            board.complete('mp', 'review', true, 'review ok');
            // 尚未等齐
            expect(board.getNode('mp').status).not.toBe('done');
            board.complete('mp', 'inspector', true, 'inspect ok');
            // 等齐
