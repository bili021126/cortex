import { PipelineEventType, PipelinePriority, getAgentTags } from "@cortex/shared";
/**
 * TaskBoard —— 任务板
 *
 * @contract 模块边界契约
...
export class TaskBoard {
    ...
    claim(nodeId, agentType) {
        ...
        if (!node)
            return null;
        // 标签匹配
        const agentTags = getAgentTags()[agentType];
        if (!node.tags.some((t) => agentTags.includes(t)))
            return null;
     ...
     */
    findPending(agentType) {
        const agentTags = getAgentTags()[agentType];
        return Array.from(this.nodes.values()).filter((n) => {
            if (!n.tags.some((t) => agentTags.includes(t)))
                return false;
    ...
    complete(nodeId, agentType, success, output, error) {
        ...
        if (!node.needsMultiPerspective) {
            // 普通节点：去重在前
            if (node.results.some((r) => r.agentType === agentType))
                return;
            node.results.push({
                nodeId,
                agentType,
    ...
    allPerspectivesComplete(nodeId) {
        const node = this.nodes.get(nodeId);
        if (!node?.needsMultiPerspective)
            return false;
        const claimed = new Set(node.claimedBy);
    ...
    getPendingNodes() {
        return Array.from(this.nodes.values()).filter((n) => n.status === "pending" || n.status === "claimed");
    }
    /**
    ...
    _reportInvariant(source, message, details) {
        const violation = { source, message, details };
        if (this._observer) {
            this._observer.emit({
