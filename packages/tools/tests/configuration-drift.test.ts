...
import {
  collectDependencies,
  detectDrift,
} from "../src/configuration-drift.js";
import type { DepEntry, DepGroup } from "../src/configuration-drift.js";
...
function makeEntry(overrides: Partial<DepEntry> = {}): DepEntry {
  return {
    depName: "dep-a",
    pkg: "cli",
    pkgName: "@cortex/cli",
...
function makeWorkspaceEntry(depName: string, pkg: string): DepEntry {
  return makeEntry({
    depName,
    pkg,
    version: "workspace:*",
...
describe("真实项目漂移检测", () => {
  it("当前项目扫描 → 无漂移或仅有已知例外", () => {
    ...
    if (realDrifts.length > 0) {
      console.warn(
        `⚠️ 检测到 ${realDrifts.length} 处版本漂移:`,
        realDrifts.map((g) => g.depName).join(", "),
      );
    }
    // 当前项目预期干净
