...
import {
  ...
  checkTimeout,
  updateStaleCount,
  syncGovernanceToMemory,
} from "../src/index.js";
import type { AmendmentProposal, AmendmentStatus } from "../../shared/src/index.js";
import { InMemoryMemoryStore } from "../../memory/src/index.js";
import type { IMemoryStore } from "../../shared/src/index.js";
...
function setup() {
  ...
  fs.writeFileSync(
    path.join(tmpDir, "docs", "constitution", "Cortex 概念顶层设计 v2.7.0.md"),
    `# Cortex Constitution\n\n**版本**: v2.7.0\n\n## 原则一 | 项目本质定位 | 不可变\n\nCortex 是智能体治理框架。\n\n## 原则二 | 架构解耦 | 可变\n\n引擎与治理层通过记忆系统解耦。`,
    "utf-8",
  );
}
...
describe("Governance -> Memory Sync", () => {
  ...
  beforeEach(() => {
    setup();
    store = new InMemoryMemoryStore() as unknown as IMemoryStore;
    store.init(":memory:");
  });
  afterEach(() => cleanup());
  ...
  it("syncGovernanceToMemory writes proposals to memory", async () => {
    ...
    const memories = await store.read({ kind: "Governance" } as any);
