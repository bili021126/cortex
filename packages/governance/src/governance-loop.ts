...
export function judgeProposals(rootDir: string): BatchJudgment[] {
  const constitutionPath = findConstitutionPath(rootDir);
  let constitution: string;
  try {
    constitution = fs.readFileSync(constitutionPath, "utf-8");
  } catch (e) {
    throw new Error(
      `[GovernanceLoop] 无法读取宪法文件: ${constitutionPath} — ${e instanceof Error ? e.message : String(e)}`,
      { cause: e },
    );
  }
...
export function applyApproved(
  ...
  const result = applyAmendment(proposal, findConstitutionPath(rootDir));
