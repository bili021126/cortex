...
import type { NotificationEvent, NotificationSemantics, SemanticNotification } from "../src/index.js";
...
describe("Notification Semantic Layer", () => {
  describe("withSemantics() 语义增强", () => {
    ...
    it("不修改原始事件对象", () => {
      ...
      expect((event as any).semantics).toBeUndefined(); // 原始事件未被修改
    });
  });
  ...
  describe("SEMANTIC_DESCRIPTIONS 描述表", () => {
    it("所有语义层级都有非空描述", () => {
      const levels: NotificationSemantics[] = ["FYI", "WARNING", "DECISION_REQUIRED"];
