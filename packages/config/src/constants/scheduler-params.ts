...
// R13-harness：懒求值覆写口（模块期冻结的 env 对运行期无效——用例 setupEnv 晚于模块加载）
export function nodeDispatchTimeoutMs(): number {
  return Number(process.env["CORTEX_NODE_DISPATCH_TIMEOUT_MS"]) || NODE_DISPATCH_TIMEOUT_MS;
}
