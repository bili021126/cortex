// ============================================================
// @cortex/plugin-runner/plugin-loader —— 插件加载容器
//
// PluginLoader 职责：
//   1. 按 engine-plugins.json 声明加载插件
//   2. 拓扑排序（依赖关系）
//   3. 依次 init() → postInit 钩子 → start()
...
export interface EnginePluginLoadConfig {
  ...
  /**
   * postInit 钩子——全部插件 init() 完成后执行。
   * 用于跨插件织入（如 Scheduler 注册 Agent），替代旧版 PluginLoader 内部硬编码。
   */
  onPostInit?: (ctx: PluginContext, plugins: Map<string, EnginePlugin>) => Promise<void>;
}
...
export class PluginLoader {
  ...
  async load(config: EnginePluginLoadConfig): Promise<PluginContainer> {
    ...
    // §5 postInit 钩子——跨插件织入（由消费方注入，插件体系本身不耦合具体业务）
    if (config.onPostInit) {
      await config.onPostInit(ctx, this._plugins);
    }
