...
import type { PluginSchema } from "../src/types.js";
...
describe("s.enum()", () => {
  it("valid enum value passes", () => {
    const v = s.enum(["low", "medium", "high"] as const);
...
describe("composeSchemas()", () => {
  it("merges validateConfig from all schemas", () => {
    const a: PluginSchema = { name: "A", validateConfig: () => ["errA"] };
    const b: PluginSchema = { name: "B", validateConfig: () => ["errB"] };
  ...
  it("validateInput takes first defined", () => {
    const a: PluginSchema = { name: "A", validateConfig: () => [], validateInput: () => ["from A"] };
    const b: PluginSchema = { name: "B", validateConfig: () => [], validateInput: () => ["from B"] };
    expect(composeSchemas("c", a, b).validateInput!("x")).toEqual(["from A"]);
  });

  it("validateOutput takes first defined", () => {
    const a: PluginSchema = { name: "A", validateConfig: () => [] };
    const b: PluginSchema = { name: "B", validateConfig: () => [], validateOutput: () => ["from B"] };
