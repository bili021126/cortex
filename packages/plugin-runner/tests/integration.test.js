...
class GreeterPlugin extends AbstractPlugin {
    ...
    async execute(ctx) {
        ...
        const name = ctx.payload?.name ?? "World";
...
class ConfigCheckedPlugin extends AbstractPlugin {
    name = "config-checked";
    version = "1.0.0";
    description = "需要 apiKey 配置";
    async execute(ctx) {
        ctx.output = { ok: true };
    }
}
function createHarness(timeout = 30_000) {
...
describe("全链路集成测试", () => {
    ...
    describe("4. 异常隔离与资源清理", () => {
        it("execute 抛异常时 destroy 仍被调用（资源不泄漏）", async () => {
            ...
            expect(failPlugin.callOrder).toContain("destroy");
            expect(failPlugin.callOrder.indexOf("destroy")).toBeGreaterThan(failPlugin.callOrder.indexOf("execute"));
        });
        ...
        it("一个插件的失败不应抛异常到调用方（异常隔离）", async () => {
            const { registry, runner } = harness;
            registry.register(new FailOnExecutePlugin());
            await expect(runner.execute("fail-execute", makeContext())).resolves.toBeDefined();
        });
        it("插件未注册时不应抛异常到调用方", async () => {
            const { runner } = harness;
            await expect(runner.execute("nonexistent", makeContext())).resolves.toBeDefined();
    ...
    describe("7. PluginConfigManager 与全链路集成", () => {
        it("PluginConfigManager 的配置应可注入到插件 init", async () => {
            ...
            await plugin.init(greeterConfig);
    ...
    describe("8. 端到端场景", () => {
        it("注册 → 配置校验 → 依赖解析 → 批量执行 → 状态查询 → shutdown 全流程", async () => {
            const { registry, validator, runner } = harness;
            // ─── 阶段1: 注册 Schema ───
            validator.registerSchema(definePluginSchema("greeter", {
                config: s.object({
                    enabled: s.boolean().optional(),
                    greeting: s.string().optional(),
                }),
            }));
            validator.registerSchema(definePluginSchema("calculator", {
                config: s.object({
                    enabled: s.boolean().optional(),
