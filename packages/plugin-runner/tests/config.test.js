...
import { PluginConfigManager, loadPluginConfig, createPluginConfig, } from "../src/config.js";
...
describe("PluginConfigManager — config.ts", () => {
    ...
    describe("fromFile", () => {
        ...
        it("显式传入不存在的路径时应抛 ENOENT 错误", async () => {
            ...
            await expect(PluginConfigManager.fromFile(nonExistent)).rejects.toThrow();
        });
        it("显式传入路径但文件内容为非法 JSON 时应抛 SyntaxError", async () => {
            const filePath = resolve(tempDir, "bad.json");
            await writeFile(filePath, "not-json");
            await expect(PluginConfigManager.fromFile(filePath)).rejects.toThrow(SyntaxError);
        });
    ...
    describe("fromJson", () => {
        ...
        it("非法 JSON 应抛 SyntaxError", () => {
            expect(() => PluginConfigManager.fromJson("{broken")).toThrow(SyntaxError);
        });
        it("非法 JSON 错误消息应包含来源路径（如提供）", () => {
            expect(() => PluginConfigManager.fromJson("{{", undefined, "/path/to/file.json")).toThrow(/file\.json/);
        });
    ...
    describe("sourcePath", () => {
        ...
        it("fromJson 传入 sourcePath 应正确设置", () => {
            const config = PluginConfigManager.fromJson('{"plugins":{}}', {}, "/custom/path.json");
    ...
    describe("ENV: 环境变量解析", () => {
        ...
        it("递归解析嵌套对象中的 ENV: 占位符", () => {
            ...
            const cfg = config.getPluginConfig("p");
            expect(cfg.nested.inner).toBe(ENV_VAL);
        });
        it("递归解析数组中的 ENV: 占位符", () => {
            const config = new PluginConfigManager({
                plugins: {
            ...
            const cfg = config.getPluginConfig("p");
            expect(cfg.hosts[0]).toBe(ENV_VAL);
            expect(cfg.hosts[1]).toBe("static-value");
        });
    ...
    describe("loadPluginConfig", () => {
        it("应委托给 PluginConfigManager.fromFile 并返回相同结果", async () => {
            ...
            try {
                ...
                await writeFile(filePath, JSON.stringify({ plugins: { p: { enabled: true } } }));
                const direct = await PluginConfigManager.fromFile(filePath);
                const viaFn = await loadPluginConfig(filePath);
                expect(viaFn.getPluginConfig("p")).toEqual(direct.getPluginConfig("p"));
            }
            finally {
                await rm(tempDir, { recursive: true, force: true });
            }
