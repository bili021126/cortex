...
class MockPluginB extends AbstractPlugin {
    name = "plugin-b";
    version = "2.0.0";
    description = "Plugin B (feature)";
    dependencies = ["plugin-a"];
    tags = ["feature", "transformer"];
    async execute(_ctx) {
        // no-op
    }
}
class MockPluginC extends AbstractPlugin {
    name = "plugin-c";
    version = "1.5.0";
    description = "Plugin C (feature)";
    dependencies = ["plugin-a"];
    tags = ["feature", "validator"];
    async execute(_ctx) {
        // no-op
    }
}
class MockPluginD extends AbstractPlugin {
    name = "plugin-d";
    version = "0.5.0";
    description = "Plugin D (高级)";
    dependencies = ["plugin-b", "plugin-c"];
    tags = ["advanced", "pipeline"];
    async execute(_ctx) {
        // no-op
    }
}
...
describe("PluginRegistry — registry.ts", () => {
    ...
    describe("register()", () => {
        ...
        it("重复注册同名插件应抛 Error（含明确消息）", () => {
            registry.register(pluginA);
            expect(() => registry.register(pluginA)).toThrow('[PluginRegistry] 重复注册: "plugin-a" 已存在');
        });
        it("重复注册同名但不同实例也应抛 Error", () => {
            registry.register(pluginA);
            const anotherA = new MockPluginA();
            expect(() => registry.register(anotherA)).toThrow('[PluginRegistry] 重复注册: "plugin-a" 已存在');
        });
        it("注册 null / undefined 应抛 Error（非空 name 断言）", () => {
            expect(() => registry.register(null)).toThrow("插件必须具有非空的 name 属性");
        });
        it("注册 name 为空字符串的插件应抛 Error", () => {
            const nameless = new NamelessPlugin();
            expect(() => registry.register(nameless)).toThrow("插件必须具有非空的 name 属性");
        });
    ...
    describe("hasCycle()", () => {
        ...
        it("无依赖时应返回 false", () => {
            registry.register(pluginA);
            registry.register(new StandalonePlugin());
            expect(registry.hasCycle()).toBe(false);
        });
    ...
    describe("discover()", () => {
        ...
        it("不匹配任何文件的 glob 应返回空数组", async () => {
            const result = await registry.discover("./nonexistent-dir-12345/*.plugin.js");
            expect(result).toEqual([]);
        });
    });
    // ── registerFromGlob() ──
    describe("registerFromGlob()", () => {
        ...
        it("不匹配任何文件应返回 0 且不注册任何插件", async () => {
            const count = await registry.registerFromGlob("./nonexistent-dir-xyz/*.plugin.js");
            expect(count).toBe(0);
            expect(registry.size).toBe(0);
        });
        it("文件发现失败时不应抛出异常（静默跳过）", async () => {
            // 使用不合法的文件路径模式
            const count = await registry.registerFromGlob("\0invalid|path*.plugin.ts");
            // 不抛异常即可
    ...
    describe("集成场景", () => {
        ...
        it("注册 → 拓扑排序 → 按批次执行的经典流程", () => {
            ...
            expect(batchIndex.get("plugin-a")).toBeLessThan(batchIndex.get("plugin-b"));
            expect(batchIndex.get("plugin-a")).toBeLessThan(batchIndex.get("plugin-c"));
            expect(batchIndex.get("plugin-b")).toBeLessThan(batchIndex.get("plugin-d"));
            expect(batchIndex.get("plugin-c")).toBeLessThan(batchIndex.get("plugin-d"));
        });
