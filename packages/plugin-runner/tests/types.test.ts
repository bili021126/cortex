...
describe("类型定义 — types.ts", () => {
  ...
  it("PluginResult 应区分成功和失败", () => {
    const success: PluginResult = { success: true, durationMs: 10 };
    const failure: PluginResult = { success: false, error: "fail", durationMs: 5 };
  ...
  it("PluginConfig 应包含 enabled 和可选字段", () => {
    const config: PluginConfig = { enabled: true, timeout: 5000 };
