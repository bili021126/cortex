...
export class FixedRetry {
     ...
     */
    constructor(options = {}) {
        const { maxAttempts = 3, delayMs = 1000, maxDelayMs, retryableErrors = [], shouldRetry, } = options;
        // ── 参数校验 ──
        if (maxAttempts < 1) {
            throw new RangeError(`FixedRetry: maxAttempts must be ≥ 1, got ${maxAttempts}`);
        }
        if (delayMs < 0) {
            throw new RangeError(`FixedRetry: delayMs must be ≥ 0, got ${delayMs}`);
        }
        if (maxDelayMs !== undefined && maxDelayMs < 0) {
            throw new RangeError(`FixedRetry: maxDelayMs must be ≥ 0, got ${maxDelayMs}`);
        }
        if (maxDelayMs !== undefined && delayMs > maxDelayMs) {
            throw new RangeError(`FixedRetry: delayMs (${delayMs}) must be ≤ maxDelayMs (${maxDelayMs})`);
        }
...
//# sourceMappingURL=FixedRetry.js.map