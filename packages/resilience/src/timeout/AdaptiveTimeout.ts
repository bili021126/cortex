 ...
 * ```
 */
export class AdaptiveTimeout implements ITimeoutPolicy {
  /** 策略名称，用于日志和监控 */
  ...
  /** 超时后是否取消 pending 操作 */
  private readonly _cancelOnTimeout: boolean;

  /** 超时值更新回调 */
  private readonly _onTimeoutUpdate?: (newTimeoutMs: number, ema: number, lastDuration: number) => void;
  ...
  constructor(options: AdaptiveTimeoutOptions = {}) {
    ...
    if (!Number.isFinite(initialTimeoutMs) || initialTimeoutMs <= 0) {
      throw new RangeError(
        `AdaptiveTimeout: initialTimeoutMs must be a positive finite number, got ${initialTimeoutMs}`,
      );
    }
    if (!Number.isFinite(minTimeoutMs) || minTimeoutMs <= 0) {
      throw new RangeError(
        `AdaptiveTimeout: minTimeoutMs must be a positive finite number, got ${minTimeoutMs}`,
      );
    }
    if (!Number.isFinite(maxTimeoutMs) || maxTimeoutMs <= 0) {
      throw new RangeError(
        `AdaptiveTimeout: maxTimeoutMs must be a positive finite number, got ${maxTimeoutMs}`,
      );
    }
    if (minTimeoutMs > maxTimeoutMs) {
      throw new RangeError(
        `AdaptiveTimeout: minTimeoutMs (${minTimeoutMs}) must not exceed maxTimeoutMs (${maxTimeoutMs})`,
      );
    }
    if (initialTimeoutMs < minTimeoutMs || initialTimeoutMs > maxTimeoutMs) {
      throw new RangeError(
        `AdaptiveTimeout: initialTimeoutMs (${initialTimeoutMs}) must be between minTimeoutMs (${minTimeoutMs}) and maxTimeoutMs (${maxTimeoutMs})`,
      );
    }
    if (!Number.isFinite(multiplier) || multiplier <= 0) {
      throw new RangeError(
        `AdaptiveTimeout: multiplier must be a positive finite number, got ${multiplier}`,
      );
    }
    if (!Number.isFinite(alpha) || alpha <= 0 || alpha >= 1) {
      throw new RangeError(
        `AdaptiveTimeout: alpha must be a number in (0, 1), got ${alpha}`,
      );
    }
  ...
  private readonly _initialEma: number;
   ...
   */
  private _classifyAndHandleError<T>(
    err: unknown,
    elapsedMs: number,
