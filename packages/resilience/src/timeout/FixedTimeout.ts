 ...
 * ```
 */
export class FixedTimeout implements ITimeoutPolicy {
  /** 策略名称，用于日志和监控 */
  ...
  constructor(options: FixedTimeoutOptions) {
    if (!Number.isFinite(options.durationMs) || options.durationMs <= 0) {
      throw new RangeError(
        `FixedTimeout: durationMs must be a positive finite number, got ${options.durationMs}`,
      );
    }
   ...
   */
  private _combineSignals(external: AbortSignal, timeout: AbortSignal): AbortSignal {
    // 如果外部信号已中止，直接返回外部信号
    if (external.aborted) {
      return external;
    }
