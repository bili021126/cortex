 ...
 *
 * 每个状态封装其下的转换规则。
 * 状态模式消除了对 CircuitState 进行 switch-case 的散弹式修改。
 *
 * 注意：状态处理器「不」负责调用 recordSuccess/recordFailure，
 * 调用决策由 StateMachineCircuitBreaker.call() 统一处理。
 * 状态处理器仅负责「记录后的状态转换判断」。
 */
interface FsmState {
  /** 状态名称 */
  ...
  /**
   * 记录成功后的状态转换逻辑。
   * @param breaker 断路器实例（用于读取计数器和配置）
   * @returns 应切换到的目标状态，若无需切换则返回当前状态
   */
...
const CLOSED_STATE: FsmState = {
  ...
  onEnter(_breaker: StateMachineCircuitBreaker): void {
...
const OPEN_STATE: FsmState = {
  ...
  onRecordSuccess(_breaker: StateMachineCircuitBreaker): CircuitState {
    // OPEN 状态下不会放行正常调用，此方法不应被调用
    return 'OPEN';
  },

  onRecordFailure(_breaker: StateMachineCircuitBreaker): CircuitState {
    // OPEN 状态下不会放行正常调用，此方法不应被调用
    return 'OPEN';
  },
 ...
 * ```
 */
export class StateMachineCircuitBreaker implements ICircuitBreaker {
  /** 断路器名称（用于日志和监控） */
   ...
   * 试探落定后释放门闩，若 maxHalfOpenRequests > 1 则后续调用可依次成为新试探。
   */
  private _halfOpenProbe: Promise<unknown> | null = null;

  /** 滑动窗口调用记录（仅 sliding-window 策略使用） */
  private readonly _callRecords: Array<{ timestamp: number; success: boolean }> = [];
  ...
  /** 状态变更处理器列表 */
  private readonly _stateChangeHandlers: Array<(state: CircuitState, previous: CircuitState) => void> = [];
  ...
  async call<T>(fn: () => Promise<T>, fallback?: () => Promise<T>): Promise<T> {
    ...
    // ── 2. HALF_OPEN 状态：原子试探门闩——仅首个请求执行 fn，其余并发请求快速失败 ──
    if (this._handler === HALF_OPEN_STATE) {
   ...
   * 试探落定后统一裁决（通过 _recordSuccess/_recordFailure 触发状态转换）：
   *   - 成功 → 累加 _halfOpenSuccesses → 达到 maxHalfOpenRequests 则闭合
   *   - 失败 → 任意一次失败回到 OPEN
   * 门闩在裁决后释放——maxHalfOpenRequests > 1 时后续调用依次成为新试探（串行化）。
   *
   * @param fn 被保护的异步函数
  ...
  private async _handleHalfOpenCall<T>(
    ...
    try {
      ...
      if (fallback && this._handler === OPEN_STATE) {
        return await fallback();
      }
      throw err;
    }
  ...
  onStateChange(handler: (state: CircuitState, previous: CircuitState) => void): () => void {
    ...
    return () => { const i = this._stateChangeHandlers.indexOf(handler); if (i >= 0) this._stateChangeHandlers.splice(i, 1); };
  }
   ...
   */
  private async _handleOpenCall<T>(
    fn: () => Promise<T>,
    fallback: (() => Promise<T>) | undefined,
  ): Promise<T> {
    // 检查是否应进入半开试探
   ...
   */
  private _evictExpiredRecords(): void {
    if (this._resolvedOptions.strategy !== 'sliding-window') {
      return;
    }
