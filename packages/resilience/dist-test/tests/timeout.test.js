...
describe("FixedTimeout", () => {
    ...
    describe("execute with external signal", () => {
        it("should handle external cancellation", async () => {
            ...
            const promise = t.execute(async (_signal) => {
                // 永不 resolve，等待外部取消
                await new Promise(() => { });
                return "done";
            }, controller.signal);
        ...
        it("should handle pre-aborted external signal", async () => {
            ...
            const result = await t.execute(async (_signal) => {
                await delay(10);
                return "done";
            }, preAborted);
    ...
    describe("edge cases", () => {
        it("should clean up timeout resources after success", async () => {
            ...
            // 清理后不应再有 pending 计时器
            const activeTimers = setTimeoutSpy.mock.results.filter((r) => typeof r.value === "number" || typeof r.value === "object");
...
describe("AdaptiveTimeout", () => {
    ...
    describe("execute external cancellation", () => {
        ...
        it("should handle pre-aborted external signal", async () => {
            ...
            const result = await t.execute(async (signal) => {
                return "done";
            }, external);
    ...
    describe("onTimeoutUpdate callback", () => {
        it("should invoke callback when timeout value changes", async () => {
            ...
            expect(callback).toHaveBeenCalledWith(expect.any(Number), // newTimeoutMs
    ...
    describe("signal combination", () => {
        it("should handle external signal that aborts early", async () => {
            ...
            const resultPromise = t.execute(async (_signal) => {
