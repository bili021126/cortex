...
import {
  ...
  emitSkillReferenced,
  extractSkillUsageFromOutput,
  SkillTemplateEngine,
} from "@cortex/skill-kit";
import type { SkillJsonValidationResult } from "@cortex/skill-kit";
import { PipelinePriority, PipelineEventType, type SkillTemplate } from "@cortex/shared";
import type { IPipelineObserver, PipelineHandler } from "@cortex/shared";

// ── Mock PipelineObserver ──────────────────────

type EmittedEvent = { type: string; priority: string; payload: unknown };

class MockObserver implements IPipelineObserver {
  emitted: EmittedEvent[] = [];
  private handlers = new Map<string, PipelineHandler[]>();

  on(priority: string | number, handler: PipelineHandler): void {
    const key = String(priority);
    const arr = this.handlers.get(key) ?? [];
    arr.push(handler);
    this.handlers.set(key, arr);
  }
  off(priority: string | number, handler?: PipelineHandler): void {
    if (!handler) { this.handlers.delete(String(priority)); return; }
    const arr = this.handlers.get(String(priority));
    if (arr) this.handlers.set(String(priority), arr.filter((h) => h !== handler));
  }
  emit(event: { type: string | number; priority: string | number; payload: unknown; timestamp?: number }): void {
    ...
    if (handlers) for (const h of handlers) h(event as Parameters<PipelineHandler>[0]);
  }
}
...
describe("skill-pipeline", () => {
  it("should register and emit skill referenced events", () => {
    ...
    emitSkillReferenced(observer, [tmpl], "node-1", "Code" as any);
  ...
  it("should register pipeline handler that extracts skills on NodeComplete", () => {
    ...
    observer.emit({
      ...
      payload: { nodeId: "n1", agentType: "Code" as any, success: true, output: skillOutput },
      timestamp: Date.now(),
    });
