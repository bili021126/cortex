// ============================================================
// @cortex/skill-kit —— 技能步骤模板渲染引擎
//
// @file-overview
// 轻量级模板渲染引擎，提供技能步骤的 prompt 模板渲染能力。
// 原位于 @cortex/engine/components/skill-template-engine.ts，
// 横向解耦后迁回 @cortex/skill-kit。
//
// 不依赖任何外部模板引擎库。
//
// 支持的语法：
...
export interface TemplateDiagnostic {
  type: "missing_variable" | "fallback_used";
  variable: string;
  fallback?: string;
}
    ...
    return template.replace(variableRegex, (_match, expression: string) => {
      ...
      if (value === undefined || value === null) {
        if (fallback !== undefined) {
          this.diagnostics.push({ type: "fallback_used", variable: path, fallback });
          const fallbackValue = this.resolvePath(context, fallback);
          return this.safeRender(fallbackValue ?? fallback);
        }
        this.diagnostics.push({ type: "missing_variable", variable: path });
