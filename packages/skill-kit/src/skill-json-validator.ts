// ============================================================
// @cortex/skill-kit —— 外源技能 JSON 校验器
//
// @file-overview
// 负责校验 skills/*.json 等外源技能清单的结构和字段合法性，
// 并提供外源格式 → SkillTemplate 的转化桥。
// 原位于 @cortex/engine/components/skill-json-validator.ts，
...
import { AgentType, type FeedbackEntry, type SkillKind, type SkillTemplate, type Tag } from "@cortex/shared";
...
const VALID_AGENT_TYPES: ReadonlySet<string> = new Set(
  Object.values(AgentType) as string[],
);

/** 合法的技能状态值 */
const VALID_STATUSES: ReadonlySet<string> = new Set<SkillStatus>([
  "draft",
  "trial",
  "active",
  "deprecated",
...
/** 可选但需类型校验的字段 */
const OPTIONAL_TYPED_FIELDS: ReadonlyArray<{
  name: string;
  type: "string" | "number" | "array" | "object";
}> = [
...
const requiredFieldsValidator: SkillJsonValidator = {
  name: "required-fields",

  validate(data: Record<string, unknown>) {
    const errors: SkillJsonValidationError[] = [];
...
const agentTypeValidator: SkillJsonValidator = {
  name: "agent-type",
...
const statusValidator: SkillJsonValidator = {
  name: "status",
...
const numericFieldsValidator: SkillJsonValidator = {
  name: "numeric-fields",

  validate(data: Record<string, unknown>) {
    const errors: SkillJsonValidationError[] = [];
    const numericFields = [
      { key: "weight", code: "INVALID_WEIGHT" as const },
      { key: "adoptionCount", code: "INVALID_ADOPTION_COUNT" as const },
      { key: "rejectionCount", code: "INVALID_REJECTION_COUNT" as const },
    ];
...
const stepsValidator: SkillJsonValidator = {
  name: "steps",

  validate(data: Record<string, unknown>) {
    const warnings: SkillJsonValidationWarning[] = [];
...
const triggerTagsValidator: SkillJsonValidator = {
  name: "trigger-tags",
...
const createdAtValidator: SkillJsonValidator = {
  name: "created-at",
...
const VALIDATOR_REGISTRY: ReadonlyArray<SkillJsonValidator> = [
 ...
 * @returns 校验结果
 */
export function validateExternalSkillJson(json: unknown): SkillJsonValidationResult {
  const errors: SkillJsonValidationError[] = [];
  const warnings: SkillJsonValidationWarning[] = [];
  const infos: SkillJsonValidationInfo[] = [];
...
const AGENT_TYPE_TO_KIND: Record<string, SkillKind> = {
  cod: "action",
  rev: "action",
  analy: "thought",
  op: "action",
...
function resolveKind(data: Record<string, unknown>): SkillKind {
  if (typeof data.kind === "string") {
    const known: SkillKind[] = ["action", "thought", "workflow"];
    if (known.includes(data.kind as SkillKind)) return data.kind as SkillKind;
  }
...
function resolveStatus(raw: string | undefined): SkillTemplate["status"] {
  if (raw === "active") return "trial"; // 外部声明 active 不可信
  if (raw === "deprecated") return "deprecated";
  if (raw === "draft") return "trial";
  return "trial";
}
 ...
 */
export function externalJsonToSkillTemplate(
  data: Record<string, unknown>,
): SkillTemplate {
  const now = Date.now();

  // triggerTags：过滤纯字符串标签
  const triggerTags: Tag[] = Array.isArray(data.triggerTags)
    ? (data.triggerTags.filter((t): t is string => typeof t === "string" && t.trim().length > 0) as Tag[])
    : [];

  // steps：过滤纯字符串步骤
  const steps: string[] = Array.isArray(data.steps)
    ? data.steps.filter((s): s is string => typeof s === "string" && s.trim().length > 0)
    : [];
  ...
  return {
    ...
    feedbackHistory: Array.isArray(data.feedbackHistory)
      ? (data.feedbackHistory as FeedbackEntry[])
      : [],
...
export function importExternalSkill(
  json: unknown,
): { template: SkillTemplate | null; validation: SkillJsonValidationResult } {
  const validation = validateExternalSkillJson(json);
  if (!validation.valid) {
    return { template: null, validation };
  }
  const template = externalJsonToSkillTemplate(json as Record<string, unknown>);
