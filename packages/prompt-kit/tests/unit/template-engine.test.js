...
describe("PromptTemplateEngine", () => {
    ...
    describe("变量插值", () => {
        ...
        it("应支持 HTML 转义（启用时）", () => {
            ...
            const result = engineEscape.render("{{content}}", { variables: { content: '<script>alert("xss")</script>' } });
    ...
    describe("条件渲染 {{#if}}", () => {
        it("条件为 true 时应渲染内容", () => {
            const result = engine.render("{{#if hasMemory}}有记忆{{/if}}", defaultContext);
            expect(result).toBe("有记忆");
        });
        it("条件为 false 时应清空内容", () => {
            const result = engine.render("{{#if missing}}不应出现{{/if}}", defaultContext);
            expect(result).toBe("");
        });
        it("应支持 {{else}} 分支（条件为 false 时渲染 else）", () => {
            const result = engine.render("{{#if missing}}是{{else}}否{{/if}}", defaultContext);
            expect(result).toBe("否");
        });
        it("条件为真时应渲染 if 分支而非 else", () => {
            const result = engine.render("{{#if hasMemory}}有记忆{{else}}无记忆{{/if}}", defaultContext);
            expect(result).toBe("有记忆");
        });
    });
    describe("循环渲染 {{#each}}", () => {
        it("应遍历数组元素", () => {
            const result = engine.render("{{#each items}}{{this}}{{/each}}", defaultContext);
            expect(result).toBe("a\nb\nc");
        });
        it("非数组应返回空字符串", () => {
            const result = engine.render("{{#each missing}}不应出现{{/each}}", defaultContext);
            expect(result).toBe("");
        });
        it("应支持 index 变量", () => {
    ...
    describe("角色切换 {{#role}}", () => {
        it("应注入角色 persona", () => {
            ...
            const result = engine.render("{{#role nahida}}你好，我是纳西妲{{/role}}", ctx);
            expect(result).toContain("[nahida 角色]");
            expect(result).toContain("智慧与知识的化身");
            expect(result).toContain("你好，我是纳西妲");
        });
        it("无角色 persona 时应仅渲染内容", () => {
            const result = engine.render("{{#role unknown}}普通内容{{/role}}", defaultContext);
    ...
    describe("边界情况", () => {
        ...
        it("自闭合指令与块指令可共存", () => {
            const result = engine.render("日期：{{#date YYYY-MM-DD}}\n{{#if hasMemory}}有记忆{{/if}}", defaultContext);
