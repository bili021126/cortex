...
import type { PromptTemplate, PromptBlock, PromptAssembly, PromptContext } from "../../src/types.js";

describe("PromptAssembler", () => {
  let engine: PromptTemplateEngine;
  let assembler: PromptAssembler;
  let defaultTemplate: PromptTemplate;
  let defaultAssembly: PromptAssembly;
  ...
  describe("块过滤", () => {
    ...
    it("自定义 blockFilter 应过滤块", async () => {
      const assembly: PromptAssembly = {
        ...
        context: {
          ...defaultContext,
          blockFilter: (block: PromptBlock) => block.type !== PromptBlockType.Instruction,
        },
      };
  ...
  describe("身份锚点注入", () => {
    ...
    it("已包含锚点不应重复注入", async () => {
      ...
      const result = await assembler.assemble(template, assembly);
      const anchorBlocks = result.renderedBlocks.filter(
        (b) => b.id === "shared-identity-anchor",
      );
