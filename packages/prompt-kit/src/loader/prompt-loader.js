 ...
 */
import { PromptErrorCode, PromptBlockType } from "../types.js";
...
export class PromptLoader {
    ...
    async load(templateId, options) {
        ...
        // 3. 未找到
        throw new PromptError(`Prompt 模板 "${templateId}" 未在任何来源中找到`, PromptErrorCode.TEMPLATE_NOT_FOUND, { templateId });
    }
    /**
     * 从文件路径加载 PromptTemplate。
     * 等价于 load(templateId)，但 id 从文件路径推导。
     */
    async loadFromFile(filePath, _options) {
        // 查找 file-source 并委托
        const fileSource = this.sources.get("file");
        if (!fileSource) {
            throw new PromptError("文件来源未注册，无法从文件加载", PromptErrorCode.LOAD_FAILED, { filePath });
        }
        // file-source 的 load 方法接受路径作为 templateId
        const template = await fileSource.load(filePath);
        if (!template) {
            throw new PromptError(`文件加载失败: ${filePath}`, PromptErrorCode.LOAD_FAILED, { filePath });
        }
        return template;
    }
    /**
     ...
     */
    async loadFromConfig(configKey, _options) {
        const configSource = this.sources.get("config");
        if (!configSource) {
            throw new PromptError("配置来源未注册，无法从配置加载", PromptErrorCode.LOAD_FAILED, { configKey });
        }
        const template = await configSource.load(configKey);
        if (!template) {
            throw new PromptError(`配置键 "${configKey}" 未找到对应模板`, PromptErrorCode.TEMPLATE_NOT_FOUND, { configKey });
        }
        return template;
    }
    /**
     * 从内联字符串加载。
     */
    loadFromInline(id, content, _options) {
        const inlineSource = this.sources.get("inline");
        if (!inlineSource) {
            throw new PromptError("内联来源未注册", PromptErrorCode.LOAD_FAILED, { id });
        }
