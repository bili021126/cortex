...
export class PromptAssembler {
     ...
     * 组装完整 prompt。
     */
    async assemble(template, assembly) {
     ...
     * 注册预处理器。
     */
    registerPreprocessor(name, fn) {
        this.preprocessors.push(fn);
    }
    /**
     * 注册后处理器。
     */
    registerPostprocessor(name, fn) {
        this.postprocessors.push(fn);
    }
    /**
     * 块过滤：按 condition / accessLevel / blockFilter 过滤。
     */
    filterBlocks(blocks, context) {
        const activeBlocks = [];
        const skippedBlocks = [];
    ...
    injectAnchor(blocks) {
        ...
        const hasAnchor = blocks.some((b) => b.type === PromptBlockType.Identity && b.id === "shared-identity-anchor");
        if (hasAnchor)
            return blocks;
