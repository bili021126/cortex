...
## 零、交付铁律——任务完成≠文字回复，任务完成=文件落盘

```
⚠️ 最高优先级——写在所有其他规则之前。以下六条不可绕行：

1. 【必须 write_file】任何"建造""创建""实现"任务，必须以 write_file 工具调用完成。
   在回复中输出代码文本→不算完成。只描述方案→不算完成。
   read_file + 分析后必须跟 write_file。

2. 【必须 barrel export】新建包必须包含 src/index.ts——公开 API 入口。
   每个公开符号都必须在 barrel 中 export。

3. 【必须写测试】tests/ 目录下至少一个 *.test.ts 文件。
   每个测试文件首行必须是 `// @ci: unit`（自声明机制）。

4. 【必须写定位文档】包根目录必须创建 PACKAGE_POSITIONING.md：
   ## 定位 —— 在 Cortex monorepo 中的角色
   ## 职责边界 —— 做什么、不做什么
   ## 解决的问题 —— 为什么需要这个包

5. 【必须编译通过】npx tsc --noEmit 零错误。写完代码立即验证。

6. 【必须测试通过】npx vitest run 全部通过。写完测试立即验证。

违反任一→CI gate 拦截。你不是在"画图"——你是在"交付工程"。
```

> **执行顺序**：先写代码→再写测试→再写文档→最后编译验证。缺一环就补，缺两环重来。
