...
import { cwd, exit, argv, stdout } from "node:process";
...
interface PkgInfo {
  ...
  /** 是否为根包 */
  isRoot: boolean;
  /** layer 层级（0=shared, 1=llm/testing, 2=engine）- 来自架构分析 */
  layer: number;
}
...
/** 从包名中提取简写 ID（如 "@cortex/engine" → "engine"） */
...
/** 查找项目根（含 pnpm-workspace.yaml 的目录） */
function findProjectRoot(start: string): string {
  let dir = resolve(start);
  for (let i = 0; i < 20; i++) {
    if (existsSync(join(dir, "pnpm-workspace.yaml"))) return dir;
    if (existsSync(join(dir, "package.json")) &&
        existsSync(join(dir, "packages"))) return dir;
...
function collectPackages(projectRoot: string): PkgInfo[] {
  ...
  // layer 映射（来自架构分析报告：shared←llm←engine，testing 独立）
...
function buildEdges(
  ...
  for (const dep of deps) {
    ...
    // 排除 devDependencies（除非 --include-dev）
...
function detectDrifts(
  ...
  ignoreList: string[],
  verbose: boolean,
): { drifts: DriftItem[]; allDeps: Record<string, Record<string, string>> } {
  // 按依赖名分组，过滤掉 workspace:* 内部包
  const groups: Record<string, DepEntry[]> = {};
  const allDeps: Record<string, Record<string, string>> = {};

  for (const dep of deps) {
    // workspace:* 的 cortex 内部包不参与漂移检测
...
function formatText(output: AnalyzerOutput, verbose: boolean): string {
  ...
  if (edges.length === 0) {
    ...
    if (sortedFrom.length > 0) {
      lines.push("");
      lines.push("  文本依赖图（→ 表示依赖方向）:");
      const maxDepth = Math.max(...output.packages.filter((p) => !p.isRoot).map((p) => p.layer), 0);
      for (let layer = 0; layer <= maxDepth; layer++) {
        const layerPkgs = output.packages
          .filter((p) => !p.isRoot && p.layer === layer)
          .map((p) => p.id)
          .sort();
        if (layerPkgs.length === 0) continue;
        const indent = "  " + "  ".repeat(layer * 2);
        const pkgStr = layerPkgs.join("  ");
        const arrow = layer > 0 ? " ↑ " : "    ";
        lines.push(`${indent}L${layer}: ${pkgStr}`);
        if (layer < maxDepth) {
          lines.push(`${indent}${arrow}  (被上层依赖)`);
        }
      }
    }
  }
  ...
  // DOT 格式提示
  lines.push("  💡 Graphviz DOT: 可用 `--json` 获取 DOT 格式，或重定向到 .dot 文件");
  lines.push("  💡 命令: npx tsx tools/monorepo-analyzer.ts --json | jq '.dependencyGraph.dot' -r > graph.dot");
  lines.push("");
  ...
  // ── 依赖快照 ──
  lines.push("  ─── 依赖快照 ───");
  const depEntries = Object.entries(output.allDeps);
  if (depEntries.length > 0) {
    // 计算列宽
    const nameW = Math.max(22, ...depEntries.map(([n]) => cjkWidth(n))) + 2;
    const countW = 8;
    const verW = 24;

    const h = `${padDisplay("依赖名", nameW)} ${"次数".padStart(countW)} ${"版本".padEnd(verW)} 涉及包`;
    lines.push(h);
    lines.push("  " + "─".repeat(cjkWidth(h)));

    for (const [depName, pkgVersions] of depEntries.sort()) {
      const uniqueVers = [...new Set(Object.values(pkgVersions))];
      const count = Object.keys(pkgVersions).length;
      const pkgs = Object.keys(pkgVersions).join(", ");

      const namePart = padDisplay(depName, nameW);
      const countPart = String(count).padStart(countW);
      const verStr = uniqueVers.length === 1
        ? uniqueVers[0]
        : uniqueVers.length <= 2
          ? uniqueVers.join(" | ")
          : `${uniqueVers[0]} | ... (${uniqueVers.length}个版本)`;
      const verPart = verStr.length > verW ? verStr.substring(0, verW - 1) + "…" : verStr.padEnd(verW);

      const isDrifted = output.drifts.some((d) => d.dependency === depName);
      const mark = isDrifted ? "  ⚠️" : "";

      lines.push(`  ${namePart} ${countPart} ${verPart} ${pkgs}${mark}`);
    }
  }
  lines.push("");
...
function main(): void {
  ...
  用法:
    npx tsx tools/monorepo-analyzer.ts [选项]
  ...
  示例:
    npx tsx tools/monorepo-analyzer.ts
    npx tsx tools/monorepo-analyzer.ts --json
    npx tsx tools/monorepo-analyzer.ts --json --output report.json
    npx tsx tools/monorepo-analyzer.ts --ignore "@types/node" --ignore "uuid"
    npx tsx tools/monorepo-analyzer.ts --include-dev --verbose
...
`);
  ...
  try {
    ...
    if (drifts.length > 0 && cycles.length > 0) status = "drift"; // drift 优先级高
